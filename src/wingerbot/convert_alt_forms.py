        ...
        if header == "Alternative forms":
            ...
            def merge_alt(m, could_parse, issued_warning):
                ...
                def extract_qualifiers_from_before_after(text_to_parse, lang):
                    returned_qualifiers = []
                    mm = re.search(
                        r"^\{\{(?:sense|s|q|i|qual|qualifier|qf|gloss|gl|a|accent)\|(?:\{\{[^{}]*\}\}|[^{}])*\}\}$",
                        text_to_parse,
                    )
                    if mm:
                ...
                for i in range(1, len(altforms), 2):
                    ...
                    if i == 1:
                        ...
                        pagemsg(
                            "WARNING: Saw different language %s on line from first language %s: %s"
                        ...
                        if mm:
                            ...
                            if saw_qualifier_gap:
                                if len(altforms) > 3:
                                    ...
                                    pagemsg(
                                        "WARNING: Multiple (%s) occurrences of {{alt}} with tags, can't merge: %s"
                ...
                if mergedt is None:
                    pagemsg(
                        "WARNING: Internal error: Didn't find any {{alt}}/{{alter}} templates in line: %s" % origline
                    )
                    issued_warning[0] = True
                    return origline
                could_parse[0] = True
                    ...
                    if len(line_parts) == 1:
                        ...
                        for partno, line_part in enumerate(line_parts):
                            ...
                            if new_line_part == line_part and not could_parse:
                                warning(
                                    "Part %s: Unable to parse ==Alternative forms== line part: %s"
                                    % (partno + 1, line_part)
                                )
                                break
