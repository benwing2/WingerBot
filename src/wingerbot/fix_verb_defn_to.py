        ...
        def add_to_to_defn(m):
            ...
            for sec in secs:
                if sec in [",", ";", ":"]:
                    ...
                    sec.startswith("''")
                    or sec.startswith("{{")
                    or (sec.startswith("(") and sec.endswith(")"))
                    or sec.startswith("[[")
                    or (sec.startswith("[") and sec.endswith("]"))
                ):
                    if segment:
            ...
            for grouped_segment in grouped_segments:
                ...
                if i < len(grouped_segment) and (
                    not re.search(r"^ *(to|etc|e\.\g.|when|then|especially|usually|or|and)\b", grouped_segment[i])
                    and not re.search(r"^\[\[[^ ]*?[a-z]*[aeiouy][a-z]*ing\]\]$", grouped_segment[i])
                    and not re.search(r"^ *[a-z]*[aeiouy][a-z]*ing\b", grouped_segment[i])
                ):
                    grouped_segment[i] = re.sub("^( *)", r"\1to ", grouped_segment[i])

            # Rejoin segments and segment groups.
            newdefn = "".join(
                "".join(grouped_segment) + sep for grouped_segment, sep in zip(grouped_segments, grouped_segments_sep)
            )
