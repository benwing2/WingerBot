...
def process_text_on_page(p):
            ...
            if labelobj.label in lang_specific_labels:
                p.msg(
                    ...
                    % (
                        labelobj.label,
                        (
                            " (existing label is canonical)"
                            if lang_specific_labels[labelobj.label].label == labelobj.label
                            else " (existing label is alias for '%s')" % lang_specific_labels[labelobj.label].label
                        ),
                    )
                )
            else:
                    ...
                    if alias in lang_specific_labels:
                        p.msg(
                            ...
                            % (
                                ...
                                (
                                    " (existing label is canonical)"
                                    if lang_specific_labels[alias].label == alias
                                    else " (existing label is alias for '%s')" % lang_specific_labels[alias].label
                                ),
                            )
                        )
                    else:
    ...
    for labelobj in regional_labelobjs_by_lang[langcode]:
        ...
        def is_canonical(label):
            return (
                " (existing label is canonical)"
                if lang_specific_labels[label].label == label
                else " (existing label is alias for '%s')" % lang_specific_labels[label].label
            )
    ...
    if num_incorporated_regional_labelobjs > 0:
        ...
        notes = [
            "move %s label(s) (%s) from [[%s]] to lang-specific labels data module"
...
regional_label_data = clean_label_module.process_text_on_page_for_label_objects(
    blib.ProcessPageParams(args, 1, args.regional_data_module, regional_module_text, None)
)
        ...
        if isinstance(labelobj, LabelData):
            ...
            if args.categories:
                ...
                for regcat in labelobj.regional_categories.cats:
                    ...
                    pagemsg(
                        "For regional label '%s', regional category '%s', languages: %s"
                        % (
                            labelobj.label,
                            regcat,
                ...
                for plaincat in labelobj.plain_categories.cats:
                    ...
                    pagemsg(
                        "For regional label '%s', plain category '%s', languages: %s"
                        % (
                            labelobj.label,
                            plaincat,
            ...
            if not langs_for_label:
                ...
                keep = True
                lines_before_label.append(
                    "-- WARNING: No existing languages or categories associated with label; add to `langs` as needed"
                )
            elif len(langs_for_label) > 3:
                pagemsg(
                    "WARNING: Not removing regional label '%s' with %s languages %s > 3"
                    % (labelobj.label, len(langs_for_label), ",".join(langs_for_label))
                )
                keep = True
            elif (
                hasattr(labelobj.fields, "aliases")
                and len(labelobj.fields.aliases.value) > 1
                and len(langs_for_label) > 1
            ):
                pagemsg(
                    "WARNING: Not removing regional label '%s' with %s aliases %s > 1 and %s languages %s > 1"
                    % (
                        labelobj.label,
                        len(labelobj.fields.aliases.value),
                        ",".join(labelobj.fields.aliases.value),
                        len(langs_for_label),
                        ",".join(langs_for_label),
                    )
                )
                keep = True
            if keep:
                        ...
                        pagemsg(
                            ...
                            % (
                                (
                                    ...
                                    (
                                        ""
                                        if existing_labelobj.label == labelobj.label
                                        else " (existing label is alias for '%s')" % existing_labelobj.label
                                    ),
                                )
                            )
                        )
                ...
                pagemsg(
                    "Removing label %s from [[%s]], moved to lang-specific modules"
                ...
                if [x for x in lines_before_label if x.strip()]:
                    pagemsg(
                        "WARNING: Skipped regional label '%s' has non-empty literal line(s) before it, inserting them raw"
                        % labelobj.label
                    )
                    filtered_regional_label_data.append(lines_before_label)
            lines_before_label = []
