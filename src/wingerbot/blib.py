...
def append_param_to_chain(
    ...
    while True:
        paramno += 1
        next_param = firstparam if paramno == 1 and not is_number else "%s%s" % (parampref, paramno)
...
talk_prefixes = [
    ...
    "Wiktionary:Tea room",
    "Wiktionary:Requests",
    "Wiktionary:Requested",
    "Wiktionary:Wikimedia Tech News",
    "Wiktionary:Feedback",
    "Wiktionary:Word of the day",
    "Wiktionary:Foreign Word of the Day",
    "Wiktionary:Sandbox",
    "Wiktionary:Votes",
...
def references(
    ...
    if templates_first:
        ...
        rest_pageiter = page.getReferences(
            namespaces=namespaces, only_template_inclusion=only_template_inclusion, filter_redirects=filter_redirects
        )
    if include_page:
...
def check_cat_filters(page, filter_cats_regex, prune_cats_regex, verbose=False):
    if filter_cats_regex:
        ...
        if not re.search(filter_cats_regex, this_cat):
            if verbose:
                msg(
                    "Skipping category '%s' as it doesn't match --filter-cats regex '%s'"
                    % (this_cat, filter_cats_regex)
                )
            return False
    if prune_cats_regex:
        this_cat = re.sub("^Category:", "", str(page.title()))
...
def cat_subcats(
    ...
    if isinstance(page, str):
        if not page.startswith("Category:"):
            page = "Category:" + page
        page = pywikibot.Category(site, page)
    pageiter = yield_subcats(
        page,
        seen,
        filter_cats_regex=filter_cats_regex,
        prune_cats_regex=prune_cats_regex,
        do_this_page=do_this_page,
        recurse=recurse,
        verbose=verbose,
    )
...
class ProcessItems:
    ...
    def should_process(self, item):
        ...
        if self.skip_ignorable_pages and page_should_be_ignored(self.get_name(item)):
            pywikibot.output(
                "Page %s %s: page has a prefix or suffix indicating it should not be touched, skipping"
    ...
    for current in items:
        ...
        if i % steps == 0:
            ...
            if isinstance(endprefix, int):
                ...
                seconds_left_str = "1 second" if seconds_left_in_minute == 1 else "%s seconds" % seconds_left_in_minute
                minutes_left_str = (
                    ""
                    if minutes_left_in_hour == 0
                    else "1 minute" if minutes_left_in_hour == 1 else "%s minutes" % minutes_left_in_hour
                )
                hours_left_str = "" if hours_left == 0 else "1 hour" if hours_left == 1 else "%s hours" % hours_left
...
def create_argparser(
    ...
    if include_pagefile:
        ...
        parser.add_argument(
            "--pages-from-find-regex",
            help="Read pages to process (and their indices) from previous find_regex.py output.",
        )
        parser.add_argument(
            "--pages-from-previous-output",
            help="Read pages to process (and their indices) from previous output of the form 'Page ### PAGENAME: '.",
        )
        ...
        parser.add_argument(
            "--skip-cats", help="List of categories whose pages will not be processed, comma-separated."
        )
        parser.add_argument(
            "--do-subcats",
            action="store_true",
        ...
        parser.add_argument(
            "--prune-cats",
            help="Regex to use to prune categories when processing subcategories recursively; any categories matching the regex will be skipped along with any of their subcategories (unless reachable in some other manner).",
        )
        parser.add_argument(
            "--filter-cats",
            help="Regex to use to filter categories when processing subcategories recursively; any categories not matching the regex will be skipped along with any of their subcategories (unless reachable in some other manner).",
        )
        parser.add_argument("--refs", help="List of references to process, comma-separated.")
        parser.add_argument(
            "--pages-and-refs", help="List of pages to process, comma-separated, along with references to those pages."
        )
        parser.add_argument(
            "--templates-first",
            help="When processing references, do templates before other pages.",
            action="store_true",
        )
        parser.add_argument("--specials", help="Special pages to do, comma-separated.")
        parser.add_argument(
            "--do-specials-cat-pages",
            action="store_true",
        ...
        parser.add_argument("--prefix-pages", help="Do pages with these prefixes, comma-separated.", default="-")
        parser.add_argument("--prefix-namespace", help="Namespace of pages to do using --prefix-pages.")
        parser.add_argument(
            "--prefix-redirects-only", help="Restrict --prefix-pages to redirects only.", action="store_true"
        )
        ...
        parser.add_argument(
            "--filter-pages-not", help="Regex to use to filter page names; only includes pages not matching this regex."
        )
        ...
        parser.add_argument(
            "--no-output", help="In conjunction with --find-regex, don't output processed text.", action="store_true"
        )
        parser.add_argument(
            "--skip-ignorable-pages", help="Skip 'ignorable' pages (talk pages, user pages, etc.).", action="store_true"
        )
        parser.add_argument(
            "--suppress-skipped-messages", help="Suppress 'Skipped, no changes' messages.", action="store_true"
        )
        # Not implemented yet.
...
def args_has_non_default_pages(args):
    return not not (
        args.pages
        or args.pagefile
        or args.pages_from_find_regex
        or args.pages_from_previous_output
        or args.cats
        or args.category_file
        or args.refs
        or args.specials
        or args.contribs
        or args.prefix_namespace
...
def do_pagefile_cats_refs(
    ...
    ref_namespaces=None,
    templates_first=False,
    canonicalize_pagename=None,
    skip_ignorable_pages=False,
    seen=None,
    process_index=lambda x: x,
):
            ...
            for allowed_namespace in args_namespaces:
                if type(allowed_namespace) is int:
                    ...
                    if (
                        ...
                        or namespace.custom_prefix() in coloned_allowed_namespace
                    ):
                        return False
            return True
        return False
    ...
    if stdin and (args.stdin or args.find_regex or args.begin_end):
        ...
        if args.find_regex:
            ...
            for index, (_, pagetitle, text, prev_comment) in iter_items(
                index_pagetitle_text_comment,
                start,
                end,
                get_name=lambda x: x[1],
        ...
        if args.pages_from_find_regex:
            ...
            for index, (_, pagetitle, _, _) in iter_items(
                index_pagetitle_text_comment,
                start,
                end,
                get_name=lambda x: x[1],
        ...
        if args.pages_from_previous_output:
            ...
            for index, (_, pagetitle) in iter_items(
                index_pagetitle,
                start,
                end,
                get_name=lambda x: x[1],
        ...
        if args.cats or args.category_file:
            ...
            def do_cat(cat):
                if args.do_cat_and_subcats:
                    for index, subcat in cat_subcats(
                        cat,
                        start,
                        end,
                        seen=seen,
                        filter_cats_regex=args_filter_cats,
                    ...
                    for index, page in cat_articles(
                        ...
                        prune_cats_regex=args_prune_cats,
                        recurse=args.recursive,
                        track_seen=not args.no_track_seen,
                        verbose=args.verbose,
                    ):
        ...
        if args.refs:
            for ref in split_arg(args.refs):
                ...
                for index, page in references(
                    ref, start, end, namespaces=args_ref_namespaces, templates_first=templates_first
                ):
        ...
        if args.pages_and_refs:
            for page_and_ref in split_arg(args.pages_and_refs):
                ...
                for index, page in references(
                    ...
                    namespaces=args_ref_namespaces,
                    templates_first=templates_first,
                    include_page=True,
                ):
                    process_pywikibot_page(index, page)
        if args.specials:
            for special in split_arg(args.specials):
                for index, page in query_special_pages(
                    special,
                    start,
                    end,
                    filter_cats_regex=args_filter_cats,
                    ...
                    if args.do_specials_refs:
                        # We don't use ref_namespaces here because the user might not want it.
                        for index2, page2 in references(
                            title, namespaces=args_ref_namespaces, templates_first=templates_first
                        ):
        ...
        for cat in default_cats:
            for index, page in cat_articles(
                ...
                seen=seen,
                filter_cats_regex=args.filter_cats,
                prune_cats_regex=args.prune_cats,
                recurse=args.recursive,
                track_seen=not args.no_track_seen,
                verbose=args.verbose,
            ):
...
def try_repeatedly[T](
    ...
    def log_exception(txt, e, skipping=False):
        txt = "WARNING: %s when trying to %s%s: %s" % (txt, operation, ", skipping" if skipping else "", str(e))
        errandpagemsg(txt)
        ...
        if isinstance(val, list):
            ...
            if len(existing_pos) > len(val):
                ...
                raise ParseException(
                    "For modifier %s, saw %s existing value(s) and trying to set only %s value(s)"
            ...
            for thispos, (thismod, thisval) in enumerate(self.modifiers):
                if thismod == mod:
                    if pos is None:
                        ...
                        raise ParseException(
                            "Modifier %s occurs twice when trying to set modifier, in positions '%s' and '%s'"
                            % (mod, pos, thispos)
                        )
            if pos is None:
        ...
        for t in parsed.filter_templates():
            ...
            def doparam(langparam: Langparam, param: ProcessLinksParam) -> bool:
                ...
                try:
                    ...
                    pagemsg("Exception processing lang %s, param %s in template %s: %s" % (tlang, param, str(t), e))
                    return False
            ...
            def doparam_checking_alt(
                ...
                if isinstance(other_lang_param, (str, list)):
                    ...
                    if other_lang_val:
                        pagemsg(
                            "Skipping param %s=%s with alt param %s=%s because it is in a different lang %s=%s: %s"
                ...
                if other_lang_param:
                    ...
                    if m:
                        ...
                        pagemsg(
                            "Skipping param %s=%s because of it begins with other-language prefix '%s:': %s"
                ...
                if check_inline_modifiers and "<" in paramval:
                    try:
                        ...
                        if altval:
                            pagemsg(
                                "WARNING: Found inline modifier in param %s=%s along with alt param %s=%s, can't process: %s"
                                % (param, paramval, altparam, altval, str(t))
                            )
                            return False
                        if other_lang_param and inline_mod.get_modifier("lang") is not None:
                            pagemsg(
                                "Skipping param %s=%s because of inline 'lang' modifier: %s" % (param, paramval, str(t))
                            )
                            return False
                        ...
                        pagemsg(
                            "WARNING: Exception processing lang %s, param %s=%s in template %s: %s"
            ...
            # or  {{suffix|LANG|<FOREIGNTEXT>|<FOREIGNTEXT>|...}}
                ...
                for i in range(1, maxind + 1):
                    ...
                    doparam_checking_alt(
                        "1",
                        str(i + offset),
                        "alt" + str(i),
                        "tr" + str(i),
                        other_lang_param="lang" + str(i),
                ...
                for i in range(1, maxind + 1):
                    ...
                    if term != ";":  # semicolons are ignored for indexed params
                        ...
                        doparam_checking_alt(
                            "1",
                            str(i + 1),
                            index_param("alt", termind),
                            index_param("tr", termind),
                            check_inline_modifiers=True,
                        )
            elif tn == "form of":
            ...
            #    doparam("lang", ("separate", "2", None))
                ...
                for p in ["card", "ord", "adv", "mult", "dis", "coll", "frac", "optx", "opt2x"]:
                    if getp(p + "alt"):
                ...
                if getp("passage") or getp("text"):
                    doparam(
                        ...
                        (
                            "separate",
                            "passage" if getp("passage") else "text",
                ...
                while True:
                    ...
                    doparam_checking_alt(
                        "1", str(i + 1), index_param("alt", i), index_param("tr", i), check_inline_modifiers=True
                    )
                    i += 1
                ...
                while getp(str(i + 2)):
                    doparam_checking_alt(
                        "2", str(i + 2), index_param("alt", i), index_param("tr", i), check_inline_modifiers=True
                    )
                    i += 1
                if include_notforeign:
                    doparam("1", ("notforeign",))
    ...
    * `subsection_levels` is a dictionary mapping section indices to their header levels (as determined by the number of
    ...
    for j in range(2, len(subsections), 2):
        ...
        if not m:
            if pagemsg:
                pagemsg(
                    "WARNING: Internal error: Can't match subsection header: %s" % (subsections[j - 1].rstrip("\n"))
                )
        else:
            ...
            if left_equals != right_equals:
                if pagemsg:
                    pagemsg(
                        "WARNING: Found %s equalsigns on the left but %s equal signs on the right, assuming smaller one: %s"
...
def replace_in_text(
    text, curr, repl, pagemsg, count=-1, no_found_repl_check=False, abort_if_warning=False, is_re=False
):
    if is_re:
    ...
    if repl_curr_diff == 0:
        ...
        if ratio == int(ratio):
            if int(ratio) > 1:
                pagemsg("WARNING: Replaced %s occurrences of curr=%s with repl=%s" % (int(ratio), curr, repl))
                if abort_if_warning:
                    return text, False
        else:
            pagemsg(
...
def compare_new_and_old_template_forms(
    ...
    for form in set(old_forms.keys()) | set(new_forms.keys()):
        if form not in new_forms:
            warn(
                "for original %s and new %s, form %s=%s in old forms but missing in new forms"
        ...
        if form not in old_forms:
            warn(
                "for original %s and new %s, form %s=%s in new forms but missing in old forms"
        ...
        if nforms != oforms:
            warn(
                "for original %s and new %s, form %s=%s in old forms but =%s in new forms"
...
def yield_text_from_find_regex(lines, verbose):
    ...
    while True:
        ...
        if in_multiline and re.search("^-+ end text -+$", line):
            ...
            # else:
            m = re.search(
                "^Page ([0-9]+) (.*): (?:Would save with comment|Skipped, no changes; previous comment) = (.*)$", line
            )
            if m:
                ...
                if m:
                    ...
                    if comment is not None and (pagenum != comment_pagenum or pagename != comment_pagename):
                        errmsg(
                            "WARNING: Processing text for index %s, page '%s' but saw comment '%s' for different index %s, page '%s'; ignoring"
