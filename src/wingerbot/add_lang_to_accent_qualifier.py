    ...
    def hack_templates(sectext, langname, langnamecode=None, is_citation=False):
        ...
        for i, line in enumerate(lines):
            ...
            newline = re.sub(
                ...
                lambda m: incorporate_a_into_pron_template(m, accent_first=True),
                line,
            )
            newline = re.sub(
                r"(\{\{\s*(?:IPA|IPA-lite)\s*\|(?:\{\{[^{}]*\}\}|[^{}])+\}\})\s*(\{\{\s*(?:a|accent)\s*\|(?:\{\{[^{}]*\}\}|[^{}=])+\}\})",
                lambda m: incorporate_a_into_pron_template(m, accent_first=False),
                newline,
            )
            newline = re.sub(
            ...
            newline = re.sub(
                ...
                lambda m: incorporate_a_into_enPR(m, accent_first=True),
                newline,
            )
            newline = re.sub(
                r"(\{\{\s*enPR\s*\|(?:\{\{[^{}]*\}\}|[^{}])+\}\})\s*(\{\{\s*(?:a|accent)\s*\|(?:\{\{[^{}]*\}\}|[^{}=])+\}\})",
                lambda m: incorporate_a_into_enPR(m, accent_first=False),
                newline,
            )
            if newline != line:
