...
inflection_of_templates = ["inflection of", "infl of", "noun form of", "verb form of", "adj form of", "participle of"]
    ...
    for i, tag in enumerate(ts):
        if "//" in tag:
            ...
            for single_tag in single_tags:
                tag_sets.extend(split_multipart_tag_set(pre_tags + [single_tag] + post_tags))
            return tag_sets
    return [ts]
        ...
        for template in inflection_of_templates:
            ...
            def combine_adjacent_inflections(m: re.Match):
                ...
                while j < len(inflections):
                    ...
                    if (
                        prev_lang == this_lang
                        and prev_lemma == this_lemma
                        and prev_alt == this_alt
                        and prev_tr == this_tr
                        and prev_enclitic == this_enclitic
                        and prev_gloss == this_gloss
                        ...
                        notes.append(
                            ...
                            % construct_abbreviated_template(template, this_lang, this_lemma)
                        )
                        ...
                        pagemsg("Replaced %s + %s with %s" % (inflections[j - 2], inflections[j], str(t)))
                        ...
                        pagemsg(
                            "Unable to combine %s with %s because %s differ"
        ...
        if re.search("^[0-9]+$", pname):
            if int(pname) >= term_param + 2:
                if pval:
                    ...
                    notes.append("remove empty tags from %s" % construct_abbreviated_template(tname(t), lang, term))
...
def combine_adjacent_tags_into_multipart(
    tn,
    lang,
    lemma,
    tags,
    ...
    pagemsg,
    warn,
    multipart_list_tag_to_parts=multipart_list_tag_to_parts,
    tag_to_canonical_form_table={},
    dont_combine_tags=None,
):
    notes = []
    origtags = tags
    while True:
                        ...
                        if len(cur_tag_set) == len(tag_set):
                            ...
                            for i, (tag1, tag2) in enumerate(zip(cur_tag_set, tag_set)):
                                ...
                                if mismatch_ind is None:
                                    warn(
                                        "Two identical tag sets: %s and %s in %s"
                                    ...
                                    if dont_combine_tags:
                                        ...
                                        for tag in tag1_tag2:
                                            if tag in dont_combine_tags:
                                                warn(
                                                    "Would combine tag sets %s and %s on %s but %s is in dont_combine_tags"
        ...
        if tag_set_group_by_style["adjacent-first"] != tag_set_group_by_style["all-first"]:
            ...
            if num_adjacent_first_combos < num_all_first_combos:
                pagemsg(
                    "Preferring adjacent-first result %s (%s combinations) to all-first result %s (%s combinations)"
                    % (
                ...
                pagemsg(
                    "Preferring all-first result %s (%s combinations) to adjacent-first result %s (%s combinations)"
                    % (
                        join_tag_set_group(tag_set_group_by_style["all-first"]),
                        num_all_first_combos,
                        join_tag_set_group(tag_set_group_by_style["adjacent-first"]),
                ...
                pagemsg(
                    "Adjacent-first and all-first combination style different but same #combinations %s, preferring adjacent-first result %s to all-first result %s"
                    % (
                        num_adjacent_first_combos,
                        join_tag_set_group(tag_set_group_by_style["adjacent-first"]),
                        join_tag_set_group(tag_set_group_by_style["all-first"]),
...
def put_back_new_inflection_of_params(
    ...
    if (
        convert_to_more_specific_template
        ...
        if altparam:
            t.add("3", altparam)
        notes.append(
            "replace {{%s|%s|%s|...|%s}} with {{%s|%s|%s}}" % (old_tn, lang, term, "|".join(tags), tempname, lang, term)
        )
