...
def default_dim_1(lemma, final_multisyllable_stress=False, modifier_final_multisyllable_stress=False, first_only=False):
    if first_only:
        ...
        if m:
            ...
            return (
                default_dim_1(first_word, final_multisyllable_stress, modifier_final_multisyllable_stress) + " " + rest
            )
    m = re.search("^([^ ]+[eë]) (.*)$", lemma)
    if m:
        first_word, rest = m.groups()
        return (
            remove_final_e(first_word, modifier_final_multisyllable_stress)
            + " "
            + default_dim_1(rest, final_multisyllable_stress, modifier_final_multisyllable_stress)
        )
    ...
    if (
        re.search(V + "$", lemma)
        or re.search("[wy]$", lemma)
        or re.search(LONGV + "[rln]$", lemma)
        or re.search("rn$", lemma)
    ):
        return lemma + "tje"
