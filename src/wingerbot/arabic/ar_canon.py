...
def do_canon_param(
    ...
    if latin:
        try:
            canonarabic, canonlatin = ar_translit.tr_matching(arabic, latin, True, msgfun=pagemsg)
            match_canon = True
        except Exception as e:
            pagemsg("NOTE: Unable to vocalize %s (%s): %s: %s" % (arabic, latin, e, str(template)))
            canonlatin, canonarabic = ar_translit.canonicalize_latin_arabic(latin, arabic, msgfun=pagemsg)
    else:
        _, canonarabic = ar_translit.canonicalize_latin_arabic(None, arabic, msgfun=pagemsg)
    ...
    try:
        ...
        if not translit:
            pagemsg("NOTE: Unable to auto-translit %s (canoned from %s): %s" % (canonarabic, arabic, str(template)))
    except Exception as e:
        pagemsg("NOTE: Unable to transliterate %s (canoned from %s): %s: %s" % (canonarabic, arabic, e, str(template)))
        translit = None
    ...
    if canonarabic == arabic:
        ...
        if show_diff_string:
            ...
            diffmsg = ""
        pagemsg("%s Arabic %s -> %s%s%s: %s" % (operation, arabic, canonarabic, latintrtext, diffmsg, str(template)))
        if fromparam == toparam:
            actions.append("%s %s=%s -> %s" % (actionop, fromparam, arabic, canonarabic))
        else:
            actions.append("%s %s=%s -> %s=%s" % (actionop, fromparam, arabic, toparam, canonarabic))
        ...
        if rdarabic != rdcanonarabic:
            ...
            pagemsg(
                ...
                % (arabic, canonarabic, msgs and " (in old: %s)" % ", ".join(msgs) or "", diffmsg, str(template))
            )
    ...
    if not latin:
        ...
        pagemsg("Removing redundant translit for %s -> %s%s" % (arabic, newarabic, latintrtext))
        ...
        if canonlatin == latin:
            pagemsg("No change in Latin %s: Arabic %s -> %s (auto-translit %s)" % (latin, arabic, newarabic, translit))
            canonlatin = False
        else:
            pagemsg(
                "%s Latin %s -> %s: Arabic %s -> %s (auto-translit %s): %s"
                % (operation, latin, canonlatin, arabic, newarabic, translit, str(template))
            )
            actions.append("%s %s=%s -> %s" % (actionop, paramtrname, latin, canonlatin))
...
def canon_param(index, pagetitle, template, param, paramtr, include_tempname_in_changelog=False):
    ...
    arabic = pagetitle if fromparam == "page title" else getparam(template, fromparam)
...
def sort_group_changelogs(actions):
    grouped_actions = {}
    begins = ["split ", "vocalize ", "match-canon ", "cross-canon ", "self-canon ", "remove redundant ", "remove ", ""]
    for begin in begins:
