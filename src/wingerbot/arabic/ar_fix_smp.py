...
from wingerbot.arabic.arlib import (
  TAM, reorder_shadda, arabic_decl_templates,
)
...
def fix_smp(save, verbose, start, end):
  for template in arabic_decl_templates:
          ...
          while pl:
            if pl == "smp":
              if head.endswith(TAM):
                pagemsg("WARNING: Found %s=smp with feminine ending head %s in %s: not changing" % (
                  param, head, tn))
              else:
    ...
    for index, page in blib.references("Template:" + template, start, end):
      blib.do_edit(index, page, fix_one_page_smp, save=save,
          verbose=verbose)
