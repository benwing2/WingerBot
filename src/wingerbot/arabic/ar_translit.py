...
punctuation = (
    "؟،؛"  # Arabic semicolon, comma, question mark
    + "ـ"  # taṭwīl
...
before_diacritic_checking_subs = [
    ...
    ["([" + lconsonants + "]\u0651?)\u0627([" + rconsonants + "])", "\\1\u064e\u0627\\2"],
...
def tr(text, lang=None, sc=None, omit_i3raab=False, gray_i3raab=False, force_translate=False, msgfun=msg):
    ...
    if not omit_i3raab and gray_i3raab:  # show ʾiʿrāb grayed in transliteration
        ...
        text = rsub(text, "\u0629([\u064b\u064c\u064d])", '<span style="color: #888888">t</span>\\1')
        text = rsub(
            text,
            ".",
            {
    ...
    text = rsub(text, "([aiuāīū](?:</span>)?) a([" + sun_letters_tr + "]-)", "\\1 \\2")
...
hamza_match_chars = [x[0] if isinstance(x, list) or isinstance(x, tuple) else x for x in hamza_match]
...
tt_to_arabic_matching = {
    ...
    "ج": ["j", "ǧ", "ğ", "ǰ", "dj", "dǧ", "dğ", "dǰ", "dž", "dʒ", ["ʒ"], ["ž"], ["g"]],
    ...
    "ظ": ["ẓ", "ðʿ", "ðˤ", "ðˁ", "ðʕ", "ð̣", "đʿ", "đˤ", "đˁ", "đʕ", "đ̣", "ż", "ʐ", "dh", "z"],
    "ع": ["ʿ", "ʕ", "`", "‘", "ʻ", "3", "ˤ", "ˁ", ("'",), ("ʾ",), "῾", ("’",)],
    "غ": ["ḡ", "ġ", "ğ", "gh", ["g"], ("`",)],
    "ف": ["f", ["v"]],
...
latin_consonants_no_double_after_cons = (
    "bcdfgjklmnpqrstvwxyzʾʔḍḥḳḷṃṇṛṣṭṿẉỵẓḃċḋḟġḣṁṅṗṙṡṫẇẋẏżčǧȟǰňřšžḇḏẖḵḻṉṟṯẕḡs̄z̄çḑģḩķļņŗşţz̧ćǵḱĺḿńṕŕśẃźďľťƀđǥħłŧƶğḫʃɖʈt̤ð"
)
latin_consonants_no_double_after_cons_re = "[%s]" % (latin_consonants_no_double_after_cons)
...
def pre_canonicalize_latin(text, arabic=None, msgfun=msg):
    ...
    text = rsub(text, ".́", {"ā́": "ā", "ḗ": "ē", "ī́": "ī", "ṓ": "ō", "ū́": "ū"})
    ...
    text = rsub(text, ".", {"â": "ā", "ê": "ē", "î": "ī", "ô": "ō", "û": "ū"})
    ...
    # legitimately occur
    text = re.sub(r"([^aeiouəāēīōū\W])(%s)\2\b" % (latin_consonants_no_double_after_cons_re), r"\1\2", text, 0, re.U)
    # Remove double consonant preceding another consonant but special-case
    # a known example that shouldn't be touched.
    if text != "dunḡḡwān":
        text = re.sub(r"([^aeiouəāēīōū\W])\1(%s)" % (latin_consonants_no_double_after_cons_re), r"\1\2", text, 0, re.U)
    if arabic:
        ...
        if len(arabicwords) == len(latinwords):
            for i in range(len(latinwords)):
                ...
                if re.search("[اى]$", aword) and not re.search("\u064b[اى]$", aword):
    ...
    text = rsub(text, ".", {"ā": "aā", "ē": "eē", "ī": "iī", "ō": "oō", "ū": "uū"})
    return text
...
def pre_pre_canonicalize_arabic(text, msgfun=msg):
    if dont_pre_canonicalize_arabic(text):
        msgfun("Not pre-canonicalizing %s due to U+2008 or overly short" % text)
        return text
    ...
    if not re.search("[\ufdf0-\ufdff]", text) and not re.search(
        "(^|[\\W])[\ufb50-\ufdcf\ufe70-\ufeff]($|[\\W])", text, re.U
    ):
        text = nfkc_form(text)
    # remove L2R, R2L markers
    ...
    if text != newtext:
        if " " in newtext:
            ...
            msgfun("Not removing shadda after initial consonant in %s because of space in text" % text)
        else:
    ...
    newtext = rsub(text, "([" + lconsonants + "])" + A + "?" + ALIF + A, r"\1" + AA)
    if text != newtext:
...
def pre_canonicalize_arabic(text, safe=False, msgfun=msg):
    ...
    if not safe:
        ...
        text = rsub(text, "\u064b\u0649", "\u064b" + silent_alif_maqsuura_subst)
    ...
    if not safe:
        ...
        for part in re.split(r"(\[\[[^]]*\|)", text):
            if (index % 2) == 0:
                ...
                part = rsub(part, "([" + lconsonants + "])([" + rconsonants + "])", "\\1\u0652\\2")
                part = rsub(part, "([" + lconsonants + "])([" + rconsonants + "])", "\\1\u0652\\2")
...
def tr_matching(arabic, latin, err=False, msgfun=msg):
    ...
    latin = re.sub(r"(^|[aeiouəāēīōū\W])([^'.])\2", "\\1\\2\u0651", latin, 0, re.U)
    ...
    after_al_pos = []
    for m in re.finditer(r"((^|\s|\[\[|\|)" + ALIF + "|" + ALIF_WASLA + ")" + A + "?" + L + SK + "?", arabic):
        after_al_pos.append(m.end(0))

    def is_bow(pos=None):
        if pos is None:
            pos = aind[0]
        return pos == 0 or ar[pos - 1] in [" ", "[", "|"] or pos in after_al_pos
    ...
    def get_matches():
        ...
        if ac == assimilating_l_subst:
            ...
            matches = (
                bow
                and tt_to_arabic_matching_bow.get(ac)
                or eow
                and tt_to_arabic_matching_eow.get(ac)
                or tt_to_arabic_matching.get(ac)
            )
        ...
        for m in matches:
            ...
            if matched:
                ...
                if preserve_latin:
                    ...
                    if not is_eow():
                        lres.append("t")
                    elif aind[0] > 0 and (ar[aind[0] - 1] == "ا" or ar[aind[0] - 1] == "آ"):
                        lres.append("h")
                    # else do nothing
                else:
                    subst = matches[0]
    ...
    def cant_match():
        if aind[0] < alen and lind[0] < llen:
            error(
                "Unable to match Arabic character %s at index %s, Latin character %s at index %s"
    ...
    def check_skip_unmatching():
        ...
        if skip_chars != None and ar[aind[0]] in skip_chars:
            debprint(
                "Skip-unmatching matched %s at %s following %s at %s"
    ...
    def check_bow_alif():
        ...
        if not (lind[0] < llen - 1 and la[lind[0]] in hamza_match_chars and la[lind[0] + 1] in "aeiouəāēīōū"):
            return False
        ...
        if la[lind[0] + 1] in "ei":
            ...
            canonalif = "أ"
        msgfun("Canonicalized alif to %s in %s (%s)" % (canonalif, origarabic, origlatin))
    ...
    while aind[0] < alen or lind[0] < llen:
        ...
        if lind[0] < llen and la[lind[0]] == "\u0651":
            ...
            lind[0] += 1
            lres.append("\u0651")
            if aind[0] < alen and (ar[aind[0]] == "\u0651" or ar[aind[0]] == double_l_subst):
                res.append(ar[aind[0]])
                aind[0] += 1
            else:
                res.append("\u0651")
        ...
        elif not is_bow() and aind[0] < alen and ar[aind[0]] in word_interrupting_chars and check_unmatching():
...
def remove_diacritics(word):
    return arlib.remove_diacritics(word)


def foreign_diff_msgs(rdarabic, rdcanonarabic):
    msgs = []
    if "ی" in rdarabic:
