...
""" % (
    ...
    if not page.exists():
        ...
        if page.text == newtext:
            ...
            pagemsg("Replacing [[%s]] with [[%s]]" % (page.text, newtext), simple=True)
        else:
...
def create_noun_plural(save, index, inflection, infltr, lemma, lemmatr, template, pos):
    pagetitle = remove_diacritics(inflection)
    ...
    if template.name == "ar-noun-nisba":
        ...
        elif singgender == "m" and not lemma.endswith(TAM) and fem_pl_noun(lemma) == inflection:
            if personal:
...
def create_adj_plural(save, index, inflection, infltr, lemma, lemmatr, template, pos):
    create_inflection_entry(
        save,
        index,
        inflection,
...
def create_noun_feminine(save, index, inflection, infltr, lemma, lemmatr, template, pos):
    ...
    def pagemsg(text):
        msg(
            "Page %s %s: %s: feminine %s%s, masculine %s%s"
            % (
                index,
                remove_diacritics(inflection),
                text,
                inflection,
                infltr and " (%s)" % infltr,
                lemma,
                lemmatr and " (%s)" % lemmatr,
            )
        )
...
def create_adj_feminine(save, index, inflection, infltr, lemma, lemmatr, template, pos):
    create_inflection_entry(
        save,
        index,
        inflection,
...
def create_inflection_entries(save, pos, tempname, param, start, end, is_plural, createfn, inflectfn):
            ...
            for template in blib.parse_text(page.text).filter_templates():
                if template.name in tempname:
                    ...
                    if lemma == "":
                        lemma = page.title()
                        msg(
                            "Page %s %s(lemma): blank head in template %s (tr=%s)"
                    ...
                    infl, infltr = inflectfn(infl, infltr, lemma, lemmatr, template, param)
                    i = 2
                    while infl:
                        numinfls += 1
                        infl = getparam(template, param + str(i))
                        infltr = getparam(template, param + str(i) + "tr")
                        infl, infltr = inflectfn(infl, infltr, lemma, lemmatr, template, param + str(i))
                        i += 1

                    if len(heads) > 1 and numinfls > 1:
                        msg(
                            "Page %s %s(lemma): WARNING: More than one head and inflection: %s"
                    ...
                    for i in range(numinfls):
                        ...
                        headind = should_match_head(heads, i, originfl, originfltr, is_plural)
                        if headind is not None:
                            head, headtr = heads[headind]
                            infl, infltr = inflectfn(originfl, originfltr, head, headtr, template, param)
                            if infl:
                                if len(heads) > 1 and numinfls > 1:
                                    msg(
                                        "Page %s %s: Using head#%s %s%s as lemma for inflection %s%s"
                                        % (
                                            index,
                                            remove_diacritics(infl),
                                            headind + 1,
                                            head,
                                            headtr and " (tr=%s)" % headtr or "",
                                            infl,
                            ...
                            if len(heads) > 1 and numinfls > 1:
                                msg(
                                    "Page %s %s: Looping over all heads for inflection %s%s"
                                    % (
                                        index,
                                        remove_diacritics(infl),
                                        originfl or "(empty)",
...
def fem_inflection(infl, infltr, lemma, lemmatr, template, param):
    if template.name in ["ar-noun-nisba", "ar-nisba", "ar-adj-sound", "ar-adj-in", "ar-adj-an"]:
...
def create_plurals(save, pos, tempname, start, end):
    return create_inflection_entries(
        save,
        pos,
        tempname,
        "pl",
...
def create_feminines(save, pos, tempname, start, end):
    return create_inflection_entries(
        save,
        pos,
        tempname,
...
def create_verbal_nouns(save, start, end):
...
def create_participles(save, start, end):
    for index, page in blib.cat_articles("Arabic verbs", start, end):
        for template in blib.parse_text(page.text).filter_templates():
            if template.name == "ar-conj":
                ...
                if has_active_form(passive):
                    ...
                    if apvalue:
                        ...
                        for ap in aps:
                            create_participle(save, index, ap, page, template, "active", "act")
                if has_passive_form(passive, None):
                    ppvalue = get_part_prop(page, template, "ar-verb-part-all|pp")
                    if ppvalue:
                        pps = re.split(",", ppvalue)
                        for pp in pps:
                            create_participle(save, index, pp, page, template, "passive", "pass")
...
form_classes_to_number = dict(zip(all_form_classes, range(1, len(all_form_classes) + 1)))

# List of all person/number/gender combinations, using the ID's in
# {{ar-verb-part-all|...}}
all_persons = ["1s", "2sm", "2sf", "3sm", "3sf", "2d", "3dm", "3df", "1p", "2pm", "2pf", "3pm", "3pf"]
# Corresponding part of {{inflection of|...}} template, e.g. 3|m|s for 3sm
all_person_infls = [re.sub("([sdpmf])", r"|\1", re.sub("([sdp])([mf])", r"\2\1", x)) for x in all_persons]
...
def create_verb_part(save, index, page, template, dicforms, passive, voice, person, tense):
            ...
            if remove_diacritics(part) in distinct_dicformsnv:
                ...
                for dicform in dicforms:
                    create_inflection_entry(
                        ...
                        [("2", form)],
                        "inflection of",
                        "||lang=ar|%s|%s" % (infl_person, infl_tense),
                    )
...
def parse_part_spec(partspec):
    def check(variable, value, possible):
        if not value in possible:
            raise ValueError("Invalid value '%s' for %s, expected one of %s" % (value, variable, "/".join(possible)))
    ...
    for part in re.split(",", partspec):
        ...
        if len(partparts) == 2:
            ...
            if partparts[1] == "ps":
                ...
                raise ValueError(
                    "Expected PERSON-TENSE or PERSON-VOICE-TENSE where VOICE is 'ps' or 'all', but found '%s'"
                    % partparts[1]
                )
        else:
            raise ValueError("Expected PERSON-TENSE or PERSON-ps-TENSE or PERSON-all-TENSE, but found '%s'" % partspec)
        if person == "all":
            persons = all_persons
        else:
...
def create_verb_parts(save, start, end, partspec):
        ...
        for template in blib.parse_text(page.text).filter_templates():
            if template.name == "ar-conj":
                ...
                for voice, person, tense in parts_desired:
                    create_verb_part(save, index, page, template, dicforms, passive, voice, person, tense)
...
def parse_elative_defn(spec):
    ...
    if not m:
        ...
        for defn in defnstext.split(" / "):
            ...
            if m:
                ...
                for synonym in defn.split(", "):
                    ...
                    if m:
                        synonyms.append(
                            [add_bracketing(m.group(1)), add_bracketing(m.group(2)), add_bracketing(m.group(3))]
                        )
                        continue
                    m = re.match(r"(.*) \[(.*)\]$", synonym)
                    if m:
        ...
        if len(arpositives) > 1:
            etymlinepos += " and " + " and ".join(
                ["{{m|ar|%s||(same)}}" % arpositive for arpositive in arpositives[1:]]
            )
        ...
        etymline = "{{ar-elative}} of %s, from the %s." % (etymlinepos, etymlineroottext)

        # Create the "elative of" line
        elative_of_line = "# " + ", ".join(["{{elative of|%s|lang=ar}}" % pos for pos in arpositives]) + ":"
                ...
                if "|" in ref:
                    ...
                    if ref == "Steingass" or ref == "GT" or ref == "Almaany":
                        ...
                        if ref_root[-1] == ref_root[-2]:
                            ...
                            ref_root = ref_root[0:-1] + "ى"
                        elif ref_root[-1] == "ء":
                            ref_root = ref_root[0:-1] + "أ"
...
def create_elatives(save, elfile, start, end):
    elative_defns = []
    ...
    for current, index in blib.iter_pages(
        ...
        for arpositive in arpositives:
            ...
            blib.do_edit(index, page, add_elative_param, save=save)
...
parser.add_argument("--participle", action="store_true", help="Do participle inflections")
parser.add_argument(
    "--non-past",
    action="store_true",
...
parser.add_argument("--elative-list", help="File containing elative directives")
parser.add_argument("--elative", action="store_true", help="Do elatives")
parser.add_argument("--personal", action="store_true", help="Predict and store personal/non-personalness when possible")
...
if params.plural:
    create_plurals(params.save, "Noun", ["ar-noun", "ar-noun-nisba"], start, end)
    create_plurals(
        params.save, "Adjective", ["ar-adj", "ar-nisba", "ar-adj-sound", "ar-adj-in", "ar-adj-an"], start, end
    )

if params.feminine:
    # FIXME: Feminine noun creation not tested yet
    create_feminines(params.save, "Noun", ["ar-noun", "ar-noun-nisba"], start, end)
    create_feminines(
        params.save, "Adjective", ["ar-adj", "ar-nisba", "ar-adj-sound", "ar-adj-in", "ar-adj-an"], start, end
    )

if params.verbal_noun:
    create_verbal_nouns(params.save, start, end)
if params.participle:
    create_participles(params.save, start, end)
if params.verb_part:
    create_verb_parts(params.save, start, end, params.verb_part)
if params.non_past:
    create_verb_parts(params.save, start, end, "3sm-all-impf")
if params.elative:
    create_elatives(params.save, params.elative_list, start, end)
