...
from wingerbot.blib import getparam, addparam, msg, remove_links, tname
...
# page if args.save is true. INDEX is the numeric index of the lemma page, for
...
def create_adj_plural(index, inflection, infltr, lemma, lemmatr, template, pos):
    create_inflection_entry(
        index,
        inflection,
        infltr,
...
def create_noun_feminine(index, inflection, infltr, lemma, lemmatr, template, pos):
...
def create_adj_feminine(index, inflection, infltr, lemma, lemmatr, template, pos):
    create_inflection_entry(
        index,
        inflection,
        infltr,
...
# Create inflection entries for POS (the part of speech, which should be capitalized, e.g "Noun") by looking for
# inflections in the parameter PARAM (e.g. "pl") of templates named TEMPNAME (e.g. "ar-noun"). Entries created are
# limited by START and END (which are typically numbers but may be strings, and are the same as the START and END
# parameters passed on the command line), and only saved if args.save is true. CREATEFN is the actual function
# called to the creation, and is passed seven parameters; see 'create_noun_plural' for an example. INFLECTFN is used
# to create the actual inflected form given the form specified in the template; the default is to use the template
# form unmodified. This function is always called even if no inflected form is found, and hence can be used in cases
# where the inflection is auto-generated by the template (e.g. in 'ar-nisba').
def create_inflection_entries(pos, tempname, param, start, end, is_plural, createfn, inflectfn):
                    ...
                    for i in range(numinfls):
                        ...
                        if headind is not None:
                            ...
                            if infl:
                                ...
                                createfn(index, infl, infltr, head, headtr, template, pos)
                        else:
                            if len(heads) > 1 and numinfls > 1:
                                msg(
...
def create_plurals(pos, tempname, start, end):
...
def create_feminines(pos, tempname, start, end):
    return create_inflection_entries(
        pos,
        tempname,
        "f",
