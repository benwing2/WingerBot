...
from wingerbot.arabic.arlib import (
  ALIF, ALIF_WASLA, AMAD, AMAQ, A, AAH, AH, AAT, AATUN, AN, AYAAT, I, IN, IYAAT, IYAH,
  IYYAH, UN, U, UNU, UUN, UUNA, W, AWN, AWNA, TAM, HAMZA,
  remove_diacritics, reorder_shadda,
)
...
def create_inflection_entry(save, index, inflection, infltr, lemma, lemmatr,
  ...
  def pagemsg(text, simple=False, msgfun=msg):
    if simple:
      ...
      msgfun("Page %s %s: %s: %s %s%s, %s %s%s" % (index, pagetitle, text,
...
def create_noun_plural(save, index, inflection, infltr, lemma, lemmatr,
  ...
  def pagemsg(text, simple=False):
    if simple:
      ...
      msg("Page %s %s: %s: plural %s%s, singular %s%s" % (index, pagetitle, text,
...
def create_inflection_entries(save, pos, tempname, param, start, end,
    is_plural, createfn, inflectfn):
...
def create_plurals(save, pos, tempname, start, end):
  return create_inflection_entries(save, pos, tempname, "pl", start, end,
      True, create_noun_plural if pos == "Noun" else create_adj_plural,
      pl_inflection)

def create_feminines(save, pos, tempname, start, end):
  return create_inflection_entries(save, pos, tempname, "f", start, end,
...
def create_elatives(save, elfile, start, end):
  ...
  for current, index in blib.iter_pages(elative_defns, start, end,
      # key is the elative
      key = lambda x: x[1]):
...
parser.add_argument("-p", "--plural", action='store_true',
    help="Do plural inflections")
parser.add_argument("-f", "--feminine", action='store_true',
    help="Do feminine inflections")
parser.add_argument("--verbal-noun", action='store_true',
    help="Do verbal noun inflections")
parser.add_argument("--participle", action='store_true',
    help="Do participle inflections")
parser.add_argument("--non-past", action='store_true',
    help="""Do non-past dictionary-form inflections; equivalent to
'--verb-part 3sm-all-impf'.""")
parser.add_argument("--verb-part",
...
parser.add_argument("--elative-list",
    help="File containing elative directives")
parser.add_argument("--elative", action='store_true',
    help="Do elatives")
