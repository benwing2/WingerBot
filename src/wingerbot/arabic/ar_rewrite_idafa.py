...
def process_text_on_page(index, pagetitle, text):
    ...
    for t in parsed.filter_templates():
        if t.name.startswith("ar-decl-"):
            ...
            if (
                getparam(t, "basestate")
                or getparam(t, "modcase")
                or getparam(t, "modstate")
    ...
    actions = []
    if idafa_added:
        actions.append("Replaced ʾidāfa params with idafa= param: %s" % (", ".join(idafa_added)))
    if num_new_style:
        actions.append("Corrected %s old-style ʾidāfa param(s) to new-style" % num_new_style)
