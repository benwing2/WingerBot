...
class Number(object):
    ...
    def __init__(
        ...
        if ord:
            if len(ord) == 6:
                self.ordroot, self.ordeng, self.ordgloss, self.cardteeneng, self.ordlemma, self.cardteen = ord
            else:
                self.ordroot, self.ordeng, self.cardteeneng = ord
                self.ordgloss = self.ordeng
                self.ordlemma = self.ordroot[0] + AA + self.ordroot[1] + I + self.ordroot[2]
            ...
            self.ordteeneng = (
                "twelfth"
                if self.cardteeneng == "twelve"
                else "twentieth" if self.cardteeneng == "twenty" else self.cardteeneng + "th"
            )
...
digits = {
    1: Number(
        ...
        hundred="مِائَة",
        thousand="أَلْف",
        ord=["حدي", "first", "first|first (combining form)", "eleven", "حَادٍ", "أَحَدَ عََشَرَ"],
        adv="مَرَّة",
        mult="مُفْرَد",
        dist=["أُحَاد", "وُحَاد", "مَوْحَد"],
        numadj="أُحَادِيّ",
    ),
    2: Number(
        "٢",
    ...
    3: Number("٣", "three", "ثَلَاثَة", ord=["ثلث", "third", "thirteen"], dist=["ثُلَاث", "مَثْلَث"]),
    4: Number("٤", "four", "أَرْبَعَة", ord=["ربع", "fourth", "fourteen"], dist=["رُبَاع", "مَرْبَع"]),
    5: Number("٥", "five", "خَمْسَة", ord=["خمس", "fifth", "fifteen"]),
    6: Number("٦", "six", "سِتَّة", ord=["سدس", "sixth", "sixteen"]),
    7: Number("٧", "seven", "سَبْعَة", ord=["سبع", "seventh", "seventeen"]),
    8: Number("٨", "eight", "ثَمَانِيَة", "ثَمَانٍ", hundred="ثَمَانِمِائَة", ord=["ثمن", "eighth", "eighteen"]),
    9: Number("٩", "nine", "تِسْعَة", ord=["تسع", "ninth", "nineteen"]),
    10: Number("١٠", "ten", "عَشَرَة", "عَشْر", hundred="أَلْف", ord=["عشر", "tenth", "twenty"]),
}
...
#: Eastern Arabic numeral: {{l|ar|%s%s}}""" % (
    ...
    decl = (
        """{{ar-decl-numeral|-|pl=%s%s|f=-|fpl=%s%s|mod=-|modpl=%s|modf=-|modfpl=%s|modprefix=وَ/wa-|state=ind,def}}"""
        % (
...
""" % (
    ...
    changelog = "Create lemma entry for %s (Arabic numeral '%s-%s')" % (pagename, ten.english, dig.english)
...
def create_non_lemma(tenval, ten, digval, dig, obl=False, fem=False):
    ...
    pagename = "%s وَ%s" % (arabic(dig), arabic(ten))
    headword = """{{head|ar|numeral form|head=%s|%s|tr=%s wa-%s}}""" % (pagename, "f" if fem else "m", tr(dig), tr(ten))
    defns = []
    for case in ["informal", "acc", "gen"] if obl else ["nom"]:
        defns.append(
            """# {{inflection of|lang=ar|%s وَ%s|tr=%s wa-%s||%s|%s|gloss=[[%s-%s]]}}"""
            % (dig.nom, ten.nom, dig.nomtr, ten.nomtr, "f" if fem else "m", case, ten.english, dig.english)
        )
    defn = "\n".join(defns)
    text = """\
==Arabic==
...
""" % (
    ...
    changelog = "Create non-lemma entry (%s) for %s وََ%s (Arabic numeral '%s-%s')" % (
        fem and obl and "feminine oblique" or fem and "feminine nominative" or "masculine oblique",
        dig.nom,
        ten.nom,
        ten.english,
        dig.english,
...
def create_ordinal_lemma(tenval, ten, digval, dig):
    ...
    defn = """# {{context|ordinal|lang=ar}} {{l|en|%s-%s}}""" % (ten.english, dig.ordeng)
...
""" % (
    ...
    changelog = "Create lemma entry for %s (Arabic ordinal numeral '%s-%s')" % (pagename, ten.english, dig.ordeng)
    return pagename, text, changelog
...
def create_unit_ordinal_lemma(digval, dig):
    ...
    etym = """From the root {{ar-root|%s}}; compare {{m|ar|%s||%s}}.""" % (
        " ".join(dig.ordroot),
        "أَحَد" if digval == 1 else dig.nom,
        dig.english,
    )
...
""" % (
    ...
    changelog = "Create lemma entry for %s (Arabic ordinal numeral '%s')" % (pagename, dig.ordeng)
    return pagename, text, changelog
...
def create_unit_ordinal_non_lemma(digval, dig):
    ...
    defn = """# {{inflection of|lang=ar|%s||f|gloss=%s}}""" % (dig.ordlemma, dig.ordeng)
    text = """\
==Arabic==
...
""" % (
    ...
    changelog = "Create non-lemma entry (feminine) for %s (Arabic ordinal numeral '%s')" % (dig.ordlemma, dig.ordeng)
    return pagename, text, changelog
...
def create_teen_ordinal_lemma(digval, dig):
    ...
    etym = (
        """{{compound|lang=ar|%s|t1=%s|عَشَرَ|t2=-ten}}, with both parts in the accusative case as with the cardinal numeral {{m|ar|%s|%s}}, and the same form for the tens part as with the cardinal. Units part from the root {{ar-root|%s}}."""
        % (dig.ordlemma, dig.ordeng, dig.cardteen, dig.cardteeneng, " ".join(dig.ordroot))
    )
...
""" % (
    ...
    changelog = "Create lemma entry for %s (Arabic ordinal numeral '%s')" % (pagename, dig.ordteeneng)
    return pagename, text, changelog
...
def create_teen_ordinal_non_lemma(digval, dig):
    ...
    headword = """{{ar-adj-fem|%s}}}""" % pagename
    defn = """# {{inflection of|lang=ar|%s||f|gloss=%s}}""" % (dig.ordteen, dig.ordteeneng)
    text = """\
==Arabic==
...
""" % (
    ...
    changelog = "Create non-lemma entry (feminine) for %s (Arabic ordinal numeral '%s')" % (dig.ordteen, dig.ordteeneng)
    return pagename, text, changelog
...
export.numbers[%s] = {
	...
	cardinal = "%s",
	ordinal = "%s","""
            % (digval, dig.eastarabnum, dig.nom, dig.ordlemma)
        )
        if dig.adv:
        ...
        if dig.dist:
            msg("""	distributive = {%s},""" % ", ".join(['"' + x + '"' for x in dig.dist]))
        if dig.numadj:
            msg(
                """	other_title = "Numeral adjective",
	other = "%s","""
                % dig.numadj
            )
        msg("""}""")
...
export.numbers[%s] = {
	...
	ordinal = {{"%s", "%s"}},
                    ...
                    % (
                        tenval + digval,
                        ten.eastarabnum[0] + dig.eastarabnum,
                        dig.nom + " وَ" + ten.nom,
                        dig.nomtr + " wa-" + ten.nomtr,
                        dig.ordlemma + " وَ" + ten.nom,
                        dig.ordlemmatr + " wa-" + ten.nomtr,
                    )
                )
    # Do 100, 200, 300, ..., 900
    for digval, dig in sorted(digits.items(), key=lambda x: x[0]):
...
parser.add_argument("--ordinal-non-lemmas", action="store_true", help="Do ordinal non-lemmas from 11-19.")
