            ...
            for t in parsed.filter_templates():
                ...
                if tn == "ar-conj":
                    ...
                    if arg_vform in conjs_by_form:
                        pagemsg(
                            "WARNING: Saw multiple form-%s conjugations for lemma %s, may need to manually annotate with definitions: %s"
...
def process_text_on_page(index, pagetitle, text):
    ...
    def do_etymtext(
        ...
        notes.append(
            "replace %s for verb form %s, unvocalized lemma%s %s, %s with %s"
            % (
                ar_verb_form_occurrences_msg,
                verb_form,
                "s" if len(unvocalized_lemmas) > 1 else "",
                ",".join(unvocalized_lemmas),
                voclemma_msg,
                ar_verb_form_call,
            )
        )
    ...
    for k in range(2, len(etym_sections), 2):
        ...
        for t in parsed.filter_templates():
            ...
            if tn == "ar-verb-form":
                ...
                unvoc_lemma = arlib.remove_diacritics(lemma)
                        ...
                        for lemma1, lemma2, lemma1_is_current in [
                            ...
                            if m:
                                ...
                                if m2:
                                    ...
                                    person_number_suffix = "(?:%s|%s)$" % (
                                        person_number_suffix_non_past,
                                        person_number_suffix_past,
                                    )
                ...
                pagemsg(
                    ...
                    % (
                        verb_form,
                        ",".join(vocalized_lemmas),
                        no_multi_etym_msg and "; %s" % no_multi_etym_msg or "",
                        str(t),
                    )
                )
                must_continue = True
