...
def process_text_on_page(p):
    ...
    def remove_arabic_section():
        ...
        if not has_non_arabic:
            ...
            notes.append(
                "excised %s subsection%s for Arabic forms wrongly lacking final aleph, leaving nothing"
            ...
            del sections[j - 1]
            notes.append(
                "excised %s subsection%s for Arabic forms wrongly lacking final aleph, leaving no Arabic section"
                % (len(movable_sections), "" if len(movable_sections) == 1 else "s")
            )
            if j > len(sections):
    ...
    if "==Etymology 1==" in secbody:
        ...
        if len(etym_sections) > 3:
            ...
            text = "".join(sections)
            notes.append(
                "excised %s subsection%s for Arabic forms wrongly lacking final aleph"
                % (len(movable_sections), "" if len(movable_sections) == 1 else "s")
            )
            ...
            text = "".join(sections)
            notes.append(
                "excised %s subsection%s for Arabic forms wrongly lacking final aleph, leaving one non-etym-split section"
                % (len(movable_sections), "" if len(movable_sections) == 1 else "s")
            )
        else:
