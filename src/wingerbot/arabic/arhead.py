...
# FIXME: Extremely old and outdated.
    ...
    for t in parsed.filter_templates():
        ...
        if tn in arlib.arabic_all_headword_templates:
            if (
                ...
                t.remove("head")
                addparam(t, "head", head, before=str(t.params[0].name) if len(t.params) > 0 else None)
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, new=True, default_refs=["Template:tracking/ar-head/head"],
)
