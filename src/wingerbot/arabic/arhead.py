...
def process_text_on_page(index, pagetitle, text):
  ...
  for t in parsed.filter_templates():
    ...
    if tn in arlib.arabic_all_headword_templates:
      if t.has("head") and not t.has(1) and not t.has(2) and not t.has(3) and not t.has(4) and not t.has(5) and not t.has(6) and not t.has(7) and not t.has(8):
...
blib.do_pagefile_cats_refs(args, start, end, process_text_on_page,
  default_refs=["Template:tracking/ar-head/head"], edit=True, stdin=True)
