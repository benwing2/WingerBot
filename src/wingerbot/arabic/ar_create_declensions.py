...
def create_declension(index, page, save, pos, tempname, decltempname, sgnum, removeparams, is_proper=False):
    pagename = page.title()
    comments = []
    ...
    def remove_al(text):
        return (
            sub_if("^" + ALIF_ANY + A + "?" + L + SK + "?(.)" + SH, r"\1", text)
    ...
    def remove_al_tr(text):
        return sub_if(r"^a?([sšṣtṯṭdḏḍzžẓnrḷ])-\1", r"\1", text) or sub_if("^a?l-", "", text) or text
            ...
            for j in range(len(subsections)):
                ...
                if j > 0 and (j % 2) == 0 and re.match("^===+%s===+\n" % pos, subsections[j - 1]):
                    ...
                    if (
                        ...
                        or j + 3 < len(subsections)
                        and re.match("^===+Usage", subsections[j + 1])
                        and re.match("^===+Declension===+\n", subsections[j + 3])
                    ):
                    ...
                    def remove_i3rab(param):
                        text = str(param)
                        if text.endswith(UN):
                            pgmsg("Removing i3rab from %s: %s" % (text, str(headword_template)))
                    ...
                    def process_param(param):
                        ...
                        if arabic.endswith("=+"):
                            ...
                            pgmsg("Converting %s to %s: %s" % (arabic, newarabic, str(headword_template)))
                            arabic = newarabic
...
def create_declensions(save, pos, tempname, decltempname, sgnum, start, end, removeparams, is_proper=False):
    for index, page in blib.references("Template:%s" % tempname, start, end):
        create_declension(index, page, save, pos, tempname, decltempname, sgnum, removeparams, is_proper=is_proper)
...
if not params.proper:
    create_declensions(params.save, "Noun", "ar-noun", "ar-decl-noun", "sg", start, end, non_gendered_params_to_remove)
    create_declensions(
        params.save, "Noun", "ar-coll-noun", "ar-decl-coll-noun", "coll", start, end, non_gendered_params_to_remove
    )
    create_declensions(
        params.save, "Noun", "ar-sing-noun", "ar-decl-sing-noun", "sing", start, end, non_gendered_params_to_remove
    )
    create_declensions(
        params.save, "Noun", "ar-noun-nisba", "ar-decl-gendered-noun", "sg", start, end, params_to_remove
    )
    for adj_template in ["ar-adj", "ar-nisba", "ar-adj-sound", "ar-adj-in", "ar-adj-an"]:
        create_declensions(params.save, "Adjective", adj_template, "ar-decl-adj", "sg", start, end, params_to_remove)
create_declensions(
    params.save,
    "Proper noun",
    "ar-proper noun",
