...
preferred_tag_variants = {"pres", "past", "fut", "act", "pass", "part"}

tag_to_dimension_table, tag_to_canonical_form_table = infltags.fetch_tag_tables(preferred_tag_variants)


from wingerbot.misc_templates_to_rewrite import misc_templates_to_rewrite
...
templates_by_cap_and_period_map = {template: spec for template, spec, verified in templates_by_cap_and_period}
...
de_specs = [
    ...
    (
        ...
        (
            ...
            ("error-if", ("present-except", ["1"])),
            ("set", "1", ["de", lambda data: "[[%s]] [[du]]" % data.getp("1")]),
        ),
    ),
    ...
    (
        ...
        (
            ...
            ("error-if", ("present-except", ["deg", "1", "2", "3", "4", "nocat", "sort", "lang"])),
            (
                "set",
                "1",
                [
        ...
        if data.getp("c") in ["nom", "n"] and data.getp("g") == "m" and data.getp("n") == "s":
            ...
            if not m:
                raise BadTemplateValue(
                    "Unable to infer comparative/superlative stem from non-lemma form %s" % data.pagetitle
                )
...
enm_specs = [
    ...
    # template.
    ("enm-first-person singular of", enm_verb_form(["1s", "pres", "ind"])),
    ("enm-first/third-person singular past of", enm_verb_form(["13", "s", "past", "ind"])),
    ("enm-plural of", enm_verb_form(["p", "pres", "ind"])),
    ("enm-plural past of", enm_verb_form(["p", "past", "ind"])),
    ("enm-plural subjunctive of", enm_verb_form(["p", "pres", "sub"])),
    ("enm-plural subjunctive past of", enm_verb_form(["p", "past", "sub"])),
    ("enm-second-person singular of", enm_verb_form(["2s", "pres", "ind"])),
    ("enm-second-person singular past of", enm_verb_form(["2s", "past", "ind"])),
    ("enm-singular subjunctive of", enm_verb_form(["s", "pres", "sub"])),
    ("enm-singular subjunctive past of", enm_verb_form(["s", "past", "sub"])),
    ("enm-third-person singular of", enm_verb_form(["3s", "pres", "ind"])),
]
...
fi_specs = [
    ...
    (
        ...
        (
            ...
            (
                "error-if",
                ("present-except", ["1", "case", "plural", "pl", "t", "suffix", "gloss", "nocat", "nodot", "lang"]),
            ),
            (
                "set",
                "1",
...
got_specs = [
    ...
    (
        ...
        (
            "verb form of",
            # lang= occurs at least once, and is ignored.
            ("error-if", ("present-except", ["1", "p", "n", "t", "tr", "v", "m", "nocap", "nodot", "lang"])),
            (
                "set",
                "1",
                [
                ...
                [
                    ...
                    (
                        ...
                        "1",
                        {
                            ...
                            "plural and definite singular past participle": [
                                "p",
                                "past",
                                "part",
                                ";",
                            ...
                            "past participle definite singular and plural": [
                                "p",
                                "past",
                                "part",
                                ";",
    ...
    (
        ...
        (
            ...
            (
                ...
                [
                    "pt",
                    lambda data: data.getp("1") + ("o" if data.getp("2") in ["a", "os", "as"] else "º"),
                    "",
                    (
                        "lookup",
                        "2",
...
ru_specs = [
    (
        ...
        (
            "participle of",
            # lang= occurs at least once, and is ignored.
            ("error-if", ("present-except", ["1", "2", "3", "4", "5", "6", "tr", "gloss", "pos", "nocat", "lang"])),
            (
                "set",
                "1",
                [
...
def sl_check_1_is_m(data, should_return):
    if data.getp("1") == "m":
        ...
        raise BadTemplateValue("Expected 1=m with output of %s" % "|".join(should_return))
...
sv_specs = [
    ...
    (
        "sv-verb-form-imp",
        lambda data: (
            sv_verb_form_with_plural(["1", "p", "imp"])
            if data.getp("plural of") and data.pagetitle.endswith("m")
            else sv_verb_form_with_plural(["2", "p", "imp"]) if data.getp("plural of") else sv_verb_form(["imp"])
        ),
    ),
    ...
    (
        "sv-verb-form-past",
        lambda data: (
            sv_verb_form_with_plural(["1", "p", "past", "ind"])
            if data.getp("plural of") and data.pagetitle.endswith("m")
            else (
                sv_verb_form_with_plural(["2", "p", "past", "ind"])
                if data.getp("plural of") and data.pagetitle.endswith("n")
                else (
                    sv_verb_form_with_plural(["p", "past", "ind"])
                    if data.getp("plural of")
    ...
    (
        ...
        lambda data: (
            sv_verb_form_with_plural(["p", "past", "pass", "ind"])
            if data.getp("plural of")
    ...
    (
        "sv-verb-form-pre",
        lambda data: (
            sv_verb_form_with_plural(["1", "p", "pres", "ind"])
            if data.getp("plural of") and data.pagetitle.endswith("m")
            else (
                sv_verb_form_with_plural(["2", "p", "pres", "ind"])
                if data.getp("plural of") and data.pagetitle.endswith("n")
                else (
                    sv_verb_form_with_plural(["p", "pres", "ind"])
                    if data.getp("plural of")
    ...
    return tuple(
        [
            ...
            (
                ...
                (
                    "present-except",
                    ["1", "2", "3", "4", "sc", "tr", "g", "t", "gloss", "pos", "cap", "nodot", "nocap", "nocat"],
                ),
            ),
            (
                "set",
...
def rewrite_to_foo_form_of(data, comment):
    ...
    if str(t) != origt:
        data.pagemsg("rewrite_to_foo_form_of: Replaced %s with %s" % (origt, str(t)))
...
def rewrite_to_pres_past_participle_of(data, comment):
    ...
    if str(t) != origt:
        data.pagemsg("rewrite_to_pres_past_participle_of: Replaced %s with %s" % (origt, str(t)))
...
def rewrite_to_general_participle_of(data, comment):
    ...
    if str(t) != origt:
        data.pagemsg("rewrite_to_general_participle_of: Replaced %s with %s" % (origt, str(t)))
...
def rewrite_person_number_of(data, comment):
    ...
    origt = str(t)
    tn = tname(t)
    if tn in ["inflection of", "infl of", "verb form of", "noun form of", "adj form of", "participle of"]:
    ...
    if str(t) != origt:
        data.pagemsg("rewrite_person_number_of: Replaced %s with %s" % (origt, str(t)))
...
def expand_set_value(value, data):
    ...
    def check(cond, err):
        if not cond:
            raise BadRewriteSpec("Error expanding set value for template %s: %s; value=%s" % (str(t), err, value))
    ...
    check(isinstance(value, tuple), "wrong type %s of %s, not tuple" % (type(value), value))
    check(len(value) >= 1, "empty value")
    direc = value[0]
    if direc == "copy":
        check(len(value) == 2, "wrong length %s of value %s, != 2" % (len(value), value))
        if t.has(value[1]):
            return getparam(t, value[1])
        else:
            return None
    elif direc == "lookup":
        check(len(value) == 3, "wrong length %s of value %s, != 3" % (len(value), value))
...
def expand_spec(spec, data):
    ...
    def check(cond, err):
        if not cond:
            raise BadRewriteSpec("Error expanding spec for template %s: %s; spec=%s" % (str(t), err, spec))
    ...
    for subspec in spec[1:]:
        ...
        if subspec[0] == "error-if":
            check(len(subspec) == 2, "wrong length %s of subspec %s, != 2" % (len(subspec), subspec))
            check(len(subspec[1]) >= 1, "empty subspec[1]")
            errtype = subspec[1][0]
            if errtype == "present-except":
                check(len(subspec[1]) == 2, "wrong length %s of subspec[1] %s, != 2" % (len(subspec[1]), subspec[1]))
                allowed_params = set(subspec[1][1])
                for param in t.params:
                    pname = str(param.name).strip()
                    if pname not in allowed_params:
                        raise BadTemplateValue("Disallowed param %s=%s" % (pname, getparam(t, pname)))
            elif errtype == "eq":
                check(len(subspec[1]) == 3, "wrong length %s of subspec[1] %s, != 3" % (len(subspec[1]), subspec[1]))
                if getparam(t, subspec[1][1]) == subspec[1][2]:
                    raise BadTemplateValue("Disallowed value: %s=%s" % (subspec[1][1], subspec[1][2]))
            elif errtype == "neq":
                check(len(subspec[1]) == 3, "wrong length %s of subspec[1] %s, != 3" % (len(subspec[1]), subspec[1]))
                if getparam(t, subspec[1][1]) != subspec[1][2]:
                    raise BadTemplateValue(
            ...
            check(len(subspec) == 3, "wrong length %s of subspec %s, != 3" % (len(subspec), subspec))
            _, param, newval = subspec
            check(isinstance(param, str), "wrong type %s of %s, not str" % (type(param), param))
            ...
            if newval is None:
                ...
                if re.search("^[0-9]+$", param):
                    ...
                    for val in newval:
                        if val is not None:
                            expanded_specs.append((param if index == 1 else "param%s" % index, val))
                        index += 1

        elif subspec[0] == "copy":
            check(len(subspec) in [2, 3], "wrong length %s of subspec %s, not in [2, 3]" % (len(subspec), subspec))
            fromparam = subspec[1]
            if len(subspec) == 2:
                toparam = fromparam
            else:
            ...
            for param in t.params:
                ...
                if pbase == frombase and (not pind and fromfirstblank or pind and int(pind) >= fromind):
            ...
            for param in t.params:
                ...
                for ename in exclude_params:
                    if isinstance(ename, str):
                        ...
                        check(len(ename) == 2, "wrong length %s of ename %s, != 2" % (len(ename), ename))
                        check(ename[0] == "list", 'ename[0] should == "list" but == %s' % ename[0])
                        check(isinstance(ename[1], str), "wrong type %s of %s, not str" % (type(ename[1]), ename[1]))
                        ...
                        if not eind:
                            ...
                            if pind and int(pind) >= int(eind):
                                ...
                                break
                if not excludeme:
                    expanded_specs.append((str(param.name), str(param.value), str(param.showkey)))

        elif subspec[0] == "comment":
            check(len(subspec) == 2, "wrong length %s of subspec %s, != 2" % (len(subspec), subspec))
            _, comment = subspec
            comment = expand_set_value(comment, data)
            check(isinstance(comment, str), "wrong type %s of %s, not str" % (type(comment), comment))
    ...
    if not comment:
        ...
        for spec in expanded_specs:
            if spec[0] == "1" and oldname.startswith(spec[1] + "-"):
                comment = "rename {{%s}} to {{%s|%s}} with appropriate param changes" % (oldname, newname, spec[1])
                break

    if not comment:
        comment = "rename {{%s}} to {{%s}} with appropriate param changes" % (oldname, newname)
    ...
    if args.lang_for_combine_inflection_of:
        ...
        for t in parsed.filter_templates():
            ...
            if tn in infltags.inflection_of_templates:
                origt = str(t)
                tags, params, lang, term, tr, alt = infltags.extract_tags_and_nontag_params_from_inflection_of(t, notes)
...
parser.add_argument(
    "--check-ignores", help="Check whether there may be problems ignoring intial cap or final dot", action="store_true"
)
parser.add_argument(
    "--lang-for-combine-inflection-of", help="Language name of section whose {{inflection of}} calls will be combined"
)
...
if args.check_ignores:
    ...
    for template in templates_to_actually_do:
        if template not in templates_by_cap_and_period_map:
            errandmsg(
                "WARNING: Template:%s not in templates_by_cap_and_period_map, not sure its ignoring behavior, skipping"
                % template
            )
            ...
            if ignore_type not in ["lcnodot", "ucdot"] or (ignore_type == "ucdot" and args.check_ignores_include_ucdot):
                errandmsg("Processing references to Template:%s [ignore_type=%s]" % (template, ignore_type))
