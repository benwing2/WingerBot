...
from wingerbot.blib import set_template_name
    ...
    for lineno, line in enumerate(lines, start=1):
        ...
        if re.search(r"^labels *\[", line):
            ...
            if alias in labels_seen[indexed_labels[canon]].aliases:
                linemsg(
                    "WARNING: Not appending alias '%s' already seen in alias(es) '%s' for canonical label '%s': %s"
            ...
            m = re.search(
                r"^\[%s\] *= *\{ *\[%s\] *= *%s *\},?$"
                % (either_quote_string_re, either_quote_string_re, either_quote_string_re),
                line.strip(),
            )
            if m:
    ...
    for labelobj in labels_seen:
        if isinstance(labelobj, LabelData):
            ...
            if display != labelobj.label:
                new_lines.append(
                    '\tdisplay = "%s",%s%s' % (display, labelobj.display_comment or "", labelobj.appendix_comment or "")
                )
