...
def init_fake_langdata():
  ...
  languages_by_code = {
    y["code"]: {"canonicalName": x} for x, y in languages_by_canonical_name.items()
  }
  etym_languages_by_canonical_name = {}
  etym_languages_by_code = {
    y["code"]: {"canonicalName": x} for x, y in etym_languages_by_canonical_name.items()
  }
...
zh_combining_accent_re = (
  "[" + GRAVE + ACUTE + CFLEX + MACRON + BREVE + DOTABOVE + DIAER + DOUBLEACUTE + CARON + VERTLINEABOVE
...
def sh_remove_accents(text):
  return unicodedata.normalize("NFC", unicodedata.normalize("NFD", text).
    replace(ACUTE, "").replace(GRAVE, "").
    replace(DOUBLEGRAVE, "").replace(INVBREVE, "").
...
def sl_remove_accents(text):
  return unicodedata.normalize("NFC", unicodedata.normalize("NFD", text).
    replace(ACUTE, "").replace(GRAVE, "").
    replace(MACRON, "").replace(CFLEX, "").
    replace(DOUBLEGRAVE, "").replace(INVBREVE, "").
    replace(DOTUNDER, "").replace("ə", "e").replace("ł", "l")
  )
...
def ie_remove_accents(text):
  return unicodedata.normalize("NFC", unicodedata.normalize("NFD", text).
    replace(ACUTE, "").replace(GRAVE, "").replace(CFLEX, "")
  )
