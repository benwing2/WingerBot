...
def process_text_on_page(index, pagetitle, text):
        ...
        if thislangname in thislangnames:
            ...
            for k, subsectitle in subsecs.subsection_headers:
                if not (
                    ...
                    and subsectitle == "Usage notes"
                    or (args.do_see_also or this_ignore_translit != "latin")
                    and subsectitle == "See also"
                ):
                    continue
    ...
    if subbed_links:
        ...
        for langname, lang_subbed_links in sorted(seen_langs.items()):
            subbed_links_notes.append(
                "replace raw %s links with templated links: %s" % (langname, ",".join(lang_subbed_links))
            )
...
if __name__ == "__main__":
    ...
    for lang in langs:
        ...
        thislangname, this_remove_accents, this_charset, this_ignore_translit = lang_utils.language_codes_to_properties[
            lang
        ]
