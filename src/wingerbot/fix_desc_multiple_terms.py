...
def process_text_on_page(p):
    ...
    def replace_serbo_croatian_with_desc(m):
        ...
        def replace_sh_term(m):
            ...
            if termlink.startswith("[["):
                termlink = re.sub(
                    r"^\[\[(.*?)\]\]$",
                    lambda m: sub_link(m, "Serbo-Croatian", "sh", sh_remove_accents, origtext, add_sclb=True),
                    termlink,
                )
            else:
    ...
    def replace_with_desc(m):
        ...
        if link_langcode in language_codes_to_properties:
            _, link_langcode_remove_accents, _, _ = language_codes_to_properties[link_langcode]
        ...
        def replace_raw_link(m):
            ...
            if linktext.startswith("["):
                ...
                if non_etym_langcode in lang_data.languages_by_code:
                    ...
                    langname = "UNKNOWN"
                retval = sub_link(mm, langname, link_langcode, link_langcode_remove_accents, origtext, add_sclb=False)
                if retval != linktext:
        ...
        notes.append(
            "incorporate %s {{l}} link%s into {{desc|%s}}"
