...
def check_for_redundant_translit(yi, tr, pagemsg, expand_text):
    ...
    if compare_normalized(autotr, tr):
        ...
        return yi, ""
    pagemsg("Keeping non-redundant translit %s != auto %s for Yiddish %s" % (tr, autotr, yi))
    return yi, tr
...
def lemma_matches(lemma, yi, tr, pagemsg, expand_text):
    ...
    if yi == lemru:
        ...
        if not trmatches:
            pagemsg(
                "WARNING: Value %s matches lemma %s of yi-(proper )noun+, but translit %s doesn't match %s"
                % (yi, lemru, tr, lemtr)
            )
        else:
            return True
...
def create_inflection_entry(
    ...
    for skip_form, skip_lemma in skip_form_pages:
        if skip_form == pagename and skip_lemma == lemma:
            pagemsg("WARNING: Skipping form because in skip_form_pages for lemma %s" % skip_lemma)
            return
    ...
    def compare_param(t, param, value, valuetr, issue_warnings=True):
        ...
        if matches:
            ...
            pagemsg_if(
                issue_warnings,
                "WARNING: Value %s matches param %s=%s, but translit %s doesn't match param %s=%s: %s"
    ...
    def template_head_matches(t, inflections, purpose, fail_when_left_over_heads=False, issue_warnings=True):
        ...
        if some_match and not all_match:
            pagemsg_if(
                issue_warnings,
                "WARNING: Some but not all inflections %s match template when %s: %s"
            ...
            if fail_when_left_over_heads:
                pagemsg_if(
                    issue_warnings,
                    "WARNING: All inflections %s match template, but extra heads in template when %s, treating as a non-match: %s"
                    % (joined_infls_with_tr(), purpose, str(t)),
                )
                return False
            else:
                pagemsg_if(
                    issue_warnings,
                    "WARNING: All inflections %s match template, but extra heads in template when %s: %s"
                    % (joined_infls_with_tr(), purpose, str(t)),
                )
        return all_match
    ...
    if entrytext:
        ...
        new_headword_template = "{{%s%s%s}}" % (headtemp, "".join(headparams), headtemp_param)

        # 4. Synthesize definition template.
        new_defn_template = "{{%s%s|%s%s%s}}" % (
            deftemp,
            "|yi" if deftemp_needs_lang else "",
            lemma,
...
""" % (
    ...
    if not blib.safe_page_exists(page, errandpagemsg):
        ...
        comment = "Create page for Yiddish %s %s of %s, pos=%s" % (infltype, joined_infls, lemma, pos)
        ...
        for i in range(len(sections)):
            ...
            if not m:
                ...
                if args.overwrite_page:
                    if pagename in pages_already_erased:
                        ...
                        for t in parsed.filter_templates():
                            tnam = tname(t)
                            if tnam in ["yi-noun", "yi-proper noun", "yi-adj", "yi-verb", "yi-adv", "yi-phrase"] or (
                                tnam == "head"
                                and getparam(t, "1") == "yi"
                                and getparam(t, "2")
                                in [
                        ...
                        if found_lemma:
                            errandpagemsg(
                                "WARNING: Page appears to have a lemma on it, not overwriting, skipping form: lemmas = %s"
                                % ",".join(found_lemma)
                            )
                            return
                    ...
                    for t in parsed.filter_templates():
                        if is_lemma_template(t):
                            if template_head_matches(t, inflections, "checking for lemma"):
                                pagemsg(
                                    "WARNING: Creating non-lemma form and found matching lemma template: %s" % str(t)
                                )
                            if is_noun_form:
                                tnam = tname(t)
                                if tnam in ["yi-noun", "yi-proper noun"] and any(
                ...
                if found_plurale_tantum_lemma and is_noun_form and is_noun_adj_plural:
                    pagemsg(
                        "WARNING: Found plurale tantum lemma and creating plural noun form, might need to add lemmas to allow_in_same_etym_section"
                    )
                    ...
                    for j in range(2, len(subsections), 2):
                        if re.match("^===+%s===+\n" % pos, subsections[j - 1]):
                            ...
                            def check_for_given_inflection_tag(parsed, tag):
                                for t in parsed.filter_templates():
                                    ...
                                    if not deftemp_needs_lang or compare_param(
                                        t, lang_param, "yi", None, issue_warnings=issue_warnings
                                    ):
                                ...
                                for tag_set_no, split_tag_set_group in enumerate(split_tag_sets):
                                    for indiv_tag_set in split_tag_set_group:
                                        if set(indiv_tag_set) > inflset:
                                            pagemsg_if(
                                                issue_warnings,
                                                "WARNING: Found actual inflection %s in template %s whose codes are a superset of intended codes %s, accepting"
                                        ...
                                        if indiv_set < inflset:
                                            ...
                                            if (indiv_set | {"pfv"}) == inflset or (indiv_set | {"impfv"}) == inflset:
                                                if len(split_tag_set_group) == 1:
                                                    ...
                                                    pagemsg_if(
                                                        issue_warnings,
                                                        "WARNING: Found actual inflection %s in template %s whose codes are a subset of intended codes %s and could update aspect except that multipart tags are present"
                                                        % ("|".join(indiv_tag_set), str(t), "|".join(infls)),
                                                    )
                                return False, 0
                            ...
                            for t in parsed.filter_templates():
                                ...
                                if compare_param(t, lemma_param, lemma, lemmatr, issue_warnings=issue_warnings) and (
                                    not deftemp_needs_lang
                                    or compare_param(t, lang_param, "yi", None, issue_warnings=issue_warnings)
                                ):
                            ...
                            if (
                                ...
                                if process_section_pass == 0:
                                    pagemsg("Exists and has Yiddish section and found %s already in it" % (infltype))
                            ...
                            # in same section.
                                ...
                                if (
                                    ...
                                    check_fix_defn_params(
                                        defn_templates_for_inserting_in_same_template[-1], -1, deftemp_param
                                    )
                                    pagemsg("Insert new tag set into existing {{%s}}" % (deftemp))
                ...
                # else of for loop over passes 0-3, i.e. no break out of loop
                    ...
                    def matching_defn_templates(parsed, check_for_sg_pl_pairs=False):
                        ...
                        for t in parsed.filter_templates():
                            ...
                            ) and (not deftemp_needs_lang or compare_param(t, lang_param, "yi", None)):
                                retval.append(t)
                        return retval
                    ...
                    for j in range(2, len(subsections), 2):
                        if re.match("^===+%s===+\n" % pos, subsections[j - 1]):
                            parsed = blib.parse_text(subsections[j])
                            defn_templates = matching_defn_templates(parsed, check_for_sg_pl_pairs=is_noun_form)
                            if defn_templates:
                                insert_at = j + 1
                    ...
                    if is_noun_form and is_noun_adj_plural:
                        ...
                        if found_plural_noun_form or found_plurale_tantum_lemma:
                            pagemsg(
                                "WARNING: Creating new etymology for plural noun form and found existing plural noun form or noun lemma"
                            )
                    ...
                    if not sections[i]:  # Erased section
                        ...
                        comment = (
                            "Wrap existing Yiddish section in Etymology 1, append entry (Etymology 2) for %s %s of %s, pos=%s"
...
def find_inflection_templates(
    text, expected_header, expected_poses, skip_poses, is_inflection_template, pagemsg, expand_text
):
    templates = []
...
def create_forms(
    ...
    for index, page in pages_to_process:
        ...
        inflection_templates = find_inflection_templates(
            page.text, expected_header, expected_poses, skip_poses, is_inflection_template, pagemsg, expand_text
        )
                        ...
                        for formval in formvals:
                            ...
                            if skip_inflections and skip_inflections(formname, formvalyi, formvaltr):
                                pagemsg(
                                    ...
                                    % (formname, formvalyi, formvaltr and " (%s)" % formvaltr or "")
                                )
                        ...
                        for formval_no_accents, inflections in formvals_by_pagename_items:
                            ...
                            if formval_no_accents == dicformyi:
                                ...
                                if len(inflections) > 1:
                                    pagemsg(
                                        "create_forms: For pagename %s, found multiple inflections %s"
...
def create_adj_forms(
    ...
    create_forms(
        ...
        adj_form_aliases,
        "adjective",
        "head|yi|adjective form",
        "pred",
        "Declension",
        ["Adjective", "Participle", "Ordinal numeral"],
...
parser.add_argument("--lemmafile", help="""File containing lemmas to process.""")
parser.add_argument("--lemmas", help="""Comma-separated list of lemmas to process.""")
parser.add_argument(
    "--overwrite-lemmas",
