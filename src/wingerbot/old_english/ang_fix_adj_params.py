        ...
        if tn == "head" and getparam(t, "1") == "ang" and getparam(t, "2") in ["adjective", "adjectives"]:
            for param in t.params:
                ...
                if pn not in ["1", "2", "head"]:
                    p.msg("WARNING: head|ang|adjective with extra params: %s" % str(t))
                    break
            else:
                # no break
...
parser = blib.create_argparser(
    "Fix Old English adjective headwords to new format", include_pagefile=True, include_stdin=True
)
args = parser.parse_args()
start, end = blib.parse_start_end(args.start, args.end)

blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_cats=["Old English adjectives"], new=True
)
