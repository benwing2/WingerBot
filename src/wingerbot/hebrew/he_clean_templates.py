...
all_he_form_of_template_map = {x[0]: (x[1], x[2]) for x in all_he_form_of_template_specs}
        ...
        for t in parsed.filter_templates():
            ...
            if tn in all_he_form_of_template_map:
                ...
                if newname == "he-verb form of":
                    assert newspecs
                    t.add("2", newspecs)
                    notes.append("rename {{%s}} to {{%s|{{{1}}}|%s%s}}" % (tn, newname, newspecs, add_nocap_msg))
                elif newname == "he-noun form of" and newspecs:
                    newparam, newval = newspecs.split("=")
                    t.add(newparam, newval)
                    notes.append(
                        "rename {{%s}} to {{%s|{{{1}}}|%s=%s%s}}" % (tn, newname, newparam, newval, add_nocap_msg)
                    )
                else:
