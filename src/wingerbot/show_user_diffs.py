    ...
    if pagetitle not in seen_pages:
        ...
        if revid == last_parentid:
            ...
            if revid == 0 or last_parentid == 0:
                pagemsg(
                    "WARNING: Non-contiguous change(s) to page, but unable to fetch in-between diff between rev ID's %s and %s"
                    % (revid, last_parentid)
                )
            else:
                pagemsg(
                    "WARNING: Non-contiguous change(s) to page: Diff between revs %s and %s:" % (revid, last_parentid)
                )
