...
class State(object):
    ...
    def is_eow(self, pos=None):
        ...
        return (
            a in [" ", "]", "|", ZWNJ]
            or a in word_final_punctuation
            or
...
tt_to_arabic_matching_boc = {  # beginning of later part of a compound
    "ا": [
        ...
        LatinMatch(
            "u",
            canon_to=lambda st: "u" if st.classical else "o",
            append=U,
        ...
        LatinMatch(
            "i",
            canon_to=lambda st: "i" if st.classical else "e",
            append=I,
...
tt_to_arabic_matching = {
    ...
    "ظ": ["z", "ẓ", "ðʿ", "ðˤ", "ðˁ", "ðʕ", "ð̣", "đʿ", "đˤ", "đˁ", "đʕ", "đ̣", "ż", "z̧", "ʐ", "dh"],
    "ع": ["'", "ʿ", "ʕ", "`", "‘", "ʻ", "3", "ˤ", "ˁ", ("ʾ",), "῾", ("’",), ""],
    ...
    "و": [
        "v",
        ["w"],
        LatinMatch("ow", canon_to="ow", insert=U),
        ...
        LatinMatch(
            "", canon_to="", when=lambda st: st.prevar() == "خ" and st.nextar() == "ا", handle_empty_match_early=True
        ),
    ],
    ...
    "ی": [
        ...
        ["i"],
        ["ê"],  # no e; short e opposite ی doesn't normally occur
...
tt_latin_to_unmatched_arabic = {
    ...
    "â": (
        ...
        and not (st.thisar() == "ه" and st.prevar() == double_l_subst and st.prevar(2) == "ل" and st.prevar(3) == "ا"),
    ),
...
def pre_canonicalize_latin(text, arabic=None, classical=False, msgfun=msg):
    if "{{" in text:
        ...
        retval = run_on_non_template_code(
            text, lambda run: pre_canonicalize_latin(run, None, classical=classical, msgfun=msgfun), msgfun
        )
    ...
    text = rsub(text, ".", {"ā": "â", "ē": "ê", "ī": "î", "ō": "ô", "ū": "û"})
    ...
    text = rsub(
        ...
        ".",
        {
            ...
            "ù": "u",
            "ḗ": "ê",
            "ṓ": "ô",  # only these two vowels have a single Unicode char for macron+acute
            "ấ": "â",
        },
    )
    if not classical:
...
def post_canonicalize_latin(text, classical=False, msgfun=msg):
    if "{{" in text:
        # Embedded templates. Don't touch the stuff inside the embedded part.
        retval = run_on_non_template_code(
            text, lambda text: post_canonicalize_latin(text, classical=classical, msgfun=msgfun), msgfun
        )
        if retval is not None:
            return retval
    ...
    text = rsub(
        text,
        "([" + vowel_chars + "][" + cons_chars + "]*)eh-e([" + word_final_punctuation + r" |\]]|'''|$])",
        r"\1e-ye\2",
    )
    text = rsub(
        text,
...
def pre_pre_canonicalize_arabic(text, msgfun=msg):
    ...
    text = rsub(text, "([\u064b\u064c\u064d\u064e\u064f\u0650\u0670])" + SH, SH + r"\1")
...
def pre_canonicalize_arabic(text, safe=False, msgfun=msg):
    if "{{" in text:
        # Embedded templates. Don't touch the stuff inside the embedded part.
        retval = run_on_non_template_code(
            text, lambda run: pre_canonicalize_arabic(run, safe=safe, msgfun=msgfun), msgfun
        )
        if retval is not None:
            return retval
...
def post_canonicalize_arabic(text, safe=False, msgfun=msg):
    if "{{" in text:
        # Embedded templates. Don't touch the stuff inside the embedded part.
        retval = run_on_non_template_code(
            text, lambda run: post_canonicalize_arabic(run, safe=safe, msgfun=msgfun), msgfun
        )
        if retval is not None:
            return retval
    ...
    text = rsub(text, SH + "([\u064b\u064c\u064d\u064e\u064f\u0650])", r"\1" + SH)
    return text
...
def tr_matching(obj, arabic, latin, err=False, msgfun=msg, no_vocalize=None):
    ...
    if latins is not None:
        ...
        for i, one_latin in enumerate(latins):
            ...
            try:
                this_arabic_res, this_latin_res, _, _ = tr_matching(
                    obj, arabic, one_latin, err=err, msgfun=msgfun, no_vocalize=True
                )
    ...
    latin = re.sub(r"(^|[\W" + vowel_chars + r"])([^'.{}\[\]])\2", r"\1\2" + SH, latin, 0, re.U)
    ...
    def check_latin_char_not_matching():
        ...
        if do_check(lcheck) and do_check(acheck):
            ...
            debprint(
                "check_latin_char_not_matching(): Saw Latin %s against %s, copying"
    ...
    while aind[0] < alen or lind[0] < llen:
        ...
        if lind[0] < llen and la[lind[0]] == SH:
            ...
            lind[0] += 1
            lres.append(SH)
            if aind[0] < alen and (ar[aind[0]] == SH or ar[aind[0]] == double_l_subst):
                res.append(ar[aind[0]])
                aind[0] += 1
        ...
        # ghurfatun al-kuuba example, among others.
            ...
            not st.is_bow()
            and aind[0] < alen
            and ar[aind[0]] in word_interrupting_chars
            and check_latin_not_matching_arabic()
        ):
...
def test_with_obj(obj, latin, arabic, should_outcome, should_latin=None):
    ...
    msg("canonicalize_latin(%s) = %s" % (latin, canonlatin))
