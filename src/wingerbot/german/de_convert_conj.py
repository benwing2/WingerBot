...
def compare_new_and_old_templates(origt, newt, pagetitle, pagemsg, errandpagemsg):
    ...
    old_forms = {
        k: v.replace("&#32;", " ").replace("&nbsp;", " ").strip().replace(" ,", ",") for k, v in old_forms.items()
    }
    ...
    for form in old_forms.keys() | new_forms.keys():
        ...
        if form not in old_forms:
            pagemsg(
                "WARNING: for original %s and new %s, form %s=%s in new forms but missing in old forms"
                % (str(origt), str(newt), form, new_forms[form])
            )
            return False
        if set(new_forms[form].split(",")) != set(old_forms[form].split(",")):
            pagemsg(
                "WARNING: for original %s and new %s, form %s=%s in old forms but =%s in new forms"
        ...
        if tn == "de-conj-strong":
            ...
            if shortimp:
                ...
                if args.correct:
                    t.add("9", "a")
                    notes.append(
                        "add 9=a (shortimp) in {{de-conj-strong}} because non-redundant pres23stem (6=) given without ä or ö"
                    )
            parts = []
...
parser = blib.create_argparser("Convert {{de-conj-strong}} to {{de-conj}}", include_pagefile=True, include_stdin=True)
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_refs=["Template:de-conj-strong"],
)
