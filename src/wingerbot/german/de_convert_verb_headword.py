...
def process_text_on_page(p):
    ...
    for t in parsed.filter_templates():
        ...
        tn = tname(t)
        if (
            tn in ["de-verb-old", "de-verb-strong", "de-verb-weak"]
            or tn == "head"
            and getparam(t, "1") == "de"
        ...
        if tn == "de-conj":
            ...
            for param in headt.params:
                ...
                if headtn == "head":
                    ...
                    allowed_params = [
                        ...
                        "past 2 qual",
                        "past participle 2",
                        "past participle 2 qual",
                        "past subjunctive",
                        "past subjunctive 2",
                        "past subjunctive 2 qual",
                        "auxiliary",
                        "cat",
                    ]
                else:
            ...
            if headtn == "de-verb-weak":
                ...
                if aux is None:
                    return
                if (
                    not compare(pres_3s, forms.get("pres_3s", "-").split(","), "pres 3sgs")
                    or not compare(past, forms.get("pret_3s", "-").split(","), "pasts")
                    or not compare(pp, forms.get("perf_part", "-").split(","), "pp's")
                    or not compare(aux, forms.get("aux", "-").split(","), "auxes")
                ):
                    headt = None
                    continue
            if headtn == "de-verb-strong":
                ...
                past_subj = canonicalize_existing(
                    [getparam(headt, "past subjunctive"), getparam(headt, "past subjunctive 2")]
                )
                ...
                if (
                    ...
                    or not compare(past, forms.get("pret_3s", "-").split(","), "pasts")
                    or not compare(pp, forms.get("perf_part", "-").split(","), "pp's")
                    or not compare(past_subj, forms.get("subii_3s", "-").split(","), "past subjs")
                    or not compare(aux, forms.get("aux", "-").split(","), "auxes")
                    or not compare(clazz, forms.get("class", "-").split(","), "classes")
                ):
                    headt = None
                    continue
            ...
            if arg1:
                headt.add("1", arg1)
            notes.append(
                "replace {{%s|...}} with new-style {{de-verb%s}}"
                % (headtn == "head" and "head|de|verb" or headtn, (arg1 and "|" + arg1 or ""))
            )
            headt = None
...
parser = blib.create_argparser(
    "Convert German verb headwords to use new {{de-verb}}"
)
args = parser.parse_args()
start, end = blib.parse_start_end(args.start, args.end)

blib.do_pagefile_cats_refs(args, start, end, process_text_on_page, default_cats=["German verbs"])