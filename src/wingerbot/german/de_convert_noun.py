...
def do_headword_template(headt, declts, pagetitle, subsections, subsection_with_head, subsection_with_declts, pagemsg):
        ...
        def compute_part(declspec, headword_parts, all_decl_parts, get_default_part, desc):
            ...
            if all_decl_parts == defparts:
                ...
                if set(headword_parts) == set(all_decl_parts):
                    ...
                    pagemsg(
                        "WARNING: Headword %s(s) %s not same as all decl %s(s) %s, continuing"
    ...
    for param in headt.params:
        ...
        pv = str(param.value)
        if (
            pn not in ["1", "2", "3", "4", "m", "f", "old"]
            and not re.search("^(g|gen|pl|dim|m|f)[0-9]+$", pn)
            and (not adjectival or pn not in "head")
        ):
    ...
    if adjectival:
        ...
        if m:
            ...
            if not noun:
                ...
                fake_declt = "{{de-decl-noun-%s%s|%s|pl=%s%s}}" % (
                    gender,
                    "" if gender == "f" else "|" + noun_gen,
                    noun_pl,
                    noun_pl_full,
                    "|n=sg" if is_sg else "",
                )
                ...
                def analyze_headword_parts_for_noun(parts, desc):
                    ...
                    for part in parts:
                        ...
                        if not m:
                            pagemsg(
                                "WARNING: Can't analyze headword %s '%s' into adjective and noun, continuing: head=%s, decl=%s"
                ...
                expected_gens = [
                    "%s %s" % (expected_gen, gen)
                    for expected_gen in expected_gens
                    for gen in ([noun] if gender == "f" else all_decl_gens)
                ]
                if is_sg:
                    expected_pls = []
                else:
            ...
            if pagetitle != expected_lemma:
                pagemsg(
                    "WARNING: For adjectival noun or adjective-noun combination, expected lemma '%s' but saw '%s': head=%s, decl=%s"
                    % (expected_lemma, pagetitle, str(headt), str(declt))
                )
                return
            if set(genders) != set(all_decl_genders):
                pagemsg(
                    "WARNING: For adjectival noun or adjective-noun combination, expected gender(s) '%s' but saw '%s': head=%s, decl=%s"
                    % (",".join(all_decl_genders), ",".join(genders), str(headt), str(declt))
                )
                return
            if not (set(gens) <= set(expected_gens)):
                pagemsg(
                    "WARNING: For adjectival noun or adjective-noun combination, expected genitive(s) '%s' but saw '%s': head=%s, decl=%s"
            ...
            if pls == ["-"]:
                if expected_pls:
                    pagemsg(
                        "WARNING: For adjectival noun or adjective-noun combination, expected plural(s) '%s' but saw '%s': head=%s, decl=%s"
        ...
        if headspec != "p":
            ...
            if saw_mn:
                ...
                if (
                    len(gens) == 1
                    and any(gens[0] == pagetitle + ending for ending in ["n", "en", "ns", "ens"])
                    and len(pls) == 1
                    and (pls[0] == "-" or any(pls[0] == pagetitle + ending for ending in ["n", "en"]))
                ):
                    is_weak = True
                def_gens = []
        ...
        if headspec != declspec:
            if set(all_decl_gens) <= set(headword_gens) and set(all_decl_pls) <= set(headword_pls):
                if set(all_decl_genders) == set(headword_genders):
                    pagemsg(
                        "NOTE: Headword spec '%s' not same as declension spec '%s', but decl gens %s a subset of headword gens %s and decl pls %s a subset of headword pls %s and gender(s) %s agree: headt=%s, declt=%s"
                        % (
                            headspec,
                            declspec,
                            ",".join(all_decl_gens),
                    ...
                    pagemsg(
                        "WARNING: Headword spec '%s' not same as declension spec '%s', decl gens %s a subset of headword gens %s and decl pls %s a subset of headword pls %s, but decl gender(s) %s don't agree with headword gender(s) %s: headt=%s, declt=%s"
                        % (
                            headspec,
                            declspec,
                            ",".join(all_decl_gens),
                ...
                pagemsg(
                    "WARNING: Headword spec '%s' not same as declension spec '%s' and either decl gens %s not a subset of headword gens %s or decl pls %s not a subset of headword pls %s, with decl gender(s) %s and headword gender(s) %s: headt=%s, declt=%s"
                    % (
                        headspec,
                        declspec,
                        ",".join(all_decl_gens),
    ...
    if declts:
        ...
        newsectext, replaced = blib.replace_in_text(
            subsections[subsection_with_declts], declts_existing, newdeclt, pagemsg, abort_if_warning=True
        )
        if not replaced:
            return
            ...
            if tn in ["de-noun", "de-proper noun"]:
                ...
                if declts:
                    if subsection_with_declts == k:
                        ...
                        pagemsg(
                            "WARNING: Saw declension template in new section without preceding head template: %s"
                            % str(t)
                        )
                        return
                if not headt:
