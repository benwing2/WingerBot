...
def analyze_stem(default_stem, stem, pagemsg):
    ...
    pagemsg(
        "WARNING: Can't analyze stem %s with respect to default stem %s, returning full stem" % (stem, default_stem)
    )
    return stem, stem
...
def analyze_comp_sup(stems, forms, generate_default, desc, pagemsg):
    ...
    minus_e_stems = [
        re.sub("^(.*)e([lmnr])$", r"\1" + OMITTED_E + r"\2", stem) if re.search("e[lmnr]$", stem) else None
        for stem in stems
    ]
    minus_e_forms_with_omitted_e = [
        x for form in minus_e_stems for x in (do_generate_default(form) if form else [None])
    ]
    minus_e_forms_to_forms_with_omitted_e = {
        form.replace(OMITTED_E, ""): form for form in minus_e_forms_with_omitted_e if form
    }
    ...
    def check_in_remainder(form, default_forms, to_append, map_to_source_forms=None):
        ...
        if not (set(default_forms) <= set(forms)):
            pagemsg(
                "WARNING: Saw %s form %s a subset of generated forms '%s' = %s, but remaining generated forms are not in total forms %s"
    ...
    while forms:
        ...
        if all(umlauted_stems) and form.startswith(umlauted_stems[0]):
            ...
            if success:
                continue
        pagemsg(
            "WARNING: Can't analyze %s form %s with respect to stem(s) %s, returning full form"
...
def do_headword_template(headt, declts, pagetitle, subsections, subsection_with_head, subsection_with_declts, pagemsg):
    ...
    def analyze_headt_or_declt(declt, headt):
        ...
        if declt:
            ...
            if pagetitle.endswith("e"):
                pagemsg(
                    "WARNING: No declt accompanying headt=%s and pagetitle ends in '-e', may be indeclinable, can't handle"
                    % str(headt)
                )
                return None
        ...
        if not head_comps and not head_sups and not decl_comps and not decl_sups:
            ...
            if declt:
                if headt and tname(headt) != "head":
                    if decl_comps != head_comps:
                        pagemsg(
                            "WARNING: Headword comparative(s) %s not equal to decl comparative(s) %s, not changing: headt=%s, declt=%s"
                            % (":".join(head_comps), ":".join(decl_comps), str(headt), str(declt))
                        )
                        return
                    if decl_sups != head_sups:
                        pagemsg(
                            "WARNING: Headword superlative(s) %s not equal to decl superlative(s) %s, not changing: headt=%s, declt=%s"
                ...
                if not comps and not sups:
                    pagemsg(
                        "WARNING: Something wrong, saw declt=%s without comps/sups and headt=%s with comps/sups"
                        % (str(declt), str(headt))
                    )
                    return
            else:
            ...
            compspecs, modified_comps = analyze_comp_sup(
                actual_stems, comps, generate_default_comp, "comparative", pagemsg
            )
            ...
            if set(sups) == set(default_sup_from_comp):
                pagemsg(
                    "Superlative(s) %s same as default superlative(s) generated from comparative(s) %s"
                    % (":".join(sups), ":".join(comps))
                )
                supspecs = []
            else:
                pagemsg(
                    "Superlative(s) %s NOT same as default superlative(s) %s generated from comparative(s) %s"
                    % (":".join(sups), ":".join(default_sup_from_comp), ":".join(modified_comps))
                )
                supspecs, _ = analyze_comp_sup(
                    analyzed_stems, sups, lambda stem: generate_default_sup(stem, ss), "superlative", pagemsg
                )
            if compspecs == ["+"]:
                compspec = "comp"
            else:
    ...
    if headt and tname(headt) == "head":
        ...
        if not old_style_headt:
            if declts:
                pagemsg(
                    "WARNING: Something wrong, saw new-style headt=%s and old-style declts=%s"
    ...
    if headt and str(headt) != newheadt:
        newsectext, replaced = blib.replace_in_text(
            subsections[subsection_with_head], str(headt), newheadt, pagemsg, abort_if_warning=True
        )
        if not replaced:
            return
    ...
    if declt:
        newsectext, replaced = blib.replace_in_text(
            subsections[subsection_with_declts], str(declt), newdeclt, pagemsg, abort_if_warning=True
        )
        if not replaced:
            return
...
def process_text_in_section(p, secnum, sectext):
            ...
            if tn in ["de-adj", "de-adjective"] or tn == "head" and getp("1") == "de" and getp("2") == "adjective":
                ...
                if declts:
                    if subsection_with_declts == k:
                        ...
                        pagemsg(
                            "NOTE: Saw declension template in new section without preceding head template: %s" % str(t)
                        )
                if not headt:
    ...
    if headt and not declts:
        if tname(headt) == "head":
            pagemsg(
                "NOTE: Saw raw head template %s without corresponding declension template, can't process" % str(headt)
            )
            return
        pagemsg("NOTE: Saw head template without corresponding declension template, still processing: %s" % str(headt))
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_cats=["German adjectives"]
)
