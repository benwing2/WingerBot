...
raw_and_form_of_alternation_re = "(?:%s)" % "|".join(
    "[%s%s]%s" % (tag[0].upper(), tag[0], tag[1:]) for tag in tags_for_raw_and_form_of_conversion
)
...
multitag_replacements = [
    ...
    # Irish
    ("pres|indc|and|pres|subj|and|impr|autonomous", "pres:ind//pres:sub//imp|autonomous"),
    ("pres|indc|and|impr|and|pres|subj|autonomous", "pres:ind//pres:sub//imp|autonomous"),
    ("pres|indc|autonomous|and|pres|subj|autonomous|and|impr|autonomous", "pres:ind//pres:sub//imp|autonomous"),
    ...
    ("3|p|pres|indc|dependent|and|pres|subj", "3|p|pres|indc|dependent|;|3|p|pres|subj"),
    ("nom|and|voc|and|dat|and|strong|gen", "nom//voc//dat//str:gen"),
    ("nom|and|voc|and|strong|gen|and|dat", "nom//voc//dat//str:gen"),
    ("nom|and|voc|and|strong|gen|p|and|dat|p", "nom//voc//dat//str:gen|p"),
    ("nom|and|voc|and|plural|and|strong|gen|p", "nom//voc//dat//str:gen|p"),
    ("nom|and|voc|and|dat|p|and|strong|gen|p", "nom//voc//dat//str:gen|p"),
    ...
    # Italian?
    ("3|s|pres|indc|and|2|s|impr", "3|s|pres|ind|;|2|s|imp"),
    # Ancient Greek
...
combinable_tags_by_dimension = {tag: dim for dim, tagdict in dimensions_to_tags.items() for tag in tagdict}
...
def output_stats_on_tag_set():
    ...
    for key, val in sorted(multipart_tag_stats_by_num_axes.items(), key=lambda x: -x[1]):
        msg("Num tag sets with %s multipart tags = %6s (%.2f%%)" % (key, val, val * 100.0 / num_tag_sets))
    msg("Tag sets by ordered dimensions of multipart tags:")
    for key, val in sorted(detailed_multipart_tag_stats.items(), key=lambda x: -x[1]):
        msg("%-40s = %6s (%.2f%%)" % (", ".join(key), val, val * 100.0 / num_tag_sets))
    msg("Tag sets by unordered dimensions of multipart tags:")
    for key, val in sorted(detailed_multipart_tag_stats_as_set.items(), key=lambda x: -x[1]):
        msg("%-40s = %6s (%.2f%%)" % (", ".join(sorted(list(key))), val, val * 100.0 / num_tag_sets))
...
def sort_tags(tags):
    ...
    for tag_group, unsortable in tag_groups:
        if len(tag_group) > 1:
            tag_group = sorted(tag_group, key=lambda x: indexed_order_of_dimensions[x[1]])
...
def canonicalize_raw_tag(tag, shorten, pagemsg, add_to_bad_tags_split_canon=False):
    if re.search(r"^(the |an |)\[*(act|object|indicative|end|all|part|form)\]*\s*$", tag, re.I):
        pagemsg(
            "WARNING: Saw 'the/an/ act/object/indicative/end/all/part/form of', not a valid tag, rejecting: %s" % tag
        )
        return None
...
def process_text_on_page(p):
    ...
    def convert_raw_section(text, section_langcode, infer_langcode):
        ...
        for with_non_gloss_defn in [False, True]:
            ...
            newtext = re.sub(
                r"^(#+(?: +|(?![#:*])))%s((?:\{\{.*?\}\} *)?\(?)(.* [Oo]f) ([[{'].*?|[a-zA-Z]+\.?)%s$"
                % (non_gloss_pretext, non_gloss_posttext),
                replace_raw_only_canonicalize,
                newtext,
                0,
                re.M,
            )
            ...
            if tn == "form of":
                ...
                if re.search(raw_and_form_of_alternation_re, tags):
                    for param in t.params:
                        ...
                        # Igore nodot
                        if (
                            pname in ["lang", "1", "2", "3", "4", "tr", "t", "gloss", "sc", "id", "nodot"]
                            or not lang_in_lang
                            and pname == "5"
                        ):
                            continue
        ...
        if tn in infltags.inflection_of_templates:
            ...
            for tag in tags:
                ...
                if repl is None:
                    ...
                    notemsg = (
                        "canonicalize multiword inflection tag '%s' to %s in %s"
                        if " " in tag
                        else "canonicalize inflection tag %s to %s in %s"
                    )
                    notes.append(
                        notemsg
                        % (
                            tag,
                            "|".join(repl) if type(repl) is list else repl,
                    ...
                    if i + len(fro) <= len(tags):
                        for j in range(len(fro)):
                            ...
                            if type(to) is list:
                                ...
                                joined_to = to
                            notes.append(
                                "replace inflection tag sequence %s with %s in %s"
            ...
            def append_combine_note(dim, orig_tags, combined_tag):
                notes.append(
                    "combine %s tags %s into %s in %s"
                    % (dim, orig_tags, combined_tag, infltags.construct_abbreviated_template(tn, lang, term))
                )
            ...
            while i < len(tags):
                ...
                if i <= len(tags) - 9 and (
                    tags[i] in combinable_tags_by_dimension
                    and tags[i + 1] in joiner_tags
                    and tags[i + 2] in combinable_tags_by_dimension
                    and tags[i + 3] in joiner_tags
                    and tags[i + 4] in combinable_tags_by_dimension
                    and tags[i + 5] in joiner_tags
                    and tags[i + 6] in combinable_tags_by_dimension
                    and tags[i + 7] in joiner_tags
                    and tags[i + 8] in combinable_tags_by_dimension
                    and combinable_tags_by_dimension[tags[i]] == combinable_tags_by_dimension[tags[i + 2]]
                    and combinable_tags_by_dimension[tags[i + 2]] == combinable_tags_by_dimension[tags[i + 4]]
                    and combinable_tags_by_dimension[tags[i + 4]] == combinable_tags_by_dimension[tags[i + 6]]
                    and combinable_tags_by_dimension[tags[i + 6]] == combinable_tags_by_dimension[tags[i + 8]]
                ):
                ...
                # are in the same dimension.
                    ...
                    tags[i] in combinable_tags_by_dimension
                    and tags[i + 1] in combinable_tags_by_dimension
                    and tags[i + 2] in joiner_tags
                    and tags[i + 3] in combinable_tags_by_dimension
                    and combinable_tags_by_dimension[tags[i]] == combinable_tags_by_dimension[tags[i + 1]]
                    and combinable_tags_by_dimension[tags[i + 1]] == combinable_tags_by_dimension[tags[i + 3]]
                ):
                    dim = combinable_tags_by_dimension[tags[i]]
                    tag1 = tag_to_canonical_form[tags[i]]
            ...
            infltags.put_back_new_inflection_of_params(
                t, notes, tags, params, lang, term, tr, alt, convert_to_more_specific_template=True
            )
            if origt != str(t):
                if not notes:
...
parser = blib.create_argparser("Clean up bad inflection tags", include_pagefile=True, include_stdin=True)
parser.add_argument("--textfile", help="File containing page text or defn line text to process.")
parser.add_argument(
    "--matching-textfile",
    help="File containing defn lines to match against; if we change a line not listed, output a warnings.",
)
...
parser.add_argument("--no-use-form-of-data", help="Don't use automatically-derived form-of data.", action="store_true")
parser.add_argument("--combine-adjacent", help="Combine adjacent calls to 'inflection of'.", action="store_true")
parser.add_argument(
    "--convert-raw", help="Convert raw inflection definitions to {{inflection of}}.", action="store_true"
)
parser.add_argument(
    "--convert-form-of", help="Convert {{form of}} inflection definitions to {{inflection of}}.", action="store_true"
)
...
if args.matching_textfile:
    matching_textfile_lines = set(text for title, text in fetch_page_titles_and_text(args.matching_textfile))

if not args.no_use_form_of_data:
    form_of_combinable_tags_by_dimension, form_of_tag_to_canonical_form = infltags.fetch_tag_tables()
...
msg(
    "Fraction of templates with bad tags = %s / %s = %.2f%%"
    % (
        num_templates_with_bad_tags,
        num_total_templates,
