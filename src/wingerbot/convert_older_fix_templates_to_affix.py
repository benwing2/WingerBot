...
templates_to_convert = (
    prefix_templates + suffix_templates + confix_templates + compound_templates + circumfix_templates + ["affix"]
)
    ...
    for t in parsed.filter_templates():
        ...
        if tn in user_specified_templates_to_do:
            ...
            if tn in prefix_templates:
                if not getp("3") and not getp("alt2") and not getp("tr2") and not getp("ts2"):
                    # If only a prefix, make the next term into a bare ^ for compatibility.
                    make_prefix(2)
                    make_non_affix(3)
                else:
                    for i in range(30, 2, -1):
                        if getp(str(i)) or getp("alt%s" % (i - 1)) or getp("tr%s" % (i - 1)) or getp("ts%s" % (i - 1)):
                            make_non_affix(i)
                            break
                    for j in range(2, i):
                        make_prefix(j)
