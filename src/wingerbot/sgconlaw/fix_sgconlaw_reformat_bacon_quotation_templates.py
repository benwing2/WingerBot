...
def process_text_on_page(index, pagename, text):
    ...
    for fromtemp, chapter in templates_to_rename:
        ...
        def reformat_template(m):
            ...
            notes.append(
                "reformat {{%s}} into {{RQ:Bacon Essayes|chapter=%s}}, incorporating following raw passage text into passage="
                % (fromtemp, chapter)
            )
            return str(t) + "\n"

        curtext = re.sub(r"(\{\{%s.*?\}\})\n#+\*:\s*(.*?)\n" % re.escape(fromtemp), reformat_template, curtext)
...
parser = blib.create_argparser(
    "Rename and reformat Bacon quotation templates for [[User:Sgconlaw]]", include_pagefile=True, include_stdin=True
)
parser.add_argument(
    "--direcfile",
    help="File containing templates to rename (without the Template: prefix) and the `chapter` parameter, separated by ' ||| '.",
    required=True,
)
