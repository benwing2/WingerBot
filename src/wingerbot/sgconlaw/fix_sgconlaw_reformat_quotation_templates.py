    ...
    for param, value in params:
        if re.search("^%[0-9]+$", value):  # %1, %2, ... for placeholder
            ...
            if existing:
                if existing != value:
                    pagemsg(
                        "WARNING: Template already has value for parameter %s (=%s): %s" % (param, existing, str(t))
                    )
                return False
...
def process_text_on_page(p):
    ...
    def reformat_template(t, from_params, totemp, to_params, text_to_incorporate=None):
        ...
        notes.append(
            ...
            % (
                fromtemp,
                "".join("|%s=%s" % (param, value) for param, value in from_params),
                totemp,
                "".join("|%s=%s" % (param, value) for param, value in to_params),
            )
        )
        return True
    ...
    for (fromtemp, from_params), (totemp, to_params) in templates_to_rename:
        ...
        curtext = re.sub(r"(\{\{%s.*?\}\})\n#+\*:\s*(.*?)\n" % re.escape(fromtemp), do_reformat_template, curtext)
...
parser.add_argument(
    "--direcfile",
    help="File containing pairs of templates to rename (without the Template: prefix), separated by ' ||| '.",
    required=True,
)
