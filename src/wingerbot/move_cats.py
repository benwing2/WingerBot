...
def process_text_on_page(p):
    ...
    for j, seclangname in secs.lang_list:
                    ...
                    if pn == "1":
                        ...
                        if newv is not None:
                            ...
                            if newtype == temptype:
                                ...
                                notes.append(
                                    "move %s cat '%s' to %s cat '%s' for lang=%s" % (temptype, pv, newtype, newv, lang)
                                )
        ...
        for m in re.finditer(
            r"\[\[(?:[Cc][Aa][Tt][Ee][Gg][Oo][Rr][Yy]|[Cc][Aa][Tt]):(.*?)((?:\|[^|\[\]\n]*)?)\]\]\n?", sectext
        ):
            ...
            if sortkey:
                sortkey = sortkey[
                    1:
                ]  # discard initial | sign; don't strip as we want to preserve space-initial sortkey
            else:
                sortkey = None
            m = re.search("^(.*?):(.*)$", fullcat)
            if m:
            ...
            if newv is not None:
                ...
                notes.append(
                    "move raw-coded %s cat '%s' to %s cat '%s' for lang=%s" % (cattype, cat, newtype, newv, langcode)
                )
        ...
        while cats_to_add:
            ...
            empty_cat_temp = "{{%s|%s%s}}" % (
                "C" if cattype == "topic" else "cln" if cattype == "poscat" else "cat",
                catlang,
                "|sort=" + catsortkey if catsortkey is not None else "",
            )
...
parser.add_argument(
    "--to", help="New name of template; can be specified multiple times", action="append", required=True
)
parser.add_argument(
    "--from-type",
    help="Old type of template; can be specified multiple times",
    action="append",
    required=True,
    choices=["topic", "poscat", "raw"],
)
parser.add_argument(
    "--to-type",
    help="New type of template; can be specified multiple times",
    action="append",
    required=True,
    choices=["topic", "poscat", "raw"],
)
...
if num_moves != len(args.to):
    raise ValueError(
        "Saw %s 'from' spec(s) '%s' but %s 'to' spec(s) '%s'; both must agree in number"
        % ((len(args.from_), ",".join(args.from_), len(args.to), ",".join(args.to)))
    )
if num_moves != len(args.from_type):
    raise ValueError(
        "Saw %s 'from' spec(s) '%s' but %s 'from' type(s) '%s'; both must agree in number"
        % ((len(args.from_), ",".join(args.from_), len(args.from_type), ",".join(args.from_type)))
    )
if num_moves != len(args.to_type):
    raise ValueError(
        "Saw %s 'from' spec(s) '%s' but %s 'to' type(s) '%s'; both must agree in number"
