    ...
    def replace_links(description):
        def replace_raw_link(m):
            ...
            if "|" in linktext:
                ...
                if len(parts) != 2:
                    p.msg("WARNING: More than two parts in linktext: %s" % linktext)
                    return m.group(0)
                link, display = parts
                ...
                p.msg("WARNING: Returning two-part link %s" % retval)
                return retval
            return "<<%s>>" % linktext
        ...
        def replace_like(m):
            ...
            p.msg("WARNING: Can't collapse like(...) expression: %s" % m.group(0))
            return m.group(0)

        description = re.sub(r"like\((.*?)\) <<(.*?)>>", replace_like, description)
        return description

    if "verb conjugation group" in p.title:
        label = re.sub("^Category:Greek ", "", p.title)
        breadcrumb = re.sub("^.*'(.*)'.*$", r"\1", label)
        lines = p.text.split("\n")
        breadcrumbs = lines[0]
        if not breadcrumbs.startswith("<small>>>"):
            p.msg("WARNING: Breadcrumb line doesn't look right: %s" % breadcrumbs)
            return
        ...
        if not m:
            p.msg("WARNING: Last line doesn't look like a category line: %s" % category)
            return
        parent2, conj_group_sort = m.groups()
        m = re.search(r"^Greek (.*) conjugation groups$", parent2)
        if not m:
            p.msg("WARNING: Category doesn't look like a conjugation group: %s" % parent2)
            return
...
""" % (
        ...
        p.msg("Appended <%s>" % cattext)
        outtext.append(cattext)
    elif "verbs conjugating like" in p.title:
        m = re.search("^Category:Greek verbs conjugating like '(.*?)'$", p.title)
        if not m:
            p.msg("WARNING: Can't parse 'verbs conjugating like' pagename")
            return
        likeverb = m.group(1)
        lines = p.text.split("\n")
        breadcrumbs = lines[0]
        ...
        if len(breadcrumbs) == 1:
            ...
            p.msg("WARNING: Too many breadcrumbs for 'verbs conjugating like' page: %s" % breadcrumbs)
            return
        m = re.search(r"^\[\[:Category:Greek verb conjugation group '(.*)'\]\]$", parent_group)
        if not m:
            p.msg("WARNING: Can't parse 'verbs conjugating like' parent-group breadcrumb: %s" % parent_group)
            return
        parent_group = m.group(1)
        if len(lines) < 3:
            p.msg("WARNING: Not enough lines for 'verbs conjugating like' page: <<%s>>" % p.text)
            return
        if not lines[-1].startswith("[[Category:") or not lines[-2].startswith("[[Category:"):
            p.msg(
                "WARNING: Last two lines aren't category references in 'verbs conjugating like' page: <<%s>>" % p.text
            )
            return
        ...
        if not m:
            p.msg(
                "WARNING: Can't parse conjugation number out of last two category lines: <%s>, <%s>"
                % (lines[-2], lines[-1])
            )
            return
        conj = m.group(1)
        ...
        p.msg("WARNING: Can't parse pagename")


parser = blib.create_argparser("Convert Greek verb categories to module", include_pagefile=True, include_stdin=True)
