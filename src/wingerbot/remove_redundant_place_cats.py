...
def process_text_on_page(p):
    ...
    def should_remove_cat(cat):
        ...
        for c in auto_added_categories:
            ...
            if m and cat in [
                m.group(1)
                + m.group(2),  # remove 'en:Cities' if auto-added cat is 'en:Cities in West Yorkshire, England'
                m.group(1)
                + m.group(
                    3
                ),  # remove 'en:West Yorkshire, England' if auto-added cat is 'en:Cities in West Yorkshire, England'
                m.group(1)
                + auto_cat_to_manual.get(
                    m.group(3), m.group(3)
                ),  # remove 'en:West Yorkshire' if auto-added cat is 'en:Cities in West Yorkshire, England'
            ]:
                return True
            if (
                m
                and m.group(2) == "Countries"
                and m.group(3) in ["Polynesia", "Micronesia", "Melanesia"]
                and ("Countries in Oceania" in cat)
            ):
                return True
            if (
                m
                and m.group(2) == "Countries"
                and m.group(3) in ["Central America", "Caribbean"]
                and ("Countries in North America" in cat)
            ):
                return True
        return False
...
parser = blib.create_argparser(
    "Remove redundant manually-added categories when {{place}} also adds them",
)
