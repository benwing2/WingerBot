                        ...
                        for param in t.params:
                            ...
                            if re.search("^" + paramspec + "$", pn):
                                if fn(pv):
                                    if negate_messages:
                                        pagemsg(
                                            "Skipping because param %s=%s matches filter %s: %s"
                                            % (pn, pv, filt_for_message, origt)
                                        )
                                    elif args.verbose:
                ...
                for old_param, new_param in params_to_rename:
                    ...
                    if t.has(old_param):
                        ...
                        if t.has(new_param):
                            if can_overwrite:
                                pagemsg(
                                    "When renaming %s=%s to %s=, already has %s=%s, overwriting"
                                    % (old_param, getp(old_param), new_param, new_param, getp(new_param))
                                )
                            else:
                                pagemsg(
                                    "WARNING: When renaming %s=%s to %s=, already has %s=%s"
                                    % (old_param, getp(old_param), new_param, new_param, getp(new_param))
                                )
                ...
                def insert_remaining_numeric_params():
                    ...
                    for param_to_insert, values_to_insert in sorted_params_to_insert:
                        if param_to_insert > local_last_param_inserted:
                            ...
                            for i in range(
                                max(max_existing_numeric_param, local_last_param_inserted) + 1, param_to_insert
                            ):
                    ...
                    if re.search("^[0-9]+$", pn):
                        ...
                        for param_to_insert, values_to_insert in sorted_params_to_insert:
                            ...
                            if param_to_insert > last_param_inserted and param_to_insert <= pnint:
                                ...
                                append_note(
                                    "insert %s=%s into {{%s}}" % (param_to_insert, "|".join(values_to_insert), tn)
                                )
...
parser.add_argument(
    "--rename-templates", help="Rename the templates whose references are being changed.", action="store_true"
)
parser.add_argument(
    "--with-redirect",
    action="store_true",
...
parser.add_argument(
    ...
    help="Comma-separated list of templates to ignore rename errors for, or 'all' for all templates.",
)
parser.add_argument("-r", "--remove", help="Param to remove, can be specified multiple times", action="append")
parser.add_argument(
    "--from", help="Old name of param, can be specified multiple times", metavar="FROM", dest="from_", action="append"
)
parser.add_argument(
    "--to",
    help="New name of param, can be specified multiple times; if param preceded by an !, can overwrite existing param",
    action="append",
)
parser.add_argument("--from-to-regex", help="Interpret values in --from and --to as regexes.", action="store_true")
parser.add_argument(
    "--prepend",
    help="PARAM=VALUE to add at the beginning, can be specified multiple times; VALUE can have {{PAGENAME}} in it to substitute the page title",
    action="append",
)
parser.add_argument(
    "--add",
    help="PARAM=VALUE to add at the end, can be specified multiple times; VALUE can have {{PAGENAME}} in it to substitute the page title",
    action="append",
)
parser.add_argument(
    "--insert",
    help="Insert numeric PARAM=VALUE|VALUE|..., moving greater numeric params to the right; can be specified multiple times, works from right to left; VALUE can have {{PAGENAME}} in it to substitute the page title",
    action="append",
)
parser.add_argument(
    "--filter",
...
parser.add_argument(
    "--recognized-params",
    help="Comma-separated list of regexps matching recognized params. Use - to indicate no recognized params. If the template contains any unrecognized params, a warning will be displayed and no action taken. Regexps are auto-anchored on both ends.",
)
...
if args.direcfile:
    ...
    if new_names is not None and len(new_names) != len(templates):
        if len(new_names) == 1:
            ...
            raise ValueError(
                "Saw %s template(s) '%s' but %s new name(s) '%s'; both must agree in number or there must be only one new name"
