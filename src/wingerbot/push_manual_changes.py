...
params.add_argument(
    "--undo-slash-newline-in-repl-only",
    action="store_true",
    help=r"Undo replacement of newlines with \n in replacement text only",
)
params.add_argument(
    "--comment",
    help="Comment of change log message (included in addition to any comments embedded in the manual changes)",
)
params.add_argument(
    "--include-what-changed",
    action="store_true",
    help="If no comment embedded in manual changes, include what changed in the changelog",
)
...
if args.origfile:
    ...
    direcfile_changes_dict = filter_split_changes_removing_no_effect_lines(
        direcfile_changes_dict, origfile_changes_dict
    )
