...
def check_participle(form, orig_pagemsg, orig_errandpagemsg):
    ...
    for t in parsed.filter_templates():
        ...
        if tn == "la-part":
            ...
            if actual_part != form:
                pagemsg("WARNING: Found actual participle %s, expected %s" % (actual_part, form))
...
def process_text_on_page(index, pagetitle, text):
    ...
    for t in parsed.filter_templates():
        tn = tname(t)
        if tn == "la-conj":
            vargs = lalib.generate_verb_forms(str(t), errandpagemsg, expand_text)
            for partslot in ["pres_actv_ptc", "perf_actv_ptc", "perf_pasv_ptc", "futr_actv_ptc", "futr_pasv_ptc"]:
...
parser = blib.create_argparser(
    "Check macrons of Latin verbs against participles", include_pagefile=True, include_stdin=True
)
args = parser.parse_args()
start, end = blib.parse_start_end(args.start, args.end)

blib.do_pagefile_cats_refs(args, start, end, process_text_on_page, stdin=True, default_cats=["Latin verbs"])
