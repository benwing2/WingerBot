...
def compare_new_and_old_templates(oldt, newt, pagetitle, pagemsg, errandpagemsg):
    ...
    return blib.compare_new_and_old_template_forms(
        oldt, newt, generate_old_forms, generate_new_forms, pagemsg, errandpagemsg
    )
            ...
            while True:
                ...
                if "N" not in subtypes and "-I" not in subtypes:
                    if (
                        ...
                        and base[:-2] == stem2
                        or base.islower()
                        and base.endswith("ēs")
                        and base[:-2] == stem2
                    ):
                        subtypes += ["I"]
                if (
                    "-N" not in subtypes
                    and "M" not in subtypes
                    and "F" not in subtypes
                    and "-I" not in subtypes
                    and "-pure" not in subtypes
                ):
                    if (
                        base.endswith("e")
                        and base[:-1] == stem2
                ...
                if "-N" not in subtypes and "M" not in subtypes and "F" not in subtypes:
                    if (
                        ...
                        and base[:-2] + "er" == stem2
                        or base.endswith("ma")
                        and base[:-2] + "mat" == stem2
                        or base.endswith("men")
                        and base[:-2] + "min" == stem2
                    ):
                        subtypes += ["N"]
...
parser = blib.create_argparser(
    "Fix Latin 3rd-decl plural nouns to specify plural lemma, and check new against old {{la-ndecl}} code",
