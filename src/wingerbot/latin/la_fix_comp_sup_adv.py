        ...
        if header == "Adverb":
            ...
            for t in parsed.filter_templates():
                if tname(t) == "comparative of":
                    if compt:
                        pagemsg("WARNING: Saw multiple {{comparative of}}: %s and %s" % (str(compt), str(t)))
                    else:
                        compt = t
                    ...
                    if supt:
                        pagemsg("WARNING: Saw multiple {{superlative of}}: %s and %s" % (str(supt), str(t)))
                    else:
                        supt = t
                        posdeg = blib.remove_links(getparam(t, "1"))
                        if not posdeg:
            ...
            if compt and supt:
                pagemsg("WARNING: Saw both comparative and superlative, skipping: %s and %s" % (str(compt), str(supt)))
                continue
            if not compt and not supt:
                pagemsg("WARNING: Didn't see {{comparative of}} or {{superlative of}} in section %s" % k)
                continue
            for t in parsed.filter_templates():
                tn = tname(t)
                ...
                if tn == "head":
                    if not getparam(t, "1") == "la":
                        ...
                        if pos not in [
                            ...
                            pagemsg("WARNING: Unrecognized part of speech '%s': %s" % (pos, str(t)))
                        else:
                            real_head, real_comp, real_sup = find_head_comp_sup(
                                lalib.remove_macrons(posdeg), pagemsg, errandpagemsg
                            )
                            if real_head:
                            ...
                            if compt:
                                ...
                                if real_comp and real_comp != "-":
                                    if lalib.remove_macrons(real_comp) != lalib.remove_macrons(head):
                                        ...
                                        pagemsg("Using real comparative degree %s instead of %s" % (real_comp, head))
                                        head = real_comp
                                else:
                                    pagemsg(
                                ...
                                if real_sup and real_sup != "-":
                                    if lalib.remove_macrons(real_sup) != lalib.remove_macrons(head):
                                        ...
                                        pagemsg("Using real superlative degree %s instead of %s" % (real_sup, head))
                                        head = real_sup
                                else:
                                    pagemsg(
