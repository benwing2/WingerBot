...
def generate_verb_forms(
    ...
    def augment_with_sync_forms(forms):
        ...
        for form in forms:
            ...
            if re.search(
                "(vi(stī|stis)|vērunt|ver(am|ās|at|āmus|ātis|ant|ō|im|[iī]s|it|[iī]mus|[iī]tis|int)|viss(e|em|ēs|et|ēmus|ētis|ent))$",
                form,
            ):
...
breves = "ăĕĭŏŭĂĔĬŎŬ" + BREVE + DOUBLE_INV_BREVE
macrons = "āēīōūȳĀĒĪŌŪȲ" + MACRON
diaereses = "äÄëËïÏöÖüÜÿŸ" + DIAER
...
def remove_macrons(text, preserve_diaeresis=False):
    if preserve_diaeresis:
        return re.sub("([" + macrons_breves + "])", lambda m: deaccent_mapper[m.group(1)], text)
    else:
        return re.sub("([" + macrons_breves_diaereses + "])", lambda m: deaccent_mapper[m.group(1)], text)


def remove_non_macron_accents(text):
    return re.sub("([" + breves + diaereses + "])", lambda m: deaccent_mapper[m.group(1)], text)
...
def sort_tag_set(tag_set, groups, pagemsg):
    ...
    tag_to_level = {tag: num for num, tag_level in enumerate(groups) for tag in tag_level}
    tag_set_with_levels = []
    for tag in tag_set:
...
la_noun_decl_overrides = ["%s_%s" % (case, num) for num in ["sg", "pl"] for case in cases]

la_adj_decl_overrides = ["%s_%s_%s" % (case, num, g) for g in ["m", "f", "n"] for num in ["sg", "pl"] for case in cases]

la_verb_finite_overrides = [
    "%s%s_%s_%s" % (pernum, tense, voice, mood)
    ...
    if not (
        mood == "subj" and tense in ["futr", "futp"] or mood == "impr" and tense in ["impf", "perf", "plup", "futp"]
    )
]

la_verb_inf_ptc_overrides = [
    "%s_%s_%s" % (tense, voice, form)
    for tense in ["pres", "perf", "futr"]
    for voice in ["actv", "pasv"]
    for form in ["inf", "ptc"]
]

la_verb_ger_sup_overrides = ["ger_nom", "ger_gen", "ger_dat", "ger_acc", "sup_acc", "sup_abl"]

la_verb_overrides = la_verb_finite_overrides + la_verb_inf_ptc_overrides + la_verb_ger_sup_overrides
...
def la_get_headword_from_template(t: Template, pagename: str, pagemsg: PagemsgCallback, expand_text: ExpandTextCallback | None = None) -> list[str]:
    ...
    if tn in ["la-adj", "la-part", "la-num-adj", "la-suffix-adj", "la-det", "la-pronoun"]:
        ...
        tn in la_adj_headword_templates
        or tn in la_adv_headword_templates
        or (tn in ["la-suffix", "la-suffix-adv", "la-gerund"])
    ):
        retval = getparam(t, "1")
    elif tn == "la-letter":
        retval = pagename
...
def slot_matches_spec(slot, spec):
    if spec == "all":
        ...
        return (
            slot != "perf_pasv_ptc"
            and "pasv" in slot
            and ("1s" in slot or "1p" in slot or "2s" in slot or "2p" in slot or "3p" in slot)
        )
    elif spec in ["12pasv", "12pass"]:
        return (
            slot != "perf_pasv_ptc"
            and "pasv" in slot
            and ("1s" in slot or "1p" in slot or "2s" in slot or "2p" in slot)
        )
    elif spec == "passnofpp":
        return slot not in ["perf_pasv_ptc", "futr_pasv_ptc"] and "pasv" in slot
    elif spec == "perf":
        return slot not in ["perf_actv_ptc", "perf_pasv_ptc"] and re.search("(perf|plup|futp)", slot)
        ...
        for t in parsed.filter_templates():
            ...
            if tn == "la-IPA":
                ...
                if headword_templates_in_section:
                    ...
                    headwords.append(most_recent_headword)
                most_recent_headword = new_headword(
                    header,
                    level,
                    k,
                    t,
                    tn in la_lemma_headword_templates or (tn == "head" and head_pos in la_lemma_poses),
                )
