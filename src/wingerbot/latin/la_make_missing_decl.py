...
def process_line(index, line, online):
    line = line.strip()
    m = re.search(
        r"^Page [0-9]+ (.*?): WARNING: Saw noun headword template.*: (\{\{la-(?:proper )?noun\|.*?\}\})$", line
    )
    if not m:
    ...
    if len(noun_decl) == 0:
        ...
        return
    if len(noun_decl) > 1:
        pagemsg("WARNING: Multiple decls %s, skipping: %s" % (",".join(noun_decl), str(t)))
        return
    noun_decl = noun_decl[0]
    if noun_decl not in noun_decl_to_decl_type:
        pagemsg("WARNING: Unrecognized declension %s, skipping: %s" % (noun_decl, str(t)))
        return
    ...
    if decl_type in ["1", "2", "4", "5"]:
        ...
        if len(genitive) == 0:
            pagemsg("WARNING: No genitives with decl 3 lemma %s, skipping: %s" % (lemma, str(t)))
            return
        elif len(genitive) > 1:
            pagemsg(
            ...
            if gen1.endswith("is"):
                ...
                if lemma.endswith("ia"):
                    ...
                    pagemsg("WARNING: Unrecognized lemma %s with decl 3 genitive -ium, skipping: %s" % (lemma, str(t)))
                    return
            elif gen1.endswith("um"):
                if lemma.endswith("a") or lemma.endswith("ēs"):
                    la_ndecl = "{{la-ndecl|%s<3.pl>}}" % lemma
                else:
                    pagemsg("WARNING: Unrecognized lemma %s with decl 3 genitive -um, skipping: %s" % (lemma, str(t)))
                    return
            else:
                pagemsg("WARNING: Unrecognized genitive %s with decl 3 lemma %s, skipping: %s" % (gen1, lemma, str(t)))
                return
    elif decl_type == "irreg":
        pagemsg("WARNING: Can't handle irregular nouns, skipping: %s" % str(t))
        return
    else:
        pagemsg("WARNING: Something wrong, unrecognized decl_type %s, skipping: %s" % (decl_type, str(t)))
        return
    ...
    if online:
        ...
        la_convert_headword_noun.compare_headword_decl_forms(
            ...
            ["gen_sg", "gen_pl"],
            noun_props,
            "headword=%s, decl=%s" % (str(t), la_ndecl),
            pagemsg,
