...
def process_text_on_page(index, pagetitle, text):
    ...
    for k in range(2, len(subsections), 2):
        ...
        def compare_headword_conj_forms(
            id_slot, headword_forms, conj_slots, adjust_for_missing_perf_forms=False, remove_conj_links=False
        ):
            conj_forms = ""
        ...
        if not compare_headword_conj_forms(
            "lemma", lemma, ["1s_pres_actv_indc", "3s_pres_actv_indc", "1s_perf_actv_indc", "3s_perf_actv_indc"]
        ):
            continue
        ...
        for param in la_verb_template.params:
            ...
            if pname.strip() in ["1", "2", "3", "4", "44", "conj", "c", "pattern"] or re.search(
                "^(head|inf|perf|sup)[0-9]*$", pname.strip()
            ):
                continue
...
parser = blib.create_argparser(
    "Convert Latin verb headword templates to new form", include_pagefile=True, include_stdin=True
)
