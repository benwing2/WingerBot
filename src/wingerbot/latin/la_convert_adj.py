...
def la_adj_1_and_2_subtype(stem1, stem2, decl, types, num, g, is_adj, pagetitle, pagemsg):
    if stem2:
        pagemsg("WARNING: stem2=%s should not be present with 1&2 adjectives" % stem2)
        stem2 = ""
    set_stem1 = False
    if stem1.endswith("(e)r"):
        ...
        if macronless_stem1 != pagetitle and macronless_stem1 + "us" != pagetitle:
            pagemsg("WARNING: Potential 1&2 adjective ending in -er or -ur, but pagetitle=%s not same" % pagetitle)
...
def la_adj_1_1_subtype(stem1, stem2, decl, types, num, g, is_adj, pagetitle, pagemsg):
    if stem2:
        pagemsg("WARNING: stem2=%s should not be present with 1-1 adjectives" % stem2)
        stem2 = ""
...
def la_adj_2_2_subtype(stem1, stem2, decl, types, num, g, is_adj, pagetitle, pagemsg):
    if stem2:
        pagemsg("WARNING: stem2=%s should not be present with 2-2 adjectives" % stem2)
        stem2 = ""
    if num == "pl":
...
def la_adj_3rd_1E_subtype(stem1, stem2, decl, types, num, g, is_adj, pagetitle, pagemsg):
    ...
    if re.search("(is|[ij]or|e)$", stem1):
        pagemsg("WARNING: Possible wrongly tagged adj, decl=3-1, stem1=%s, stem2=%s" % (stem1, stem2))
        decl = "3-1"
...
def la_adj_3rd_3E_subtype(stem1, stem2, decl, types, num, g, is_adj, pagetitle, pagemsg):
    ...
    if not stem1.endswith("er"):
        pagemsg("WARNING: Possible wrongly tagged adj, decl=3-3, stem1=%s, stem2=%s" % (stem1, stem2))
        decl = "3-2"
    else:
...
def generate_old_adj_forms(template, errandpagemsg, expand_text, return_raw=False, include_linked=False):
    ...
    def generate_adj_forms_prefix(m):
        ...
        if m.group(1) in decl_suffix_to_decltype:
            return "{{la-generate-adj-forms|decltype=%s|" % (decl_suffix_to_decltype[m.group(1)])
        return m.group(0)

    if template.startswith("{{la-adecl|"):
        generate_template = re.sub(r"^\{\{la-adecl\|", "{{la-generate-adj-forms|", template)
    else:
        generate_template = re.sub(r"^\{\{la-(.*?)\|", generate_adj_forms_prefix, template)
...
def convert_template_to_new(t, pagetitle, pagemsg, errandpagemsg):
    ...
    if specified_subtypes:
        ...
        specified_subtypes = []
    lemma, stem2, decl, subtypes = compute_props(
        stem1, stem2, decl, specified_subtypes, num, None, True, pagetitle, pagemsg
    )
    if num == "sg":
        subtypes.append("sg")
    decl += "+"
...
if __name__ == "__main__":
    parser = blib.create_argparser(
        "Convert Latin adj decl templates to new form", include_pagefile=True, include_stdin=True
    )
