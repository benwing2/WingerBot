...
la_noun_decl_suffix_to_decltype = {
    ...
    "1st-am": [("1", "am"), "ām", None, ()],
    "1st-Greek": [("1", "Greek"), "ē", None, ()],
    "1st-Greek-Ma": [("1", "Greek-Ma"), "ās", None, ()],
    "1st-Greek-Me": [("1", "Greek-Me"), "ēs", None, ()],
    "2nd": ["2", "us", "ī", lambda stem1, stem2, num: ("-ius",) if stem1.endswith("i") else ()],
    "2nd-er": [("2", "er"), "", None, ()],
    "2nd-Greek": [("2", "Greek"), "os", None, ()],
    "2nd-N-ium": [("2", "N-ium"), "ium", "ia", ()],
    "2nd-ius": [("2", "ius"), "ius", "iī", la_noun_2nd_ius_subtype],
    "2nd-ius-voci": [("2", "ius-voci"), "ius", "iī", la_noun_2nd_ius_voci_subtype],
    "2nd-N": [("2", "N"), "um", "a", lambda stem1, stem2, num: ("-ium",) if stem1.endswith("i") else ()],
    "2nd-N-Greek": [("2", "Greek-N"), "on", None, ()],
    "2nd-N-us": [("2", "N-us"), "us", "ī", ("N",)],
    "3rd": ["3", "", None, la_noun_3rd_subtype],
    "3rd-Greek": [("3", "Greek"), "", None, la_noun_3rd_Greek_subtype],
    "3rd-Greek-er": [("3", "Greek-er"), "ēr", None, ("Greek",)],
    "3rd-Greek-on-M": [("3", "Greek-on"), "ōn", None, ("Greek",)],
    ...
    "4th": ["4", "us", "ūs", ()],
    "4th-argo": [("4", "argo"), "ō", None, ("argo",)],
    "4th-echo": [("4", "echo"), "ō", None, ("echo",)],
    "4th-N": [("4", "N"), "ū", "ua", ()],
    "4th-N-ubus": [("4", "N-ubus"), "ū", "ua", ("ubus",)],
    "4th-ubus": [("4", "ubus"), "us", "ū", ("ubus",)],
    "5th": ["5", "ēs", None, lambda stem1, stem2, num: ("-i",) if stem1.endswith("i") else ()],
    "5th-i": [("5", "i"), "iēs", None, ()],
    "5th-VOW": [("5", "vow"), "ēs", None, lambda stem1, stem2, num: ("-i",) if stem1.endswith("i") else ()],
    "indecl": ["indecl", "", None, ()],
...
def generate_old_noun_forms(template, errandpagemsg, expand_text, include_linked=False):
    ...
    def generate_noun_forms_prefix(m):
        if m.group(1) in la_noun_decl_suffix_to_decltype:
            ...
            return "{{la-generate-noun-forms|decl=%s%s%s|" % (decl, decltype, num)
        return m.group(0)

    if template.startswith("{{la-ndecl|"):
        generate_template = re.sub(r"^\{\{la-ndecl\|", "{{la-generate-noun-forms|", template)
    else:
        generate_template = re.sub(r"^\{\{la-decl-(.*?)\|", generate_noun_forms_prefix, template)
...
def compare_new_and_old_templates(origt, newt, pagetitle, pagemsg, errandpagemsg):
    ...
    return blib.compare_new_and_old_template_forms(
        origt, newt, generate_old_forms, generate_new_forms, pagemsg, errandpagemsg
    )


def compute_noun_lemma_and_subtypes(decl, stem1, stem2, num, stem_suffix, pl_suffix, to_auto, pagemsg, origt):
    ...
    if re.search("^[A-ZĀĒĪŌŪȲĂĔĬŎŬ]", lemma):
        ...
        auto_sg = (
            decl == "3"
            and lemma.endswith("polis")
            and "-polis" not in subtypes
            or decl == "indecl"
            or decl == "irreg"
            and lemma in ["Deus", "Iēsus", "Jēsus", "Callistō", "Themistō"]
        )
    ...
    if num:
        subtypes.append(num)
    if stem2:
        if decl == "3" and lalib.infer_3rd_decl_stem(lemma) == stem2 or decl == "2" and lemma == stem2:
            stem2 = ""
...
def convert_la_decl_multi_to_new(t, pagetitle, pagemsg, errandpagemsg):
    ...
    for i in range(1, len(segments) - 1, 2):
        ...
        if decl == "":
            ...
            lemma, stem2, decl, subtypes = adj_compute_props(
                stem1, stem2, decl, list(specified_subtypes), num, g, False, pagetitle, pagemsg
            )
            ...
            if lookup_key not in noun_decl_and_subtype_to_props:
                errandpagemsg("WARNING: Lookup key %s not found: %s" % (lookup_key, origt))
                return None
            auto_num, stem_suffix, pl_suffix, to_auto = noun_decl_and_subtype_to_props[lookup_key]
            lemma, stem2, subtypes = compute_noun_lemma_and_subtypes(
                decl, stem1, stem2, num, stem_suffix, pl_suffix, to_auto, pagemsg, origt
            )
            base_and_detected_subtypes = expand_text(
                "{{#invoke:User:Benwing2/la-noun|detect_subtype|%s|%s|%s|%s}}"
                % (lemma, stem2, decl, ".".join(subtypes))
            )
            ...
            if (
                g == "N"
                and (
                    "M" in detected_subtypes
                    or "F" in detected_subtypes
                    or "N" not in detected_subtypes
                    and "N" not in subtypes
                )
                or (g == "M" or g == "F")
                and ("N" in detected_subtypes)
            ):
                errandpagemsg(
                    "WARNING: Incompatible gender specification: g=%s, subtypes=%s, detected_subtypes=%s: %s"
...
def convert_template_to_new(t, pagetitle, pagemsg, errandpagemsg):
    ...
    num = getrmparam(t, "num")
    lemma, stem2, subtypes = compute_noun_lemma_and_subtypes(
        declspec, stem1, stem2, num, stem_suffix, pl_suffix, to_auto, pagemsg, origt
    )
    loc = getrmparam(t, "loc")
    if bool_param_is_true(loc):
        subtypes.append("loc")
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_cats=["Latin nouns", "Latin proper nouns"],
)
