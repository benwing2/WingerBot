...
def la_verb_1st_subtype(stem, arg2, arg3, arg4, types):
    ...
    if not perf_stem and not depon and "noperf" not in types:
        perf_stem = stem + "āv"
        arg2 = perf_stem
    if not supine_stem and (
        "nopass" not in types
        and "noperf" not in types
        and "nosup" not in types
        and "no-pasv-perf" not in types
        and "nopasvperf" not in types
        and "memini" not in types
        and "pass-3only" not in types
        and "pass3only" not in types
    ):
        supine_stem = stem + "āt"
        if depon:
            arg2 = supine_stem
...
def la_verb_2nd_subtype(stem, arg2, arg3, arg4, types):
    ...
    types = [x for x in types if x != "depon" and x != "impers"]
    if (depon and arg2 == stem + "it") or (not depon and arg2 == stem + "u" and arg3 == stem + "it"):
...
def la_verb_4th_subtype(stem, arg2, arg3, arg4, types):
    ...
    if "impers" in types or "3only" in types:
        ...
        lemma = stem + ("ior" if "depon" in types else "iō")
    types = [x for x in types if x != "depon" and x != "impers"]
    if (depon and arg2 == stem + "īt") or (not depon and arg2 == stem + "īv" and arg3 == stem + "īt"):
...
def convert_template_to_new(t, pagetitle, pagemsg, errandpagemsg):
    ...
    if not types:
        ...
        types = re.split(
            "(opt-semi-depon|semi-depon|pass-3only|pass-impers|no-actv-perf|no-pasv-perf|perf-as-pres|short-imp|sup-futr-actv-only|[a-z0-9]+)",
            types,
        )
    ...
    if conj != "irreg":
        if "noperf" in types and not depon:
            if arg3:
                pagemsg("WARNING: Perfect %s specified along with noperf: %s" % (arg3, origt))
                arg3 = ""
        if "nosup" in types:
            if depon:
                if arg3:
                    pagemsg("WARNING: Supine %s specified along with nosup: %s" % (arg3, origt))
                    arg3 = ""
            else:
                if arg4:
                    pagemsg("WARNING: Supine %s specified along with nosup: %s" % (arg4, origt))
                    arg4 = ""
...
parser = blib.create_argparser(
    "Convert Latin verb conj templates to new form", include_pagefile=True, include_stdin=True
)
