...
def process_text_on_page(p):
                ...
                if i % 2 == 0:
                    ...
                    if key in ["Cyrillic", "Cyrillic spelling", "Cyrillic equivalent"]:
                        if Cyrillic_equivalent is not None:
                            langpagemsg(
                                "WARNING: Saw two Cyrillic equivalents %s and %s" % (Cyrillic_equivalent, value)
                            )
                        else:
                    ...
                    if key in ["Devanagari", "Devanagari spelling", "Devanagari equivalent"]:
                        if Devanagari_equivalent is not None:
                            langpagemsg(
                                "WARNING: Saw two Devanagari equivalents %s and %s" % (Devanagari_equivalent, value)
                            )
                        else:
    ...
    for j, seclang in secs.lang_list:
        ...
        def replace_ordinal_def(m):
            ...
            def alphrest_in_default_script(alphrest):
                return (
                    args.letter_def == "Arab-def"
                    and alphrest.lower()
                    in ["arabic script", "perso-arabic script", "shahmukhi script", "{{w|shahmukhi}} script"]
                    or args.letter_def == "letter def"
                    and alphrest.lower()
                    in [
                        "devanagari",
                        "devanagari script",
        ...
        for t in parsed.filter_templates():
            ...
            if letter_def_template:
                ...
                for line in lines:
                    m = re.search(
                        r"^\* Previous letter: *\{\{l\|%s\|([^|{}]*)(?:\|tr=([^{}|]*))?\}\} *$" % langcode, line
                    )
                    if m:
                        newprec = m.group(1)
                        if m.group(2):
                        ...
                        m = re.search(
                            r"^\* Next letter: *\{\{l\|%s\|([^|{}]*)(?:\|tr=([^{}|]*))?\}\} *$" % langcode, line
                        )
                        if m:
                            newfoll = m.group(1)
                            if m.group(2):
    ...
    if notes_replace_ordinal_def:
        notes.append(
            "replace ordinal def for %s letter with {{%s|...}}"
