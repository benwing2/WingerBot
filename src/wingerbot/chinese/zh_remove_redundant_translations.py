                ...
                if re.search(r"^\* *Chinese:* *[^: ]", line):
                    ...
                    if not m:
                        ...
                        def combine_traditional_simplified(tradt, simpt, reversed=False):
                            ...
                            if lang == "zh":
                                append_line_pagemsg(
                                    "WARNING: Internal error: Unconverted 'zh' in Chinese translation template %s"
                                    % str(simpt)
                                )
                                return False, warnings
                            if lang != lectcode:
                                append_line_pagemsg(
                                    "WARNING: Strange language in %s translation template %s" % (lect, str(simpt))
                                )
                                return False, warnings
                            trad = get(tradt, "2")
                            simp = get(simpt, "2")
                            ...
                            if trad_to_simp == simp:
                                if tname(tradt) != tname(simpt):
                                    append_line_pagemsg(
                                        "Traditional translation template %s and corresponding simplified translation template %s have different template names, will still combine"
                                        % (str(tradt), str(simpt))
                                    )
                                ...
                                for param in simpt.params:
                                    ...
                                    if pn not in ["1", "2", "3", "tr", "alt", "sc", "lit"]:
                                        append_line_pagemsg(
                                            "WARNING: Unrecognized parameter %s=%s in simplified translation template %s, can't combine"
                                            % (pn, pv, str(simpt))
                                        )
                                        return False, warnings
                                    if pn == "3" and pv:
                                        append_line_pagemsg(
                                            "WARNING: Gender %s=%s specified in simplified translation template %s, can't combine"
                                            % (pn, pv, str(simpt))
                                        )
                                        return False, warnings
                                else:  # no break
                                    for param in tradt.params:
                                        ...
                                        if pn not in ["1", "2", "3", "tr", "alt", "sc", "lit"]:
                                            append_line_pagemsg(
                                                "WARNING: Unrecognized parameter %s=%s in traditional translation template %s, can't combine"
                                                % (pn, pv, str(tradt))
                                            )
                                            return False, warnings
                                        if pn == "3" and pv:
                                        ...
                                        if trad_tr and not simp_tr:
                                            ...
                                            append_line_pagemsg(
                                                "WARNING: Saw transliteration parameter '%s' in traditional translation template %s different from simplified transliteration '%s', can't combine"
                                        ...
                                        if trad_lit and not simp_lit:
                                            ...
                                            append_line_pagemsg(
                                                "WARNING: Saw literal parameter '%s' in traditional translation template %s different from simplified literal '%s', can't combine"
                                        ...
                                        if trad_alt and not simp_alt:
                                            append_line_pagemsg(
                                                "WARNING: Traditional translation template %s has alt= but corresponding simplified template %s doesn't, can't combine"
                                                % (str(tradt), str(simpt))
                                            )
                                            return False, warnings
                                        if not trad_alt and simp_alt:
                                            append_line_pagemsg(
                                                "WARNING: Traditional translation template %s doesn't have alt= but corresponding simplified template %s does, can't combine"
                                                % (str(tradt), str(simpt))
                                            )
                                            return False, warnings
                                        if trad_alt and simp_alt:
                                            ...
                                            if auto_simp_alt != simp_alt:
                                                append_line_pagemsg(
                                                    "WARNING: Traditional translation template %s has alt= which converts to %s != %s from corresponding simplified template %s, can't combine"
                                        ...
                                        if tradt.has("3"):
                                            if get(tradt, "3"):
                                                append_line_pagemsg(
                                                    "WARNING: Internal error: Non-blank gender 3= in traditional template %s"
                                                    % str(tradt)
                                                )
                                                return False, warnings
                                            rmparam(tradt, "3")
                                        ...
                                        if trad == simp:
                                            ...
                                            text_to_remove.append(
                                                (
                                                    (
                                                        "%s%s" % (this_text_to_remove, str(tradt))
                                                        if reversed
                                                        else "%s%s" % (str(tradt), this_text_to_remove)
                                                    ),
                                                    str(tradt),
                                                )
                                            )
                                        ...
                                        this_notes.append(
                                            "remove redundant simplified %s translation template (trad=%s, simp=%s%s%s)"
                                            % (
                                                lect,
                                                trad,
                                                simp,
                                                ", tr=%s" % either_tr if either_tr else "",
                                                ", lit=%s" % either_lit if either_lit else "",
                                            )
                                        )
                                        return True, warnings
                            elif not get(tradt, "tr") and get(simpt, "tr"):
                                append_line_pagemsg(
                                    "WARNING: Traditional translation template %s has translation %s which simplifies to %s != %s from corresponding simplified template %s, can't combine"
                        ...
                        for t in parsed.filter_templates():
                            if prevt and tname(t) in translation_templates and tname(prevt) in translation_templates:
                                ...
                                if combinable is False:
                                    line_pagemsg(
                                        "Can't combine traditional template %s and simplified template %s, trying reversed"
                                        % (str(prevt), str(t))
                                    )
                                    rev_combinable, rev_warnings = combine_traditional_simplified(
                                        t, prevt, reversed=True
                                    )
                                    if rev_combinable:
                                        line_pagemsg(
                                            "Able to combine simplified template %s with traditional template %s by assuming backwards order"
                                            % (str(prevt), str(t))
                                        )
                        ...
                        for this_text_to_remove, this_repl in text_to_remove:
                            newline, replaced = blib.replace_in_text(
                                line,
                                this_text_to_remove,
                                this_repl,
                                line_pagemsg,
...
parser = blib.create_argparser("Remove redundant Chinese translations")
