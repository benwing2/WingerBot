    ...
    if arg2 and re.search("[一-龯㐀-䶵]", arg2):
        ...
        if arggloss:
            ...
            if arg3 or (
                arg2
                and (
                    re.search("[āōēīūǖáóéíúǘǎǒěǐǔǚàòèìùǜâêîôû̍ⁿ]", arg2)
                    or re.search("[bcdfghjklmnpqrstwz]h?y?[aeiou][aeiou]?[iumnptk]?g?[1-9]", arg2)
                )
            ):
                tr = arg2
                gloss = arg3
...
def find_southern_min_types(p, linkt, linkpage, linkglosses, all_types=False):
    ...
    def find_section_min_types(sectext):
                ...
                if tn in blib.label_templates and getp("1") == "zh":
                    for i in range(2, 30):
                        ...
                        if label in lects_to_codes:
                            ...
                            "(Quanzhou|Chinchew|Choanchew)", label
                        ):  # or Quanzhou dialect or Chinchew dialect or Choanchew dialect
                            ...
                            "(Taiwan[a-z]* Hokkien .* Hakka|Taiwan[a-z]* Hakka .* Hokkien|Taiwan[a-z]* (Southern Min|Min Nan) .* Hakka|Taiwan[a-z]* Hakka .* (Southern Min|Min Nan))",
                            label,
                        ):
        ...
        for defn in defns_matching_gloss:
            ...
            if not lects_seen:
                ...
                return (
                    "Saw multiple definitions with different Southern Min types for %s: defn '%s' has %s while defn '%s' has %s; skipping"
    ...
    if "Etymology 1" in secbody or "Pronunciation 1" in secbody:
        ...
        if not index_of_secbegin:
            return (
                "Something wrong, can't find any Etymology N or Pronunciation N sections in Chinese section for %s"
                % linkmsg
            )
            ...
            for secheader, sectext in etym_pron_sectext:
                ...
                if section_min_types:
                    if not southern_min_types:
                        ...
                        return (
                            "Saw multiple Etymology/Pronunciation sections with different Southern Min types for %s: section %s has %s while section %s has %s; skipping"
                            % (
                                linkmsg,
                                header_for_southern_min_types,
                                ",".join(southern_min_types),
                                secheader,
                                ",".join(section_min_types),
                            )
                        )
        ...
        if not southern_min_types:
            return (
                "Multiple Etymology or Pronunciation sections for %s and couldn't identify any Southern Min lect from scraping page"
                % (linkmsg)
            )
            ...
            for term in terms:
                ...
                if m:
                    ...
                    def append_nan_lang_codes(existing_code, lect_types):
                        ...
                        nan_new_lang_codes = cut_redundant_langcodes(
                        ...
                        if m:
                            ...
                            if new_modifiers != modifiers:
                                ...
                                msg_body = "modifiers %s%s to %s (term %s)" % (
                                    modifiers,
                                    (
                                        " and lang codes %s" % ",".join(before_modified_langcodes)
                                        if before_modified_langcodes
                                        else ""
                                    ),
                                    " and ".join(new_text),
                                    linked_term,
                    ...
                    if modified_langcodes:
                        ...
                        if cut_modified_langcodes != modified_langcodes:
                            notes.append(
                                "remove nan-hbl from derived langcodes %s because saw more specific Hokkien variety (term %s)"
                    ...
                    for t in parsed.filter_templates(recursive=False):
                        ...
                        if not q_t:
                            ...
                            all_linkpage_txt = ",".join(
                                "%s:%s" % (link_term(page), gloss) if gloss else link_term(page)
