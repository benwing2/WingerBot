...
def process_text_on_page_for_label_objects(p):
    langcode = None
    langname = None
    m = re.search("^Module:labels/data/lang/(.*)$", p.title)
    if m:
    ...
    if m:
        ...
        if langcode in lang_data.languages_by_code:
            ...
            p.errandmsg("WARNING: Can't locate language %s" % langcode)
    ...
    for lineno, line in enumerate(lines, start=1):
        ...
        if re.search(r"^labels *\[", line):
            ...
            optimize_category_field(
                "topical_categories", lambda val: True if val == label or val == ucfirst(label) else val
            )
            ...
            optimize_category_field(
                "regional_categories", lambda val: True if val == label or val == ucfirst(label) else val
            )
            regcats = []
                    ...
                    if m:
                        ...
                        for i, split_val in enumerate(split_vals):
                            if i % 2 == 1:
                                ...
                                if val is None:
                                    linemsg(
                                        "WARNING: Internal error: Can't parse value '%s' for field '%s': %s"
                                        % (split_val, fieldname, line)
                                    )
                                    return None
                                vals.append(val)
                            elif (
                                (i == 0 or i == len(split_vals) - 1)
                                and split_val.strip()
                                or (i > 0 and i < len(split_vals) - 1)
                                and split_val.strip() != ","
                            ):
                                linemsg(
                                    "WARNING: Junk '%s' between values for field '%s': %s"
                                    % (split_val, fieldname, line)
                                )
                                return None
                        return vals
    ...
    return ProcessForLabelObjectsRetval(labels_seen, langcode, langname)
        ...
        if isinstance(labelobj, LabelData):
            ...
            for label_line in labelobj.label_lines:
                if type(label_line) is str:
                    ...
                    new_lines.append(
                        "\t%s = %s,%s" % (label_line.field, output_value(fieldval.value), fieldval.comment or "")
                    )
...
def process_text_on_page(p):
    ...
    max_label_index_seen = max(max_label_index_seen, p.index)
    retval = process_text_on_page_for_label_objects(p)
    if retval is None:
        return
    ...
    for labelobj in retval.labels_seen:
        if isinstance(labelobj, LabelData):
            if labelobj.label in labelobjs_by_label:
                p.msg("WARNING: Saw duplicate label '%s'" % labelobj.label)
            else:
            ...
            for alias in labelobj.aliases.cats:
                if alias in labelobjs_by_label:
                    if labelobjs_by_label[alias].label == alias:
                        p.msg("WARNING: Saw alias '%s' of label '%s' duplicating label" % (alias, labelobj.label))
                    else:
                        p.msg(
                            "WARNING: Saw alias '%s' of label '%s' duplicating alias of label '%s'"
                ...
                if member in labelobjs_by_label:
                    ...
                    if bleeding_through_members:
                        ...
                        if lang_specific_obj.label not in canon_labels_with_bleed_through:
                            ...
                            dupmsg = (
                                "WARNING: Alias set members '%s' of regional label '%s' with aliases '%s' are bleeding through with respect to lang-specific label '%s' with aliases '%s'"
                                % (
                                    ",".join(sorted(list(bleeding_through_members))),
                                    labelobj.label,
                                    ",".join(labelobj.aliases.cats),
                                    lang_specific_obj.label,
                                    ",".join(lang_specific_obj.aliases.cats),
                                )
                            )
                            p.msg(dupmsg)
                            lang_specific_obj.lines_after.extend(
                                comment_out(labelobj, dupmsg + "; regional label definition follows:", p.msg)
                            )
                else:
                    not_completely_overridden = True
            if not_completely_overridden and not any_bleed_through:
                if labelobj.label in labelobjs_by_label:
                    p.msg("WARNING: Saw duplicate regional label '%s'" % labelobj.label)
                else:
                    labelobjs_by_label[labelobj.label] = labelobj
                ...
                for alias in labelobj.aliases.cats:
                    if alias in labelobjs_by_label:
                        if labelobjs_by_label[alias].label == alias:
                            p.msg(
                                "WARNING: Saw alias '%s' of regional label '%s' duplicating regional label"
                                % (alias, labelobj.label)
                            )
                        else:
                            p.msg(
                                "WARNING: Saw alias '%s' of regional label '%s' duplicating alias of regional label '%s'"
                                % (alias, labelobj.label, labelobjs_by_label[alias].label)
                            )
                    else:
                        labelobjs_by_label[alias] = labelobj
                ...
                if isinstance(labelobj, LabelData):
                    ...
                    def output_dup_with_regional(alias):
                        if labelobj.label not in canon_labels_duplicating[existing.label]:
                            dupmsg = (
                                "WARNING: {{alt}} label '%s' %s existing regional %s, inserting both commented-out%s"
                                % (
                                    labelobj.label,
                                    "has alias '%s' matching" % alias if alias is not None else "matches",
                                    (
                                        "label '%s'" % match
                                        if match == existing.label
                                        else "alias '%s' of label '%s'" % (match, existing.label)
                                    ),
                                    (
                                        " (also duplicates label(s) '%s')"
                                        % ",".join(sorted(list(canon_labels_duplicating[existing.label])))
                                        if canon_labels_duplicating[existing.label]
                                        else ""
                                    ),
                                )
                            )
                            ...
                            retval.labels_seen.append(comment_out(labelobj, dupmsg, p.msg))
                            retval.labels_seen.append(
                                comment_out(existing, "corresponding regional label follows:", p.msg)
                            )
                    ...
                    if existing is None:
                            ...
                            if existing:
                                ...
                                if labelobj.label not in canon_labels_duplicating[existing.label]:
                                    ...
                                    if existing.label in label_is_regional:
                                        ...
                                        dupmsg = (
                                            "WARNING: {{alt}} label '%s' has alias '%s' matching existing %s, inserting commented-out%s"
                                            % (
                                                labelobj.label,
                                                alias,
                                                (
                                        ...
                                        p.msg(dupmsg)
                                        canon_labels_duplicating[existing.label].add(labelobj.label)
                                        existing.lines_after.extend(comment_out(labelobj, dupmsg, p.msg))
                        if not has_dup_alias:
                            num_new_alt_labels += 1
                            p.msg("Didn't see {{alt}} label '%s', appending" % labelobj.label)
                            if lines_before_label:
                                retval.labels_seen.append(lines_before_label)
                            else:
                                retval.labels_seen.append([""])
                        ...
                        if match not in canon_labels_duplicating[existing.label]:
                            ...
                            # {{lb}} label might share multiple aliases
                            dupmsg = (
                                "WARNING: {{alt}} label '%s' matches existing alias '%s' of label '%s', inserting commented-out%s"
                                % (
                                    labelobj.label,
                                    match,
                                    existing.label,
                        ...
                        if labelobj.aliases.cats:
                            ...
                            existing.aliases.comment = combine_comments(
                                existing.aliases.comment, labelobj.aliases.comment
                            )
                            ...
                            if getattr(labelobj_props, field):
                                if not getattr(existing_props, field):
                                    ...
                                    setattr(
                                        existing_props,
                                        field + "_comment",
                                        combine_comments(existing_comment, labelobj_comment),
                                    )
                                else:
                                    p.msg(
                                        "WARNING: {{alt}} label '%s' has %s value '%s' different from existing '%s'"
                                        % (
                                            labelobj.label,
                                            field,
                                            getattr(labelobj_props, field),
                                            getattr(existing_props, field),
                                        )
                                    )
                                    new_comment = " -- FIXME: {{alt}} label %s value '%s' different from existing" % (
                                        field,
                                        getattr(labelobj_props, field),
                                    )
                                    ...
                                    setattr(
                                        existing_props,
                                        field + "_comment",
                                        combine_comments(existing_comment, new_comment),
                                    )
                        ...
                        if [x for x in lines_before_label if x.strip()]:
                            p.msg(
                                "WARNING: {{alt}} label '%s' has non-empty literal line(s) before it, inserting them raw"
                                % labelobj.label
                            )
                            retval.labels_seen.append(lines_before_label)
                        lines_before_label = []
                        if labelobj.label_lines:
                            p.msg(
                                "WARNING: {{alt}} label '%s' has %s unparsable line(s), inserting them raw"
    ...
    new_lines = output_labels(retval.labels_seen, p.msg)
    text = "\n".join(new_lines)
    text = text.replace("\n\n\n", "\n\n")
    ...
    if num_alt_labels > 0:
        notes.append(
            "incorporate %s {{alt}} label(s) (%s new) from [[Module:%s:Dialects]] into label data module"
