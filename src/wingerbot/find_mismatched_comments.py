...
def process_text_on_page(index, pagetitle, text):
    ...
    for i, (near_text, comment) in enumerate(grouped_text_and_comments):
        ...
        if comment != should_see:
            warnings.append(
                "Saw <nowiki>%s</nowiki> at position %s but expected <nowiki>%s</nowiki>, near text '<nowiki>%s</nowiki>'"
                % (comment, i, should_see, near_text)
            )
            break
        if comment == "<!--" and i == len(grouped_text_and_comments) - 1:
            warnings.append(
                "Saw final unclosed <nowiki>%s</nowiki> at position %s, near text '<nowiki>%s</nowiki>'"
                % (comment, i, near_text)
            )
            break
    if warnings:
