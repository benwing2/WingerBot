...
def process_text_on_page(p):
    ...
    def do_sectext(sectext, do_infl_of):
        ...
        for k in range(1, len(chunks), 2):
            ...
            for t in parsed.filter_templates():
                ...
                if tn == "ca-verb form of":
                    ...
                    for oldparam in oldparams:
                        if t.has(oldparam):
                            ...
                            break
                    if not is_old:
                        p.msg("Saw new-style {{ca-verb form of}}, skipping: %s" % origt)
                        must_continue = True
                        break
                    inf = getp("1")
                ...
                notes.append(
                    "replace %s conjugation(s) for infinitive [[%s]] with {{ca-verb form of|%s}}"
    ...
    for k, header in subsecs.header_list:
        if header == "Verb" and re.search(r"\{\{head\|ca\|verb form[|}]", subsections[k]):
...
parser = blib.create_argparser("Convert {{ca-verb form of}} to new format")
...
blib.do_pagefile_cats_refs(
    ...
    default_refs=["Template:ca-verb form of"],
