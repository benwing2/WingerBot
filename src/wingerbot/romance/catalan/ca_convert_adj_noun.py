...
def make_feminine(base, special=None):
    ...
    if (
        ...
        (re.search(V + "[^ ]*a[lr]$", base) and not re.search(AV + "[^ ]*a[lr]$", base))
        or
    ...
    if (
        re.search(V + "[^ ]*[aiu]t$", mod_base)
        and not re.search(AV + "[^ ]*[aiu]t$", mod_base)
        and not re.search("[aeo][iu]t$", mod_base)
    ) or re.search("[ïü]t$", mod_base):
        return base[:-1] + "da"
...
def process_text_on_page(p):
    ...
    if "ca-adj" not in p.text and "ca-noun" not in p.text:
        return
        ...
        if tn == "ca-noun" and args.do_nouns:
            ...
            for param in t.params:
                ...
                if re.search("_qual$", pn):
                    p.msg("WARNING: Saw _qual parameter, can't handle: %s=%s" % (pn, pv))
                    saw_qual = True
                    break
            if saw_qual:
                continue
            ...
            if not is_plural:
                while True:
                    ...
                    if new_pls == ["+"]:
                        redundant_msg = "redundant plural%s %s" % (
                            "s" if len(pls) > 1 else "",
                            ",".join("'%s'" % pl for pl in pls),
                        )
                    ...
                    for special in romance_utils.all_specials:
                        ...
                        if len(special_pl) > 1:
                            if set(pls) == set(special_pl):
                                ...
                                p.msg(
                                    "WARNING: pls=%s subset of special_pl=%s, replacing with +%s"
            ...
            def handle_mf(g, g_full, make_mf):
                ...
                if mfs:
                    ...
                    if actual_special:
                        subnotes.append(
                            "replace explicit %s '%s' with special indicator '+%s'"
        ...
        if tn == "ca-adj" and args.do_adjs:
            ...
            for param in t.params:
                ...
                if pn == "sp" or pn.startswith("fpl") or pn.endswith("_qual"):
                    p.msg("WARNING: Saw sp=, fpl* or *_qual parameter, can't handle: %s=%s: %s" % (pn, pv, str(t)))
                    saw_unhandlable = True
                    break
                if (pn == "1" or pn.startswith("f") or pn.startswith("pl") or pn.startswith("mpl")) and (
                    pv == "+" or pv.startswith("#")
                ):
                    p.msg("Saw + or #, skipping: %s=%s: %s" % (pn, pv, str(t)))
                    saw_unhandlable = True
                    break
            ...
            origfs = fs
            if fs == ["inv"] or fs == ["ind"]:
                p.msg("Saw invariable adjective, skipping: %s" % str(t))
                saw_unhandlable = True
            ...
            if not saw_unhandlable:
                ...
                if origfs:
                    for f in fs:
                        if f == deff:
                            p.msg("Replacing feminine '%s' with '+'" % f)
                            this_subnotes.append("replace feminine '%s' with '+'" % f)
                            new_fs.append("+")
                        else:
                            new_fs.append(replace_lemma_with_hash(f))
                    if new_fs == ["+"]:
                        redundant_msg = "redundant feminine%s %s" % (
                            "s" if len(origfs) > 1 else "",
                            ",".join("'%s'" % f for f in origfs),
                        )
                        p.msg("Removing %s" % redundant_msg)
                        subnotes.append("remove %s" % redundant_msg)
                        fs = []
                ...
                if mpls:
                    ...
                    if len(defmpl) > 1:
                        if set(mpls) == set(defmpl):
                            ...
                            p.msg(
                                "WARNING: mpls=%s subset of defmpl=%s, replacing with +"
                        ...
                        for mpl in mpls:
                            if mpl == defmpl[0]:
                                p.msg("Replacing masculine plural '%s' with '+'" % mpl)
                                this_subnotes.append("replace masculine plural '%s' with '+'" % mpl)
                                new_mpls.append("+")
                            else:
                                new_mpls.append(replace_lemma_with_hash(mpl))
                    if new_mpls == ["+"]:
                        redundant_msg = "redundant masculine plural%s %s" % (
                            "s" if len(mpls) > 1 else "",
                            ",".join("'%s'" % mpl for mpl in mpls),
                        )
                        p.msg("Removing %s" % redundant_msg)
                        subnotes.append("remove %s" % redundant_msg)
                        mpls = []
                ...
                if pls and defmpl == deffpl:
                    ...
                    if len(defmpl) > 1:
                        if set(pls) == set(defmpl):
                            ...
                            p.msg(
                                "WARNING: pls=%s subset of defmpl=%s, replacing with +"
                        ...
                        for pl in pls:
                            if pl == defmpl[0]:
                                p.msg("Replacing plural '%s' with '+'" % pl)
...
if __name__ == "__main__":
    parser = blib.create_argparser(
        "Remove redundant args in {{ca-noun}} or {{ca-adj}}", include_pagefile=True, include_stdin=True
    )
