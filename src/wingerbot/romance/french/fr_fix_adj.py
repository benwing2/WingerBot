        ...
        if tn == "fr-adj":
            ...
            if inv:
                ...
                if getparam(t, "1") or getparam(t, "f") or getparam(t, "mp") or getparam(t, "fp") or getparam(t, "p"):
                    pagemsg("WARNING: Found extraneous params with inv=: %s" % str(t))
                continue
            ...
            expected_mp = (
                pagetitle
                if re.search("[sx]$", pagetitle)
                else re.sub("al$", "aux", pagetitle) if pagetitle.endswith("al") else pagetitle + "s"
            )
            ...
            expected_fem = (
                ...
                if pagetitle.endswith("e")
                    ...
                    pagetitle + "ne"
                    if pagetitle.endswith("en")
                    else (
                        re.sub("er$", "ère", pagetitle)
                        if pagetitle.endswith("er")
                        else (
                            pagetitle + "le"
                            if pagetitle.endswith("el")
                            else (
                                pagetitle + "ne"
                                if pagetitle.endswith("on")
                                else (
                                    pagetitle + "te"
                                    if pagetitle.endswith("et")
                                    else (
                                        pagetitle + "e"
                                        if pagetitle.endswith("ieur")
                                        else (
                                            re.sub("teur$", "trice", pagetitle)
                                            if pagetitle.endswith("teur")
                                            else (
                                                re.sub("eur$", "euse", pagetitle)
                                                if pagetitle.endswith("eur")
                                                else (
                                                    re.sub("eux$", "euse", pagetitle)
                                                    if pagetitle.endswith("eux")
                                                    else (
                                                        re.sub("if$", "ive", pagetitle)
                                                        if pagetitle.endswith("if")
                                                        else (
                                                            re.sub("c$", "que", pagetitle)
                                                            if pagetitle.endswith("c")
                                                            else pagetitle + "e"
                                                        )
                                                    )
                                                )
            ...
            if not fem.endswith("e"):
                if not getparam(t, "fp"):
                    pagemsg("WARNING: Found f=%s not ending with -e and no fp=: %s" % (fem, str(t)))
                continue
            expected_fp = fem + "s"
            ...
            if getparam(t, "fp") and not getparam(t, "f"):
                pagemsg("WARNING: Found fp=%s and no f=: %s" % (getparam(t, "fp"), str(t)))
                continue
            if getparam(t, "fp") == fem:
                pagemsg("WARNING: Found fp=%s same as fem=%s: %s" % (getparam(t, "fp"), fem, str(t)))
                continue
            ...
            if g == "mf":
                ...
                pagemsg("WARNING: Found unexpected p=%s: %s" % (getparam(t, "p"), str(t)))
            if not re.search("[ -]", pagetitle) and (
                getparam(t, "f") or getparam(t, "mp") or getparam(t, "fp") or getparam(t, "p")
            ):
                pagemsg("Found remaining explicit feminine or plural in single-word base form: %s" % str(t))
        newt = str(t)
        if origt != newt:
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, edit=True, stdin=True, default_cats=["French adjectives"]
)
