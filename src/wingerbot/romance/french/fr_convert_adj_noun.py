        ...
        if tn == "fr-noun" and args.do_nouns:
            ...
            if not pls:
                ...
                if len(defpl) > 1:
                    ...
                    for i, pl in enumerate(pls):
                        if pl == defpl[0]:
                            pls_with_def.append("+")
                            pls_with_def_notes.append(
                                "replace default plural %s with '+' in {{fr-noun}}" % join_with_brackets(defpl)
                            )
                        elif pl == lemma:
                            pls_with_def.append("#")
                ...
                if pls_with_def == ["+"] and mode != ["-"]:
                    notes.append(
                        "remove redundant plural%s %s from {{fr-noun}}"
                    ...
                    notes.append(
                        "replace plural%s %s with +%s in {{fr-noun}}"
            ...
            def handle_mf(mf, mf_full, make_mf):
                ...
                if mfs and not any(x.startswith("+") for x in mfs):
                    ...
                    if actual_special:
                        notes.append(
                            "replace explicit %s %s with special indicator '+%s' in {{fr-noun}}"
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_refs=["Template:fr-noun", "Template:fr-adj"]
)
