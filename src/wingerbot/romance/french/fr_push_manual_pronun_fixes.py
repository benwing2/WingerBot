...
def process_text_on_page(index, pagetitle, text):
    ...
    if "{{a|" in textline:
        pagemsg("WARNING: Replacing %s with %s and saw accent spec on line: %s" % (orig_template, real_repl, textline))

    newtext, did_replace = blib.replace_in_text(text, orig_template, real_repl, pagemsg)
    text = newtext
    if did_replace:
    ...
    if respelling != "y":
        ...
        for t in parsed.filter_templates():
            ...
            if tn == "fr-conj-auto":
                if saw_fr_conj_auto:
                    pagemsg("WARNING: Saw {{fr-conj-auto}} twice, first=%s, second=%s" % (saw_fr_conj_auto, str(t)))
