...
def make_plural(form, new_algorithm, special=None):
    ...
    (
        check("ão$", "ões")
        or check("aõ$", "oens")
        or check("(" + AV + ".*)[ei]l$", r"\1eis")  # final unstressed -el or -il
        or check("el$", "éis")  # final stressed -el
        or check("il$", "is")  # final stressed -il
        or check("(" + AV + ".*)ol$", r"\1ois")  # final unstressed -ol
        or check("ol$", "óis")  # final stressed -ol
        or check("(" + V + ")l$", r"\1is")  # any other vowel + -l
        or check("m$", "ns")  # final -m
        or check("([rz])$", r"\1es")  # final -r or -z
        or check("(" + V + ")$", r"\1s")  # final vowel
        or check("(" + AV + ")s$", lambda m: remove_accent.get(m.group(1), m.group(1)) + "ses")  # final -ês, -ós etc.
        or check("^(" + NAV + "*" + C + "[ui]s)$", r"\1es")  # final stressed -us or -is after consonant
    )
...
def make_masculine(form, special=None):
    ...
    (check("([dts])ora$", r"\1or") or check("a$", "o"))
...
def process_text_on_page(p):
    ...
    if "pt-adj" not in p.text and "pt-noun" not in p.text:
        return
    ...
    for t in parsed.filter_templates():
        ...
        if tn == "pt-noun" and (args.do_nouns or args.do_old_nouns and getp("old")):
            ...
            if not is_plural:
                ...
                if pls_with_def == ["+"]:
                    if pl_quals[0]:
                        if orig_pls:
                            subnotes.append(
                                "replace plural '%s' (same as default) with '+' because qualifier '%s' present"
            ...
            def handle_mf(g, g_full, make_mf):
                ...
                if mfs:
                    ...
                    if actual_special:
                        ...
                        if actual_special:
                            subnotes.append(
                                "replace explicit %s '%s' with special indicator '+%s' and remove explicit %s plural"
                            ...
                            if ok:
                                ...
                                for special in romance_utils.all_specials:
                                    ...
                                    if set(defpl) == set(mfpls):
                                        ...
                                        subnotes.append(
                                            "replace explicit %s plural '%s' with special indicator '+%s'"
            ...
            if args.add_old:
                if origt != str(t):
                    ...
                    p.msg("No changes to %s" % str(realt))

            elif origt != str(t):
                p.msg("Replaced %s with %s" % (origt, str(t)))
                notes.append(
                    "replace {{pt-noun}} with new syntax%s" % (" (%s)" % ", ".join(subnotes) if subnotes else "")
                )
            else:
        ...
        if tn == "pt-noun" and args.do_new_hyphenated_nouns:
                ...
                if mfs:
                    ...
                    if new_mfs != mfs and len(new_mfs) == 1 and actual_special:
                        if is_plural:
                            ...
                            p.msg(
                                "WARNING: Explicit %s=%s matches special=%s and multiple %s plurals %s, allowing"
            ...
            if origt != str(t):
                p.msg("Replaced %s with %s" % (origt, str(t)))
                notes.append(
                    "replace hyphenated {{pt-noun}} with special indicator(s)%s"
        ...
        if tn == "pt-adj" and args.do_adjs:
            ...
            if args.add_old:
                if not t.has("old") and not t.has("1") and not t.has("2"):
                    ...
                    t.add("old", "1")
                    notes.append(
                        "add old=1 to old-style Portuguese adjective template not automatically identifiable as such"
                    )
                    if origt != str(t):
                        p.msg("Replaced %s with %s" % (origt, str(t)))
                continue
            ...
            if base == "-":
                ...
                if not infl_type:
                    ...
                    if not getp("f") and not getp("mpl") and not getp("pl") and not getp("fpl"):
                        p.msg("WARNING: Probable bad template invocation, no parameters: %s" % str(t))
                        continue
                    f = getp("f") or lemma
                    ...
                    fpl = base + fpl
                    if f is None:
                        p.msg("WARNING: Unrecognized inflection type %s: %s" % (infl_type, str(t)))
                        continue
                    lemma = base + infl_type
                    if lemma != p.title:
                        p.msg("WARNING: Saw lemma '%s' not equal to page title: %s" % (lemma, str(t)))
            ...
            if comp and comp not in ["yes", "no", "both"]:
                p.msg("WARNING: Saw unrecognized value '%s' for comp=: %s" % (comp, str(t)))
                continue
        ...
        if args.do_new_adjs_with_inflections:
            if tn == "pt-adj":
                if adj_headword:
                    p.msg(
                        "Saw adjective headword without intervening inflection; first=%s, second=%s"
            ...
            if tn == "pt-adj-infl":
                if not adj_headword:
                    p.msg(
                        "WARNING: Saw adjective inflection template %s without previous adjective headword" % str(t)
                    )
                    continue
                orig_adj_headword = str(adj_headword)
                head = getparam(adj_headword, "head")
                headword_lemma = blib.remove_links(head or p.title)
                infl_base = getp("1")
                infl_type = getp("2")
                infl_lemma = infl_base + (infl_type[:-1] if infl_type in ["co2", "ático2"] else infl_type)
                if infl_lemma != headword_lemma:
                    p.msg(
                        "WARNING: Inflection lemma %s not same as headword lemma %s: adj_headword=%s, infl=%s"
                    ...
                    for stem in stems:
                        if type(stem) is tuple:
                            if stem[0] == "dim_a":
                                ...
                                p.msg(
                                    "WARNING: For %s, separate feminine %s" % (engtype, infl_base + f_stem + f_ending)
                                )
                        else:
                ...
                if orig_adj_headword != str(adj_headword):
                    p.msg("Replaced %s with %s" % (orig_adj_headword, str(adj_headword)))
                else:
                    p.msg("No changes to %s" % str(adj_headword))
                adj_infl_templates_to_remove.append(str(t))

    text = str(parsed)
    for template_to_remove in adj_infl_templates_to_remove:
        text, changed = blib.replace_in_text(
            text,
            "\n\n==+(Inflection|Declension|Conjugation)==+\n%s" % re.escape(template_to_remove),
            "",
            p.msg,
            is_re=True,
        )
        if not changed:
            p.msg("WARNING: Can't remove adjective inflection template %s" % template_to_remove)
        else:
