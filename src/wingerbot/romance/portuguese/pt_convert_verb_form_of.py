...
def process_text_on_page(p):
    ...
    def do_sectext(sectext, do_infl_of):
        tn_re = r"(?:inflection of\|pt|infl of\|pt)" if do_infl_of else "pt-verb[ -]form[ -]of"
        ...
        for k in range(1, len(chunks), 2):
            ...
            if not re.search(r"\A((?:# \{\{%s\|.*\}\}\n)+)\Z" % tn_re, verb_form_chunk):
                ...
                if m:
                    p.msg(
                        "WARNING: Extraneous text after {{%s}}, adding after new {{pt-verb form of}}: <%s>"
            ...
            for t in parsed.filter_templates():
                ...
                if tn == "pt-verb form of":
                    ...
                    for oldparam in oldparams:
                        if t.has(oldparam):
                            ...
                            break
                    if not is_old:
                        p.msg("Saw new-style {{pt-verb form of}}, skipping: %s" % origt)
                        must_continue = True
                        break
                    inf = getp("1")
                    ...
                    for gloss_param in gloss_params:
                        if t.has(gloss_param):
                            p.msg(
                                "WARNING: Saw gloss param %s=%s in {{pt-verb-form-of}}, skipping: %s"
                ...
                notes.append(
                    "replace %s conjugation(s) for infinitive [[%s]] with {{pt-verb form of|%s}}"
...
parser = blib.create_argparser(
    "Convert {{pt-verb form of}} and {{pt-verb-form-of}} to new format", include_pagefile=True, include_stdin=True
)
