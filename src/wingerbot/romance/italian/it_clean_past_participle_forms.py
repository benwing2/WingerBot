        ...
        if header in ["Verb", "Participle"]:
            ...
            for t in parsed.filter_templates():
                ...
                if (
                    ...
                    and getp("1") == "it"
                    and getp("2")
                    in ["verb form", "participle form", "past participle form", "participle", "past participle"]
                    or tn == "it-pp"
                ):
                    maybe_saw_participle = True
                    break
            ...
            try:
                            ...
                            if addltemp and not filtered_tag_sets:
                                ...
                                if new_tags != tags:
                                    infltags.put_back_new_inflection_of_params(
                                        t, this_sec_notes, new_tags, params, lang, term, tr, alt
                                    )
                                ...
                                this_sec_notes.append(
                                    "remove %s from {{%s}} and replace with {{%s}}"
                ...
                for t, lemma in templates_to_remove:
                    ...
                    if did_replace:
                        this_sec_notes.append(
                            "remove past participle defn for reflexive %s because equivalent non-reflexive defn already exists"
                            % lemma
                        )
                ...
                if saw_pp_of:
                    ...
                    if pos in ["verb form", "participle form"]:
                        ...
                        this_sec_notes.append(
                            "convert {{head|it|%s}} to {{head|it|past participle form}} for participle form" % pos
                        )
                        newsubseck = str(parsed)
                    ...
                    if not existing_gender:
                        ...
                        this_sec_notes.append(
                            "add g=%s to {{head|it|past participle form}} for participle form" % should_be_gender
                        )
                        newsubseck = str(parsed)
                    else:
                        head_template.add("g", should_be_gender)
                        this_sec_notes.append(
                            "correct g=%s to g=%s in {{head|it|past participle form}} for participle form"
                                        ...
                                        if tn == "it-noun" and getp("m") or tn == "female equivalent of":
                                            p.msg(
                                                ...
                                                % (
                                                    ...
                                                    existing_section,
                                                    ";".join(blib.find_defns(subsections[k], "it")),
                                                )
                                            )
                                            ok_to_group = True
                                            break
