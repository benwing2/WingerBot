...
def process_text_on_page(p):
    def expand_text(tempcall, suppress_errors):
        return blib.expand_text(tempcall, p.title, p.msg, args.verbose, suppress_errors=suppress_errors)
    ...
    if not re.search(V + ".*" + V + ".*" + V, p.title):
        p.msg("Skipping page without at least three vowels")
        return
    ...
    for t in parsed.filter_templates():
        ...
        if tn in ["it-pr"]:
            p.msg("Saw %s" % str(t))
            if getp("2"):
                p.msg("Skipping multiple pronunciations: %s" % str(t))
                continue
            pron = getp("1")
            if not pron:
                continue
            ...
            if "*" in pron or "!" in pron or "°" in pron:
                p.msg("Skipping pron with initial/final symbol: %s" % str(t))
                continue
            m = re.search("^(.*?)(<.*>)$", pron)
            if m:
                pron, mods = m.groups()
            ...
            if pron == "+":
                ...
                default_phonemic = getpron(p.title, suppress_errors=True)
                ...
                if default_phonemic == pron_phonemic:
                    p.msg(
                        "Respelling '%s' produces phonemic /%s/, same as default: %s" % (pron, pron_phonemic, str(t))
                    )
                    pron = "+"
                else:
                    p.msg(
                        "Respelling '%s' produces phonemic /%s/, while pagename as respelling produces /%s/: %s"
            ...
            if newpron == "+":
                newpron = ""
            if newpron != origpron:
                p.msg("Replacing respelling '%s' with '%s'" % (origpron, newpron))
                if not newpron:
                    rmparam(t, "1")
...
parser = blib.create_argparser("Remove redundant respellings in {{it-pr}}", include_pagefile=True, include_stdin=True)
