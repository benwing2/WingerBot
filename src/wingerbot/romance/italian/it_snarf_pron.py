...
def process_text_on_page(p):
    ...
    def apply_default_pronun_to_pagetitle():
        respellings, this_msgs = apply_default_pronun(p.title)
            ...
            for t in parsed.filter_templates():
                ...
                if tn == "it-IPA":
                    ...
                    if prev_it_IPA_t:
                        ...
                        p.msg(
                            "WARNING: Saw multiple {{it-IPA}} templates in a single Pronunciation section: %s"
                            % " ||| ".join(pronun_lines)
                        )
                        must_continue = True
                        break
                ...
                if tn == "it-pr":
                    ...
                    if prev_it_pr_t:
                        ...
                        p.msg(
                            "WARNING: Saw multiple {{it-pr}} templates in a single Pronunciation section: %s"
                            % " ||| ".join(pronun_lines)
                        )
                        must_continue = True
                        break
    ...
    if not saw_existing_pron:
        if args.include_defns and has_etym_sections:
            ...
            respellings = apply_default_pronun_to_pagetitle()
            p.msg(
                "<respelling> %s: %s <end> %s"
                % ("top" if has_etym_sections else "all", " ".join(respellings), " ".join(msgs))
            )
