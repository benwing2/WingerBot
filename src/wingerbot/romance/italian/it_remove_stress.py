...
def synchronize(term, hyphenation, pagemsg):
    ...
    while i < len(term) and j < len(hyph):
        ...
        if term[i] == " ":
            ...
            continue
        pagemsg(
            "WARNING: Unable to match it-stress term %s against hyphenation %s at it-stress char #%s=%s, hyphenation char #%s=%s"
            % (term, hyph, i, term[i], j, hyph[j])
        )
        return None
    if i < len(term):
        pagemsg(
            "WARNING: Trailing it-stress chars %s in term %s when matching against hyphenation %s"
            % (term[i:], term, hyph)
        )
        return None
    if j < len(hyph):
        pagemsg(
            "WARNING: Trailing hyphenation chars %s in term %s when matching against it-stress %s"
            % (hyph[j:], hyph, term)
        )
        return None
...
if args.stressfile:
    for index, line in blib.iter_items_from_file(args.stressfile, start, end):
        m = re.search(
            r"^\* Page [0-9]+ \[\[(Rhymes:.*?)\]\]: WARNING: Saw it-stress template \{\{temp\|it-stress\|(.*?)\}\}.*$",
            line,
        )
        if m:
