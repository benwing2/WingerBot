            ...
            if tn in ["it-conj", "it-conj-rfc"]:
                ...
                if conjinf is not None:
                    if conjinf.endswith("si") and not inf.endswith("si"):
                        ...
                        pagemsg(
                            "%s: Converting reflexive conjugation '%s' for non-reflexive infinitive to non-reflexive '%s'"
                            % (inf, arg1, newarg1)
                        )
                        arg1 = newarg1
...
def process_text_on_page(p):
    ...
    def do_sectext(sectext, secheadertext):
        ...
        for k in range(1, len(chunks), 2):
            ...
            for t in parsed.filter_templates():
                ...
                if tn == "it-verb form of":
                    p.msg("Saw new-style {{it-verb form of}}, skipping: %s" % origt)
                    must_continue = True
                    break
                ...
                if len(expansions) > 1:
                    p.msg(
                        ...
                        % (
                            old_template_desc,
                            ", ".join(
                                "%s=%s" % (expansion_conjugations[i], expansion)
        ...
        retval_body_included_headword = re.sub(
            r"\A\{\{head\|it\|verb form(?:\|head=[^{}|=]*)?\}\}\n\n*# (\{\{it-verb form of\|.*)\|noheadword=1(\}\}\n*)\Z",
            r"\1\2",
            retval_body,
        )
        if retval_body_included_headword != retval_body:
            p.msg("Removed headword and |noheadword=1 from {{it-verb form of}}")
            retval_body = retval_body_included_headword
            for old_template_desc, inf, conj in this_note_parts:
                notes.append(
                    "replace headword and %s conjugation(s) for infinitive [[%s]] with {{it-verb form of|%s}}"
            ...
            def split_headers(m):
                ...
                verb_form_ofs = [
                    re.sub(r"^# (.*)\|noheadword=1(.*)$", r"\1\2", verb_form_of) for verb_form_of in verb_form_ofs
                ]
                return ("\n\n%s" % secheadertext).join(verb_form_ofs) + "\n" + trailing

            # there may be more than one {{it-verb form of}}; split headers
            retval_body_split_headword = re.sub(
                r"\A\{\{head\|it\|verb form(?:\|head=[^{}|=]*)?\}\}\n\n*((?:# \{\{it-verb form of\|.*\|noheadword=1\}\}\n)+)(\n*)\Z",
                split_headers,
                retval_body,
            )
            if retval_body_split_headword != retval_body:
                p.msg("Split multiple {{it-verb form of}} invocations into separate header sections")
                retval_body = retval_body_split_headword
                for i, (old_template_desc, inf, conj) in enumerate(this_note_parts):
                    notes.append(
                        "replace headword and %s conjugation(s) for infinitive [[%s]] with %s{{it-verb form of|%s}}"
                        % (old_template_desc, inf, "header + " if i > 0 else "", conj)
                    )
            else:
                for old_template_desc, inf, conj in this_note_parts:
                    notes.append(
                        "replace %s conjugation(s) for infinitive [[%s]] with {{it-verb form of|%s|noheadword=1}}"
                        % (old_template_desc, inf, conj)
                    )
...
parser = blib.create_argparser(
    "Convert {{inflection of|it}} to {{it-verb form of}}", include_pagefile=True, include_stdin=True
)
...
blib.do_pagefile_cats_refs(
    ...
    new=True,
    default_cats=["Italian verb forms"],
