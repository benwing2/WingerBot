                ...
                if AC not in ipa and GR not in ipa:
                    ...
                    if vowel_count > 1:
                        new_ipa = re.sub(
                            ...
                            lambda m: m.group(1) + (AC if m.group(1) in "eoɛɔ" else GR) + m.group(2),
                            ipa,
                        )
                        if new_ipa == ipa:
                            ...
                            notes.append(
                                unicodedata.normalize(
                                    "NFC", "add stressed form %s to defaulted {{it-IPA}} pronun" % new_ipa
                                )
                            )
                            ipa = new_ipa
                if "z" in ipa:
                    ...
                    for i in range(1, len(split_z), 2):
                        ...
                        if voiced in ["y", "yes"] or i == 1 and voiced == "1":
                            ...
                            if (
                                split_frobbed_ipa[i] == "z"
                                and re.search(vowel_c + stress_c + "?$", split_frobbed_ipa[i - 1])
                                and re.search("^" + vowel_c, split_frobbed_ipa[i + 1])
                            ):
                                default_voiced = True
                    ...
                    if new_ipa != ipa:
                        notes.append(
                            unicodedata.normalize(
                                "NFC", "convert z to ts or dz in %s -> %s in {{it-IPA}}" % (ipa, new_ipa)
                            )
                        )
                        ipa = new_ipa
