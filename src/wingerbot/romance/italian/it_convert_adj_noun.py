...
def make_plural(form, gender, new_algorithm, special=None):
    retval = romance_utils.handle_multiword(
        form, special, lambda form: make_plural(form, gender, new_algorithm), prepositions
    )
    if retval:
        assert len(retval) == 1
        return retval[0]
...
def process_text_on_page(p):
    ...
    for t in parsed.filter_templates():
        ...
        if tn == "it-noun" and args.do_nouns:
            ...
            autohead = romance_utils.add_links_to_multiword_term(
