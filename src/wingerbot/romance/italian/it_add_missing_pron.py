...
def ipa_to_respelling(ipa):
    ...
    ipa = sub_repeatedly(
        "ˌ([^ ]*?)([aeɛioɔu])(?!\u032f)(.*ˈ)",
        lambda m: m.group(1) + vowel_ipa_to_spelling[m.group(2)] + m.group(3),
        ipa,
    )
    # 0331 = LINEUNDER
    ipa = re.sub(
        "ˌ(.*?)([aeɛioɔu])(?!\u032f)", lambda m: m.group(1) + vowel_ipa_to_spelling[m.group(2)] + "\u0331", ipa
    )
...
def hack_respelling(pagetitle, respelling):
    ...
    if len(pagetitle_words) != len(respelling_words):
        warnings.append(
            "WARNING: Page title has %s words but respelling %s has %s words"
        ...
        for ptw, rw in zip(pagetitle_words, respelling_words):
            ...
            if len(split_ptw) != len(split_rw):
                warnings.append(
                    "WARNING: Different # of c/k/q's in pagetitle word %s vs. c/k/q's in respelling word %s" % (ptw, rw)
                )
            else:
                parts = []
            ...
            if len(split_ptw) != len(split_rw):
                warnings.append(
                    "WARNING: Different # of c(i)e's in pagetitle word %s vs. c(i)e's in respelling word %s" % (ptw, rw)
                )
            else:
                parts = []
                for i in range(len(split_rw)):
            ...
            if len(split_ptw) != len(split_rw):
                warnings.append(
                    "WARNING: Different # of z's in pagetitle word %s vs. ts/dz's in respelling word %s" % (ptw, rw)
                )
            else:
                parts = []
                for i in range(len(split_rw)):
...
def process_text_on_page(p):
                ...
                if tn == "IPA" and getparam(t, "1") == "it":
                    ...
                    for pronun in pronuns:
                        ...
                        for rw in respelling_words:
                            ...
                            if (
                                not re.search("[àèéìòóùÀÈÉÌÒÓÙ]", hacked_rw)
                                and len(re.sub("[^aeiouAEIOU]", "", hacked_rw)) > 1
                            ):
                                set_unable(
                    ...
                    if not unable:
                        ...
                        notes.append(
                            "replace raw {{IPA|it}} with {{it-pr|%s}}%s" % ("|".join(respellings), manual_assist)
                        )
                    pronun_based_respellings.extend(respellings)
                if str(t) != origt:
                    ...
                    for ending, ending_respelling in spellings:
                        if p.title.endswith(ending):
                            ...
                            if ending_respelling == "óso":
                                ...
                                append_respelling(respelling)
                            if (
                                re.search("[aeiouàèéìòóù]s([aeiouàèéìòóù]|$)", prevpart.lower())
                                or not saw_oso_ese
                                and re.search("[aeiouàèéìòóù][sz][aeiouàèéìòóù]", ending_respelling.lower())
                            ):
                                append_warnings(
                                    "WARNING: Unable to add pronunciation due to /s/ or /z/ between vowels: %s" % rhy
                                )
                                unable = True
                                break
                            if "z" in prevpart:
                                append_warnings(
                                    "WARNING: Unable to add pronunciation due to z in part before rhyme: %s" % rhy
                                )
                                unable = True
                                break
                            ...
                            if re.search("[^aeiou][iu]([aeiou]|$)", hacked_prevpart.lower()):
                                append_warnings(
                                    "WARNING: Unable to add pronunciation due to hiatus in part before rhyme %s" % rhy
                                )
                                unable = True
                                break
                            if re.search("[aeiouàèéìòóù]i([^aeiouàèéìòóù]|$)", respelling.lower()):
                                append_warnings(
                                    "WARNING: Unable to add pronunciation due to falling diphthong in -i: %s" % rhy
                                )
                                unable = True
                                break
                            matched = True
                ...
                if rhyme_based_respellings:
                    if not saw_it_pr:
                        ...
                        for respelling in rhyme_based_respellings:
                            if (
                                not re.search("^qual[0-9]*=", respelling)
    ...
    if need_ref_section:
        for k in range(len(subsections) - 1, 2, -2):
            if subsecs.headers[k] == "References":
            ...
            for k in range(len(subsections) - 1, 2, -2):
                if not subsecs.headers[k] in ["Anagrams", "Further reading"]:
...
parser.add_argument(
    "--ipa-direcfile",
    help="File containing IPA respelling directives, modified from lines output with <respelling> ... <end> in them.",
)
parser.add_argument(
    "--rhyme-direcfile",
    help="File containing rhyme respelling directives, modified from lines output with <rhyme-respelling> ... <end> in them.",
)
