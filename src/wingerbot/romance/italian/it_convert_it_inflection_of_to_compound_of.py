      ...
      if page_base.endswith("ndo"):
        if inf.endswith("are") and inf[:-2] + "ndo" == page_base:
          if verb_is_italian(inf):
            ...
            notes.append("convert {{inflection of}} wrongly indicated as a reflexive gerund to {{it-compound of}}")
          else:
            pagemsg("Base infinitive %s doesn't exist, not converting: %s" % (inf, str(t)))
          continue
        if (page_base.endswith("ducendo") and inf.endswith("ure") or page_base.endswith("ponendo") and inf.endswith("ore") or
            page_base.endswith("aendo") and inf.endswith("are")):
          inf = inf[:-2] + "rre"
        ...
        if page_base[:-4] + "ere" != inf and page_base[:-4] + "ire" != inf:
          pagemsg("WARNING: Apparent irregular infinitive %s for gerund %s, verify: %s" % (inf, page_base, str(t)))
        if verb_is_italian(inf):
          origt = str(t)
          del t.params[:]
          blib.set_template_name(t, "it-compound of")
        ...
        if inf.endswith("trare"):
          ...
          if trare_exists and not trarre_exists:
            ...
            pagemsg("WARNING: Both infinitive %s and %s exist, can't handle: %s" % (inf, trarre_inf, str(t)))
            continue
          else:
        ...
        if not need_explicit_inf and re.search("[aeiou]re$", inf) and inf[:-1] == page_base:
          ...
          if verb_is_italian(inf):
            ...
            blib.set_template_name(t, "it-compound of")
            pagemsg("Converting %s to %s" % (origt, str(t)))
            notes.append("convert {{inflection of}} wrongly indicated as a reflexive infinitive to {{it-compound of}}")
          else:
            pagemsg("Base infinitive %s doesn't exist, not converting: %s" % (inf, str(t)))
          continue
        if page_base + "e" != inf:
          pagemsg("WARNING: Apparent irregular infinitive %s for page base infinitive %s, verify: %s" % (inf, page_base, str(t)))
        if verb_is_italian(inf):
          origt = str(t)
          del t.params[:]
          blib.set_template_name(t, "it-compound of")
...
parser = blib.create_argparser("Convert {{inflection of}} for reflexive gerunds/infinitives to {{it-compound of}}",
