...
def process_text_on_page(p):
    ...
    def templatize_masc_fem_line(m):
        mm = re.search(
            r"^(.*)\{\{(it-noun\|.*?)\}\}[(' ]*([Mm]asculine|[Ff]eminine|[Mm]asculine of|[Ff]eminine of|[Ff]emale)[': ]*(\[\[[^\[\]{}\n]*?\]\]|\{\{[lm]\|it\|[^\[\]{}\n]*?\}\})[)' ]*$",
            m.group(0),
        )
        if not mm:
...
parser = blib.create_argparser(
    "Templatize 'masculine/feminine' into {{it-noun}}", include_pagefile=True, include_stdin=True
)
