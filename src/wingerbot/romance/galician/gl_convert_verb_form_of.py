...
def process_text_on_page(p):
    ...
    def do_sectext(sectext, do_infl_of):
        ...
        tn_re = r"(?:inflection of\|gl|infl of\|gl)" if do_infl_of else "gl-verb form of-old"
        ...
        for k in range(1, len(chunks), 2):
            ...
            for t in parsed.filter_templates():
                ...
                notes.append(
                    "replace %s conjugation(s) for infinitive [[%s]] with {{gl-verb form of|%s}}"
    ...
    for k, header in subsecs.header_list:
        if header == "Verb" and re.search(
            r"(\{\{head\|gl\|(verb form|gerund|participle form)|\{\{gl-verb-form)[|}]", subsections[k]
        ):
...
parser = blib.create_argparser(
    "Convert {{gl-verb form of-old}} and {{inflection of}} to new-format {{gl-verb form of}}",
