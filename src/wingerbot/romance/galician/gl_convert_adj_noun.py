...
def make_feminine(term, is_noun, special=None):
    ...
    return (
        tr("o$", "a")
        or tr("º$", "ª")  # ordinal indicator
        or tr("^(" + C + "*)u$", r"\1úa")  # [[nu]] -> núa, [[cru]] -> crúa
        or tr("eu$", "ía")  # [[sandeu]] -> sandía, [[xudeu]] -> xudía
        or
        ...
        is_noun
        and tr("z$", "za")
        or tr("ín$", "ina")  # [[bailarín]], [[benxamín]], [[danzarín]], [[galopín]], [[lampantín]], [[mandarín]],
        or
        ...
        # [[xiprón]]
        tr("ón$", "ona")
        or tr("és$", "esa")  # [[francés]], [[portugués]], [[fregués]], [[vigués]] etc.
        or
...
def make_plural(term, special=None):
    ...
    return (
        tr("r$", "res")
        or tr("z$", "ces")
        or tr("(" + V + "be)l$", r"\1is")  # vowel + -bel
        or tr("(" + AV + ".*" + V + "l)$", r"\1es")  # non-final stress + -l e.g. [[túnel]] -> 'túneles'
        or tr("^(" + C + "*" + V + C + "*l)$", r"\1es")  # monosyllable ending in -l e.g. [[sol]] -> 'soles'
        or tr("il$", "ís")  # final stressed -il e.g. [[civil]] -> 'civís'
        or tr("(" + V + ")l$", r"\1is")  # any other vowel + -l e.g. [[papel]] -> 'papeis'
        or tr("(" + V + "[íú])s$", r"\1ses")  # vowel + stressed í/ú + -s e.g. [[país]] -> 'países'
        or tr(
            "(" + AV + ")s$",  # other final accented vowel + -s e.g. [[autobús]] -> 'autobuses'
            lambda m: remove_accent[m.group(1)] + "ses",
        )
        or tr("(" + V + "[iu]?s)$", r"\1es")  # diphthong + final -s e.g. [[deus]] -> 'deuses'
        or tr("^(C" + "*" + V + "s)$", r"\1es")  # monosyllable + final -s e.g. [[fros]] -> 'froses', [[gas]] -> 'gases'
        or tr("([sx])$", r"\1")  # other final -s or -x (stressed on penult or antepenult or ending in cluster), e.g.
        or
...
if __name__ == "__main__":
    parser = blib.create_argparser(
        "Convert {{gl-noun-old}} or {{gl-adj-old}} to {{gl-noun}} or {{gl-adj}} and remove redundant args",
    )
