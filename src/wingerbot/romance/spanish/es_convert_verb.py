...
def get_def_forms(lemma, prep, pagemsg):
    ...
    if m:
        ...
        def_pres_ue = (
            last_vowel == "o"
            and before_last_vowel.endswith("g")
            and before_last_vowel + "üe" + after_last_vowel + "o"
            or last_vowel in ["o", "u"]
            and before_last_vowel + "ue" + after_last_vowel + "o"
            or None
        )
        def_pres_i = last_vowel == "e" and before_last_vowel + "i" + after_last_vowel + "o" or None
        def_pres_iacc = (
            (last_vowel == "e" or last_vowel == "i") and before_last_vowel + "í" + after_last_vowel + "o" or None
        )
    ...
    if refl and clitic:
        ret["linked_verb"] = (
            verb == ret["accented_verb"] and "[[" + verb + "]]" or "[[" + verb + "|" + ret.accented_verb + "]]"
        )
                    ...
                    if x == d["pres"]
                        ...
                        if x == d["pres_ie"]
                            ...
                            if x == d["pres_ue"]
                                ...
                                "+i"
                                if x == d["pres_i"]
                                else "+í" if x == d["pres_iacc"] else "+ú" if x == d["pres_uacc"] else x
                            )
                        )
                    )
                )
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_cats=["Spanish verbs"]
)
