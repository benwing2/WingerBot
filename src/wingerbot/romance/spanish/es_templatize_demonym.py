...
def process_text_on_page(p):
    ...
    for k, header in subsecs.header_list:
        def raw_toponym_to_toponym(raw_toponym):
            ...
            toponym_and_country_state_re = r"^(.*?)(?:,| in) (%s(?:, %s)*)$" % (
                possibly_bracketed_toponym_re,
                possibly_bracketed_toponym_re,
            )
        ...
        if header == "Adjective" and sectail_is_demonym:
            ...
            subsections[k] = re.sub(
                r"^# *(?:\{\{lb\|es\|relational\}\} *)?%s (.*)$" % of_or_from_re, replace_of, subsections[k], 0, re.M
            )
            ...
            subsections[k] = re.sub(
                r"^# *\[\[([^\[\]|={}]+)\]\] \{\{%s\|%s (.*)\}\}$" % (gloss_qual_re, of_or_from_re),
                replace_of_with_raw_demonym,
                subsections[k],
                0,
                re.M,
            )
        ...
        if header == "Noun":
            ...
            if demonym_gender is not None:
                someone_from_re = (
                    "(?:(?:someone|(?:a )?person) from|(?:an? )?(?:native or )?(?:resident|inhabitant) (?:of|from))"
                )
                ...
                subsections[k] = re.sub(
                    r"^# *%s (.*)$" % someone_from_re,
                    lambda m: replace_someone_from(m, demonym_gender),
                    subsections[k],
                    0,
                    re.M,
                )
                ...
                subsections[k] = re.sub(
                    r"^# *\[\[([^\[\]|={}]+)\]\] \{\{%s\|%s (.*)\}\}$" % (gloss_qual_re, someone_from_re),
                    lambda m: replace_native_or_resident_with_raw_demonym(m, demonym_gender),
                    subsections[k],
                    0,
                    re.M,
                )
    ...
    if sectail_is_demonym and need_to_remove_cat[0]:
        ...
        if newsectail != sectail:
            ...
            sectail = newsectail
        newsectail = re.sub(
            r"(\{\{(?:C|c|top|topic|topics|catlangcode)\|es(?:\|[^{}=|]*)?)\|Demonyms([|}])", r"\1\2", sectail
        )
        if newsectail != sectail:
...
parser = blib.create_argparser("Templatize Spanish demonyms")
