...
from wingerbot.lang_utils import AC, GR, CFLEX, TILDE, DIA
                        ...
                        for style in styles:
                            ...
                            if rhyme == rhyme_pronuns[style]:
                                ...
                                if nsyl is None:
                                    ...
                                    extra_notes.append(
                                        "ignore known-incorrect syllable count %s in {{%s}}" % (nsyl, tname(rhymet))
                                    )
                                    break
                            ...
                            if matching_styles:
                                p.msg(
                                    ...
                                    % (
                                        bare_arg,
                                        rhyme,
                                        ",".join(matching_styles),
                                        nsyl,
                                        ", ".join("%s=%s" % (style, rhyme_nsyl[style]) for style in styles),
                                        str(rhymet),
                                    )
                                )
                            else:
                                p.msg(
                                    ...
                                    % (
                                        ...
                                        ", ".join("%s=%s" % (style, rhyme_pronuns[style]) for style in styles),
                                        str(rhymet),
                                    )
                                )
                            must_continue = True
                        ...
                        if getparam(hmpt, "1") != "es":
                            ...
                            for param in hmpt.params:
                                ...
                                def normalize_homophone_qualifier(q):
                                    q = re.sub(
                                        r"(non-Castilian|non-Iberian|seseo and ceceo|seseo|Latin-America|in dialects without distinción|in dialects without distinction between S and Z|non-\[*distinción\]*)",
                                        "Latin America",
                                        q,
                                    )
                                    q = re.sub(r"((?:in dialects with )?\[*yeísmo\]*)", "[[yeísmo]]", q)
                                    q = re.sub(" dialects", "", q)
                                    return q
                                ...
                                for pn, pv in sorted(homophones.items()):
                                    ...
                                    if pn in homophone_qualifiers:
                                        hmp_args[-1] += "<q:%s>" % normalize_homophone_qualifier(
