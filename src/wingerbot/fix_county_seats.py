    ...
    for t in parsed.filter_templates():
        ...
        if tn == "place":
            ...
            if (
                ...
                and len(runs[1]) == 2
                and (
                    runs[1][0] in ["county seat", "parish seat", "borough seat"]
                    and re.search(r"^(co|par|bor)/", runs[1][1])
                )
            ):
                if (
                    len(runs[0]) >= 4
                    and runs[0][1].startswith("co/")
                    and runs[0][2] == "and"
                    and runs[0][3].startswith("co/")
                ):
                    numargs = (
                        [numargs[0], "%s/%s" % (runs[0][0], runs[1][0]), runs[1][1]]
                        + runs[0][4:]
                        + ["located in"]
                        + runs[0][1:4]
                    )
                ...
                pagemsg("<from> %s <to> {{place|%s%s}} <end>" % (str(t), "|".join(numargs), namedargs))
            else:
