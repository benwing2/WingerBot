        ...
        if tn in ["wikipedia", "wp", "slim-wikipedia", "swp"]:
            ...
            if not (cat or cat2) and ":" in link:
                ...
                if (
                    linkpref == "Category"
                    or lang
                    and linkpref.lower() == lang_category_namespaces.get(lang[:-1], "Category").lower()
                ):
                    cat = rest
                    link = link_label
                    link_label = None
            ...
            def make_link(link, label, fragment=None):
                ...
                if (
                    label
                    and not fragment
                    and (label + " (disambiguation)" == link or label + " (disambiguation)" == origlink)
                ):
...
parser = blib.create_argparser("Convert Wikipedia templates to new syntax", include_pagefile=True, include_stdin=True)
