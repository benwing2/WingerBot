            ...
            if lang != "en":
                if aval in ["A", "a", "An", "an"]:
                    ...
                    if m:
                        ...
                        notes.append(
                            "change explicit article A=%s to lowercase in non-English {{%s|%s}}" % (aval, tn, lang)
                        )
                        aval = "a" + aval[1:]
                        t.add("A", aval)
...
parser = blib.create_argparser(
    "Do basic clean up of {{surname}} and {{given name}}, removing redundant articles and final periods",
