...
def do_canon_param(obj, translit_module):
    ...
    if latin:
        try:
            canonforeign, canonlatin, match_canon_partial_failure_error, partial_success = translit_module.tr_matching(
                obj, foreign, latin, err=True, msgfun=pagemsg
            )
            if match_canon_partial_failure_error:
                if partial_success:
                    match_canon_error = "Partially unable to match-canon %s (%s) with multiple translits: %s" % (
                        foreign,
                        latin,
                        match_canon_partial_failure_error,
                    )
                    total_num_succeeded += 1
                else:
                    match_canon_error = "Unable to match-canon %s (%s) with multiple translits: %s" % (
                        foreign,
                        latin,
                        match_canon_partial_failure_error,
                    )
            ...
            total_num_failed += 1
            canonlatin, canonforeign = translit_module.canonicalize_latin_foreign(obj, latin, foreign, msgfun=pagemsg)
    else:
        _, canonforeign = translit_module.canonicalize_latin_foreign(obj, None, foreign, msgfun=pagemsg)
    ...
    try:
        ...
        if translit is NotImplemented:
            ...
            pagemsg("NOTE: Unable to auto-translit %s (canoned from %s): %s" % (canonforeign, foreign, str(obj.t)))
    except Exception as e:
        pagemsg("NOTE: Unable to transliterate %s (canoned from %s): %s: %s" % (canonforeign, foreign, e, str(obj.t)))
        translit = None
    ...
    if canonforeign == foreign:
        ...
        if fromparam == toparam:
            actions.append(
                "%s %s=%s -> %s in {{%s}}"
                % (actionop, fromparam, foreign, canonforeign, template_changelog_name(obj.t, obj.tlang))
            )
        else:
            actions.append(
                "%s %s=%s -> %s=%s in {{%s}}"
                % (actionop, fromparam, foreign, toparam, canonforeign, template_changelog_name(obj.t, obj.tlang))
            )
        ...
        if rdcanonforeign != rdforeign:
            ...
            pagemsg(
                "NOTE: Without diacritics, old foreign %s different from canon %s%s: %s"
    ...
    if not latin:
        ...
        pagemsg("Removing redundant translit for %s -> %s%s" % (foreign, newforeign, latintrtext))
        ...
        if translit and translit is not NotImplemented:
            pagemsg(
                "NOTE: Canoned Latin %s not same as auto-translit %s, can't remove: %s"
                % (canonlatin, translit, str(obj.t))
            )
        if canonlatin == latin:
            pagemsg(
                "No change in Latin %s: foreign %s -> %s%s"
            ...
            pagemsg(
                ...
                % (
                    ...
                    foreign,
                    newforeign,
                    "" if translit is NotImplemented else " (auto-translit %s)" % translit,
                )
            )
            if not match_canon:
                match_canon_error = match_canon_error or "NO ERROR"
            ...
            actions.append(
                "%s %s=%s -> %s in {{%s}}"
                % (actionop, paramtr, latin, canonlatin, template_changelog_name(obj.t, obj.tlang))
            )
...
def canon_param(obj, translit_module):
    ...
    if obj.param[0] in ["separate", "separate-pagetitle"]:
        ...
        if canonforeign:
            if toparam is None:
                pagemsg("WARNING: No param to set new foreign value `%s` to in %s" % (canonforeign, str(obj.t)))
            else:
...
def canon_one_page_links(
    ...
    text, actions = blib.process_one_page_links(
        index, pagetitle, text, [lang], process_param, templates_seen, templates_changed
    )
