...
recognized_named_params_list = (
    recognized_named_params_everywhere_list + recognized_named_single_per_template_params_list
)
...
params_with_inline_modifiers_single_everywhere_list = process_inline_modifier_list(
    params_with_inline_modifiers_single_everywhere_list, 1
)
...
params_with_inline_modifiers_by_template = {
    ...
    "quote-song": ["title_series", "title_seriesvolume", "album", "work"],
...
params_with_inline_modifiers_list = (
    params_with_inline_modifiers_everywhere_list + inline_modifiers_per_template_params_list
)
...
def normalize_and_split_on_balanced_delims(author, authparam, pagemsg):
    ...
    try:
        return blib.parse_multi_delimiter_balanced_segment_run(
            processed_author,
            [
                (r"[\[%s]" % TEMP_LBRAC, r"[\]%s]" % TEMP_RBRAC),
                (r"\(", r"\)"),
                (r"\{", r"\}"),
    ...
    for t in parsed.filter_templates():
                ...
                if t.has(fr):
                    ...
                    for to_check in all_to_check:
                        ...
                        if existing_val:
                            ...
                            from_to(
                                "WARNING: Would replace %s=%s -> %s= but %s=%s is already present"
                    ...
                    if oldval != newval:
                        ...
                        this_notes.append("move %s=%s to %s=%s in {{%s}}" % (fr, oldval, to, newval, tn))
                    else:
                        rmparam(t, to)  # in case of blank param
                        # See comment above.
                        if re.search("^[0-9]+$", fr) or re.search("^[0-9]+$", to):
        ...
        if tn in quote_templates:
            if args.check_unhandled_params:
                ...
                for param in t.params:
                    ...
                    if re.search("^[0-9]+$", pn):
                        ...
                        pn in recognized_named_single_per_template_params
                        and tn in recognized_named_single_per_template_params[pn]
                    ):
                    ...
                    if "://" not in check_pv and (
                        re.search("(^|[^a-z])%s *=" % recognized_named_params_re_1_to_3, check_pv)
                        or re.search("%s *=" % recognized_named_params_re_4_or_more, check_pv)
                    ):
            ...
            if args.check_compound_pages:
                page = (
                    getp("page")
                    or tn in quote_templates_by_page_param
                    and getp(quote_templates_by_page_param[tn])
                    or ""
                )
                page = page.strip()
                if page:
            ...
            if args.split_authors:
                    ...
                    for author_param in [
                        ...
                        if m:
                            for tlrparam in params_to_check:
                                ...
                                if current_tlr:
                                    pagemsg(
                                        "WARNING: Moving %s in %s=%s, already saw %s=%s, can't move"
                                ...
                                if saw_comma and not saw_semicolon:
                                    pagemsg(
                                        "WARNING: Moving %s in %s=%s, saw comma and no semicolon, not sure how to partition authors from %s"
                                    ...
                                    if author_param.startswith("author") and getp("authorlink" + author_param[6:]):
                                        ...
                                        if "{{" in tlr or "[[" in tlr:
                                            pagemsg(
                                                "WARNING: Would move %s in %s=%s, but saw authorlink%s=%s and brackets or braces in %s"
                                    ...
                                    moved_tlr_notes_msg = "move %s=%s to %s=%s in %s" % (
                                        author_param,
                                        author,
                                        dest_param,
                                        tlr,
                                    ...
                                    pagemsg(
                                        "WARNING: Would split %s off of %s=%s, but saw authorlink%s=%s"
                                        % (desc, author_param, author, author_param[6:], getp("authorlink"))
                                    )
                                else:
                                    # Extract translator(s) after last semicolon
                                    split_runs = blib.split_alternating_runs(
                                        tlr_runs, r"\s*;\s+", preserve_splitchar=True
                                    )
                                    new_tlrs = undo_html_entity_replacement("".join(split_runs[-1]))
                                    new_authors = undo_html_entity_replacement(
                                        "".join("".join(split_run) for split_run in split_runs[:-2])
                                    )
                                    moved_author_param = author_param
                                    moved_tlr_dest_param = dest_param
                ...
                def do_author_param(author, authparam, rearrange_only=False):
                    ...
                    suffix_re = (
                        r"(I+|IV|V|VI+|(Jr|Sr|Esq|Jun|Sen)\.?|(M|J|D|Ph|PH|Ll|LL)[. ]*D[. ]*|[BM][. ]*[AS][. ]*)"
                    )
                    deny_list_words = [
                        "United",
                        "States",
                    ...
                    if et_al_author != author:
                        pagemsg(
                            "Splitting %s=%s: Normalized 'et al' variant to '%s'" % (authparam, author, et_al_author)
                        )
                        author = et_al_author
                    ...
                    if serial_comma_author != author:
                        pagemsg(
                            "Splitting %s=%s: Normalized serial comma to '%s'"
                            % (authparam, author, serial_comma_author)
                        )
                    ...
                    if missing_space_comma_author != author:
                        pagemsg(
                            "Splitting %s=%s: Normalized missing space after comma to '%s'"
                    ...
                    if ampersand_comma_author != author:
                        pagemsg(
                            "Splitting %s=%s: Normalized ampersand-comma to '%s'"
                            % (authparam, author, ampersand_comma_author)
                        )
                    ...
                    if missing_space_ampersand_author != author:
                        pagemsg(
                            "Splitting %s=%s: Normalized missing space after ampersand to '%s'"
                            ...
                            if re.search(", *" + suffix_re + "$", author_runs[0]):
                                ...
                                for deny_list_word in deny_list_words:
                                    if re.search(r"\b%s\b" % deny_list_word, author_runs[0]):
                                        pagemsg(
                                            "%s: Saw deny-listed word '%s', won't try to rearrange"
                                            % (msgpref, deny_list_word)
                                        )
                                        break
                                else:  # no break
                                    rearranged_author = undo_html_entity_replacement(m.group(2) + " " + m.group(1))
                                    pagemsg(
                                        "%s: Rearranged '%s' to '%s' (VERIFY)"
                                        % (msgpref, author_runs[0], rearranged_author)
                                    )
                                    notes_msg = "rearrange LAST, FIRST in %s= in {{%s}} into FIRST LAST" % (
                                        authparam,
                                        tn,
                                    )
                                    return (
                                        "%s: Would rearrange into %s (VERIFY)" % (msgpref, rearranged_author),
                                        notes_msg,
                                        rearranged_author,
                                    )
                    ...
                    if "et al" in author_runs[-1]:
                        ...
                        if not re.search(et_al_re + "$", author_runs[-1]):
                            ...
                            return "WARNING: %s: Saw 'et al.' followed by junk, won't split: %s" % (
                                msgpref,
                                author_runs[-1],
                            )
                        ...
                        if i % 2 == 0:
                            ...
                            if run != "et al.":
                                ...
                                if not run[0].isupper() and not re.search(r"^[{\[]", run):
                                    return (
                                        "%s: Saw run '%s' not beginning with uppercase, link or template, won't split"
                                        % (msgpref, run)
                                    )
                                if (
                                    not re.search(r"\s", run)
                                    and not re.search(r"^\{\{w\|.*\}\}$", run)
                                    and not re.search(r"^\[\[w:.*\]\]$", run)
                                    and not re.search(r"^\{\{lang\|.*\}\}$", run)
                                ):
                                ...
                                for j, word in enumerate(recombined_space_split_runs):
                                    if (
                                        ...
                                        and not word.startswith("al-")
                                        and not re.search("^([dl][aeo]s?|v[oa]n|vom|te[rn]?|de[lnr]|di|du|à|y)$", word)
                                    ):
                                        return (
                                            "%s: Saw run '%s' with non-allow-listed lowercase word '%s' (index %s) in it, won't split"
                                            % (msgpref, run, word, j)
                                        )
                                for deny_list_word in deny_list_words:
                                    if re.search(r"\b%s\b" % deny_list_word, run):
                                        return "%s: Saw run '%s' with deny-listed word '%s', won't split" % (
                                            msgpref,
                                            run,
                                            deny_list_word,
                                        )
                    ...
                    if authparam == "author":
                        ...
                        if len(authors) > 1 and (author2 or last2):
                            ...
                            return "WARNING: %s: Would normally split into %s, but saw %s" % (
                                msgpref,
                                split_authors,
                                msgauth2,
                            )
                    ...
                    if origauthor == semicolon_joined_authors:
                        pagemsg(
                            "%s: Would normally split into %s, but rejoined value is same as current"
                ...
                if splitmsgs:
                    pagemsg(
                        "%s: <from> %s <to> %s <end> <comment> %s <endcom>"
            ...
            if args.combine_authors:
                ...
                for i in range(1, maxind + 1):
                    ...
                    if author:
                        if "<" in author and (trans_author or authorlink or trans_authorlink):
                            ...
                            from_to(
                                "WARNING: Saw inline modifier in author%s=%s along with %s"
                                % (ind, author, ",".join(extra_vals)),
                                str(t),
                            )
                            break
                        else:
                    ...
                    if authors:
                        if t.has("author"):
                            ...
                            from_to(
                                "WARNING: Didn't see either author= or last=, not sure where to put new author=%s"
                                % authors,
                                str(t),
                            )
                            t.add("author", authors)
                    ...
                    if str(t) != t_right_now:
                        this_template_notes.append(
                            "consolidate author=/author2=/first=/last=/etc. into author= in {{%s}}" % tn
                        )
                ...
                if author:
                    if "<" in author and authorlink:
                        from_to(
                            "WARNING: Saw inline modifier in 2ndauthor=%s along with 2ndauthorlink=%s"
                            % (author, authorlink),
                            str(t),
                        )
                        continue
                ...
                if author:
                    ...
                    if t.has("2ndauthor"):
                        ...
                        from_to(
                            "WARNING: Didn't see either 2ndauthor= or 2ndlast=, not sure where to put new 2ndauthor=%s"
                            % author,
                            str(t),
                        )
                        t.add("2ndauthor", author)
                    ...
                    if str(t) != t_right_now:
                        this_template_notes.append(
                            "consolidate 2ndauthor=/2ndfirst=/2ndlast=/etc. into 2ndauthor= in {{%s}}" % tn
                        )
        ...
        if args.rename_bad_params:
            if tn in quote_templates:
                ...
                params_to_remove = ["indent2"]
                params_to_remove_if_blank = ["archiveorg"]
        ...
        if args.remove_wrapping:
            if tn in quote_templates:
                ...
                def convert_lang_like(pn, pv):
                    ...
                    m = re.search(
                        r"^\[\[ *(?:w|wikipedia) *:(?:%s:)?%s\]\]$"
                        % (langcode_with_spaces_re, allowable_char_no_brackets_param_with_spaces_re),
                        pv,
                    )
                    if not m:
                        m = re.search(
                    ...
                    if m:
                        ...
                        if langcode:
                            ...
                            return (
                                newval,
                                "convert raw foreign Wikipedia link in %s= in {{%s}} to w:%s:..." % (pn, tn, langcode),
                                False,
                            )
                        else:
                            newval = "w:%s" % dest
                    ...
                    if m:
                        ...
                        if langcode:
                            ...
                            return (
                                newval,
                                "convert raw foreign Wikisource link in %s= in {{%s}} to w:%s:..." % (pn, tn, langcode),
                                False,
                            )
                        else:
                            newval = "s:%s" % dest
                    ...
                    m = re.search(
                        r"^\{\{ *w *\| *lang *=%s\|%s\}\}$"
                        % (langcode_with_spaces_re, allowable_char_param_with_spaces_re),
                        pv,
                    )
                    if m:
                        langcode, dest = m.groups()
                    ...
                    if langcode:
                        ...
                        return (
                            newval,
                            "convert templatized foreign Wikipedia link in %s= in {{%s}} to w:%s:..."
                            % (pn, tn, langcode),
                            False,
                        )

                    m = re.search(
                        r"^\{\{ *lw *\|%s\|%s\}\}$" % (langcode_with_spaces_re, allowable_char_param_with_spaces_re), pv
                    )
                    if m:
                        langcode, dest = m.groups()
                        ...
                        return (
                            newval,
                            "convert {{lw|...}} foreign Wikipedia link in %s= in {{%s}} to lw:%s:..."
                            % (pn, tn, langcode),
                            False,
                        )
                    ...
                    if m:
                        ...
                        pagemsg(
                            "Convert {{lj|...}} Japanese pseudo-Ruby link %s=%s to %s: %s" % (pn, pv, newval, origt)
                        )
                        return (
                            newval,
                            "convert {{lj|...}} Japanese pseudo-Ruby link in %s= in {{%s}} to ja:..." % (pn, tn),
                            False,
                        )
                    ...
                    if m:
                        ...
                        pagemsg(
                            "Convert {{wj|...}} Japanese Wikipedia pseudo-Ruby link %s=%s to %s: %s"
                            % (pn, pv, newval, origt)
                        )
                        return (
                            newval,
                            "convert {{lj|...}} Japanese Wikipedia pseudo-Ruby link in %s= in {{%s}} to ja:..."
                            % (pn, tn),
                            False,
                        )

                    if re.search(
                        r"\{\{ *lang *\|%s\|%s\}\}" % (langcode_with_spaces_re, allowable_char_param_with_spaces_re), pv
                    ):
                        m = re.search(
                            r"^\{\{ *lang *\|%s\|%s\}\}$"
                            % (langcode_with_spaces_re, allowable_char_param_with_spaces_re),
                        ...
                        if m:
                            ...
                            return (
                                newval,
                                "convert {{lang}} wrapping in %s= in {{%s}} to %s:..." % (pn, tn, langcode),
                                False,
                            )
                            ...
                            if not m:
                                m = re.search(
                                    r"^'' *([^{}]+?) *''\s+\[? *\{\{ *lang *\| *(ko) *\|%s\}\} *\]?$"
                                    % (allowable_char_param_with_spaces_re),
                                    pv.strip(),
                                )
                                if m:
                            ...
                            if gloss or tr:
                                ...
                                if gloss:
                                    ...
                                    if (
                                        lang in ["ar", "ur", "fa", "pa"]
                                        and (
                                            re.search("([ʾʿāīūṣṭẓḍḥǧġṯḵḏśĀĪŪṢṬẒḌḤǦĠŠṮḴḎŚʔʕ]|t̤|n̲|T̤|N̲)", cleaned_gloss)
                                            or re.search("\bal-", cleaned_gloss)
                                        )
                                        or lang == "fa"
                                        and (re.search("[âÂčČ]", cleaned_gloss) or re.search("-e\b", cleaned_gloss))
                                    ):
                                        pagemsg(
                                            "Assuming gloss '%s' is transliteration in %s=%s: %s"
                                            % (gloss, pn, pv, origt)
                                        )
                                        tr = gloss
                                        gloss = None
                                if tr:
                                    ...
                                    t.add(pn, newval)
                                    pagemsg(
                                        "Strip lang wrapping with transliteration %s=%s to %s (VERIFY): %s"
                                        % (pn, pv, newval, origt)
                                    )
                                    retmsg = "convert {{lang}} wrapping in %s= in {{%s}} to %s:...<tr:...>" % (
                                        pn,
                                        tn,
                                        lang,
                                    )
                                    ...
                                    t.add(pn, newval)
                                    pagemsg(
                                        "Strip lang wrapping with translation %s=%s to %s (VERIFY): %s"
                                        % (pn, pv, newval, origt)
                                    )
                                    retmsg = "convert {{lang}} wrapping in %s= in {{%s}} to %s:...<t:...>" % (
                                        pn,
                                        tn,
                                        lang,
                                    )
                ...
                for param in t.params:
                    ...
                    if pn in ["year", "year2"]:
                        ...
                        if m:
                            ...
                            t.add(pn, m.group(1))
                            this_template_notes.append(
                                "remove extraneous boldface in %s=%s in {{%s}}" % (pn, pv.strip(), tn)
                            )
                            continue
                        if pv.startswith("'''"):
        ...
        if args.check_bad_year:
            if tn in quote_templates:
                for param in t.params:
                    ...
                    pv = str(param.value).strip()
                    if pv and (
                        pn in ["year", "start_year", "origyear", "year published"]
                        or pn == "2"
                        and tn in ["quote-book", "quote-journal"]
                    ):
                        cleaned_pv = re.sub("<sup>(.*?)</sup>", r"\1", pv)
                        if (
                            not re.search("[0-9][0-9][0-9]", cleaned_pv)
                            and not re.search("[0-9]+(st|nd|rd|th)", cleaned_pv)
                            and not re.search(
                                r"[0-9] (BC|BCE|AD|CE|\{\{(BC|BCE|AD|CE|B\.C\.|B\.C\.E\.|A\.D\.|C\.E\.)\}\})",
                                cleaned_pv,
                            )
                        ):
        ...
        if args.templates_to_rename_numbered_params and tn in args.templates_to_rename_numbered_params:
            ...
            if tn in quote_templates:
                ...
                for param in t.params:
                    ...
                    pv = str(param.value)
                    if (
                        (re.search("^[0-9]+$", pn) or pn not in recognized_named_params)
                        and pn not in unrecognized_after_url_named_params
                        and pv.strip()
                        and last_url_like
                        and not (pn in ["6", "7"] and re.search("^[0-9]+$", pv))
                    ):
                        pagemsg(
                            "WARNING: Possible unescaped vertical bar, saw %s=%s after %s=%s: %s"
            ...
            if tn == "quote-av":
                ...
                if origt != str(t):
                    this_template_notes.append(
                        "rename numbered params in {{quote-av}} to named params and remove blank ones"
                    )
            if tn == "quote-book":
                move_params(
                    [
                ...
                if origt != str(t):
                    this_template_notes.append(
                        "rename numbered params in {{quote-book}} to named params and remove blank ones"
                    )
            if tn == "quote-hansard":
                move_params(
                    [
                ...
                if origt != str(t):
                    this_template_notes.append(
                        "rename numbered params in {{quote-hansard}} to named params and remove blank ones"
                    )
            if tn == "quote-journal":
                move_params(
                    [
                ...
                if origt != str(t):
                    this_template_notes.append(
                        "rename numbered params in {{quote-journal}} to named params and remove blank ones"
                    )
            if tn == "quote-mailing list":
                ...
                if origt != str(t):
                    this_template_notes.append(
                        "rename numbered params in {{quote-mailing list}} to named params and remove blank ones"
                    )
            if tn == "quote-newsgroup":
                ...
                if origt != str(t):
                    this_template_notes.append(
                        "rename numbered params in {{quote-newsgroup}} to named params and remove blank ones"
                    )
            if tn == "quote-song":
                move_params(
                    [
                ...
                if origt != str(t):
                    this_template_notes.append(
                        "rename numbered params in {{quote-song}} to named params and remove blank ones"
                    )
            if tn == "quote-text":
                move_params(
                    [
                ...
                if origt != str(t):
                    this_template_notes.append(
                        "rename numbered params in {{quote-text}} to named params and remove blank ones"
                    )
            if tn == "quote-us-patent":
                ...
                if origt != str(t):
                    this_template_notes.append(
                        "rename numbered params in {{quote-us-patent}} to named params and remove blank ones"
                    )
            if tn == "quote-video game":
                ...
                if origt != str(t):
                    this_template_notes.append(
                        "rename numbered params in {{quote-video game}} to named params and remove blank ones"
                    )
            if tn == "quote-web":
                move_params(
                    [
                ...
                if origt != str(t):
                    this_template_notes.append(
                        "rename numbered params in {{quote-web}} to named params and remove blank ones"
                    )
...
parser.add_argument(
    "--split-authors",
    action="store_true",
    help="Try to split cases where multiple authors given in author= and similar params",
)
parser.add_argument(
    "--combine-authors",
    action="store_true",
    help="Combine multiple author fields and first/last fields into a single param",
)
parser.add_argument("--remove-wrapping", action="store_true", help="Remove {{lang|...}} and other wrapping in params")
parser.add_argument(
    "--remove-unsafe-wrapping", action="store_true", help="Remove unsafe {{lang|...}} wrapping in params"
)
...
parser.add_argument(
    "--templates-to-rename-numbered-params",
    help="Comma-separated list of templates for which to rename numbered params",
)
