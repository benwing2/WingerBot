...
from wingerbot.blib import getparam, getrmparam, tname, msg
    ...
    for line in result.split("\n"):
        ...
        if m:
            ...
            if number == "both" and definiteness == "both":
                if caseind >= 4:
                    errandpagemsg(
                        "WARNING: Found line with too many forms after encountering case heading: %s" % line.strip()
                    )
                    return None
...
def compare_new_and_old_templates(origt, newt, pagetitle, pagemsg, errandpagemsg):
    ...
    return (
        blib.compare_new_and_old_template_forms(
            origt,
            newt,
            generate_old_forms,
            lambda: new_forms_for_compare,
            pagemsg,
            errandpagemsg,
            already_split=True,
            show_all=True,
...
def convert_template_to_new(t, pagetitle, pagemsg, errandpagemsg, notes):
    ...
    if re.search("(dagur|dyr|son|sonur|bylur)$", pagetitle):
        ...
        j = getp("j")
        if (
            tn == "is-decl-noun-m-s1"
            and p4 == "i"
            and p5 == "r"
            and re.search("^(k|kk|lk|nk|rk|sk|g|gg|lg|ng|rg)$", p3)
        ):
            j = j or "j"
        u = getp("u")
        v = getp("v")
        ...
        if sg != "-":
            ...
            pls = (
                ""
                if pl == "-" or re.search("[áóú]$", pagetitle) or tn == "is-decl-noun-f-s2"
                else ",%sar" % (j or v) if tn == "is-decl-noun-f-s1" else ",%s%sur" % (imut_prefix, j or v)
            )
...
parser.add_argument(
    "--ignore-differences",
    action="store_true",
    help="Convert even when new-old comparison doesn't check out. BE CAREFUL!",
)
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_cats=["Icelandic nouns"]
)
