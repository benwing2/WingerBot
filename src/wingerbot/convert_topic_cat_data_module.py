            ...
            while True:
                ...
                if "parents" in value_by_key:
                    ...
                    def process_parents(value):
                        ...
                        if m:
                            ...
                            split_parents = re.split(
                                r"(\{[^{}]*\}|%s)" % either_quote_string_re_no_parens, m.group(1).strip()
                            )
                            parents = []
                                ...
                                if i % 2 == 1:
                                    if split_parent.startswith("{"):
                                        ...
                                        if parent == "list of sets":
                                            if typ:
                                                linemsg(
                                                    "WARNING: For label '%s', saw explicit 'type=%s' as well as 'list of sets' in parents %s"
                                                    % (label, typ, value)
                                                )
                                                return None
                                            typ = "set"
                                            ...
                                            if typ:
                                                linemsg(
                                                    "WARNING: For label '%s', saw explicit 'type=%s' as well as 'list of topics' in parents %s"
                                                    % (label, typ, value)
                                                )
                                                return None
                                            typ = "topic"
                                    ...
                                    i == 0
                                    and split_parent.strip()
                                    or i == len(split_parents) - 1
                                    and split_parent.strip() not in ["", ","]
                                    or i > 0
                                    and i < len(split_parents) - 1
                                    and split_parent.strip() != ","
                                ):
                ...
                if "description" in value_by_key:
                    ...
                    if desc.startswith("default-set"):
                        if typ == "topic":
                            linemsg(
                                "WARNING: For label '%s', saw 'default-set' but no 'list of sets' in parents, assuming set"
                                % label
                            )
                            typ = "set"
