...
def uk_lemma_is_indeclinable(t, pagetitle, pagemsg):
    if tname(t) in ["uk-noun", "uk-proper noun"]:
        ...
        if (
            headword
            and headword == getparam(t, "3")
            and (not re.search("я́?$", headword) or not getparam(t, "2").startswith("n"))
        ):
...
def hi_lemma_is_indeclinable(t, pagetitle, pagemsg):
    ...
    if tname(t) == "hi-adj":
        ...
        return not (pagename.endswith(AA) or pagename.endswith(IND_AA) or pagename.endswith(AA + M))
    return False
