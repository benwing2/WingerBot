            ...
            for j, term in enumerate(terms):
                if ", " in term:
                    ...
                    for i, new_term in enumerate(this_new_terms):
                        ...
                        if (
                            right_q
                            and (j < len(terms) - 1 or i < len(this_new_terms) - 1)
                            and not (i < len(this_new_terms) - 1 and this_new_terms[i + 1] == ";")
                        ):
...
parser = blib.create_argparser(
    "Split multiple terms in a single param in {{desc}} into multiple params", include_pagefile=True, include_stdin=True
)
