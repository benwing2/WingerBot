...
def convert_bg_manual_ipa(ipa, pagetitle, pagemsg):
    ...
    if num_ipa_ij != num_pagetitle_ij:
        pagemsg(
            "WARNING: IPA %s (original %s) has %s i/j chars but pagetitle '%s' has %s и/й chars"
        ...
        for (ipa_index, ipa_ij), (pagetitle_index, pagetitle_ij) in zip(ipa_ij_indices, pagetitle_ij_indices):
            ...
            if ipa_ij == "и" and pagetitle_ij == "й":
                pagemsg(
                    "Replacing IPA и with й at index %s in %s based on pagetitle й at index %s"
    ...
    for i, x in ipa_vowel_indices_with_stress:
        if x in accent:
            if len(ipa_vowel_indices) == 0:
                pagemsg(
                    "WARNING: Stress mark at index %s not following vowel for IPA %s (original %s)" % (i, ipa, orig_ipa)
                )
                return
            if ipa_vowel_indices[-1][0] != i - 1:
                pagemsg(
                    "WARNING: Stress mark at index %s not directly following vowel for IPA %s (original %s)"
                    % (i, ipa, orig_ipa)
                )
                return
    ...
    if num_ipa_vowels != num_pagetitle_vowels:
        pagemsg(
            "WARNING: IPA %s (original %s) has %s vowel(s) but pagetitle '%s' has %s vowel(s)"
            % (ipa, orig_ipa, num_ipa_vowels, pagetitle, num_pagetitle_vowels)
        )
    else:
        # If so, match up corresponding vowels.
        for (ipa_index, ipa_vowel), (pagetitle_index, pagetitle_vowel) in zip(
            ipa_vowel_indices, pagetitle_vowel_indices
        ):
            # Correct unstressed IPA о to у/ю as appropriate.
            if ipa_vowel == "о" and pagetitle_vowel == "у":
                pagemsg(
                    "Replacing unstressed IPA о with у at index %s in %s based on pagetitle vowel at index %s"
                    % (ipa_index, ipa, pagetitle_index)
                )
                ipa_chars[ipa_index] = "у"
            elif (
                ipa_vowel == "о" and ipa_index > 0 and ipa_chars[ipa_index - 1] in ["ь", "й"] and pagetitle_vowel == "ю"
            ):
                pagemsg(
                    "Replacing unstressed IPA %sо with ю at index %s in %s based on pagetitle vowel at index %s"
            ...
            # Correct unstressed IPA у to о as appropriate (e.g. акробатика with pronunciation /akruˈbatikə/)
                ...
                pagemsg(
                    "Replacing unstressed IPA у with о at index %s in %s based on pagetitle vowel at index %s"
                    % (ipa_index, ipa, pagetitle_index)
                )
            ...
            # Correct unstressed IPA ъ to а/я as appropriate.
                ...
                pagemsg(
                    "Replacing unstressed IPA ъ with а at index %s in %s based on pagetitle vowel at index %s"
                    % (ipa_index, ipa, pagetitle_index)
                )
                ipa_chars[ipa_index] = "а"
            elif (
                ipa_vowel == "ъ" and ipa_index > 0 and ipa_chars[ipa_index - 1] in ["ь", "й"] and pagetitle_vowel == "я"
            ):
                pagemsg(
                    "Replacing unstressed IPA %sъ with я at index %s in %s based on pagetitle vowel at index %s"
                    % (ipa_chars[ipa_index - 1], ipa_index, ipa, pagetitle_index)
                )
            ...
            # Correct stressed IPA ъ to а/я as appropriate if word-final, setting endschwa=1.
                ...
                ipa_vowel == "ъ́"
                and pagetitle_vowel == "а"
                and (ipa_index == len(ipa) - 2 or ipa_index == len(ipa) - 3 and ipa.endswith("т"))
            ):
                pagemsg(
                    "Replacing stressed IPA ъ with а at index %s in %s and endschwa=1 based on pagetitle vowel at index %s"
                    % (ipa_index, ipa, pagetitle_index)
                )
                ipa_chars[ipa_index] = "а"
                endschwa = True
                ...
                ipa_vowel == "ъ́"
                and ipa_index > 0
                and ipa_chars[ipa_index - 1] in ["ь", "й"]
                and pagetitle_vowel == "я"
                and (ipa_index == len(ipa) - 2 or ipa_index == len(ipa) - 3 and ipa.endswith("т"))
            ):
                pagemsg(
                    "Replacing unstressed IPA %sъ with я at index %s in %s and endschwa=1 based on pagetitle vowel at index %s"
                    % (ipa_chars[ipa_index - 1], ipa_index, ipa, pagetitle_index)
                )
                ipa_chars[ipa_index] = "я"
                ipa_chars[ipa_index - 1] = ""
    ...
    if pagetitle[-1] in voiced_to_unvoiced and ipa[-1] == voiced_to_unvoiced[pagetitle[-1]]:
        pagemsg(
            "Replacing final devoiced IPA %s with %s based on final voiced pagetitle consonant"
            % (ipa[-1], pagetitle[-1])
        )
    ...
    if num_ipa_cons != num_pagetitle_cons:
        pagemsg(
            "WARNING: IPA %s (original %s) has %s consonant(s) but pagetitle '%s' has %s consonant(s)"
        ...
        for (ipa_index, ipa_cons), (pagetitle_index, pagetitle_cons) in zip(ipa_cons_indices, pagetitle_cons_indices):
            ...
            def check_next_voicing(obstruents):
                return (
                    ipa_index < len(ipa) - 1
                    and ipa_chars[ipa_index + 1] in obstruents
                    or (
                        ipa_index < len(ipa) - 2
                        and ipa_chars[ipa_index + 1] in stress_mark
                        and ipa_chars[ipa_index + 2] in obstruents
                    )
                )

            # Correct devoiced IPA obstruent to voiced equivalent as appropriate.
            if (
                ipa_cons in unvoiced_to_voiced
                and check_next_voicing(unvoiced_obstruent)
                and (pagetitle_cons == unvoiced_to_voiced[ipa_cons])
            ):
                new_ipa_cons = unvoiced_to_voiced[ipa_cons]
                pagemsg(
                    "Replacing unvoiced IPA %s before unvoiced obstruent with %s at index %s in %s based on pagetitle cons at index %s"
            ...
            # Correct voiced IPA obstruent to unvoiced equivalent as appropriate.
                ...
                ipa_cons in voiced_to_unvoiced
                and check_next_voicing(voiced_obstruent)
                and (pagetitle_cons == voiced_to_unvoiced[ipa_cons])
            ):
                new_ipa_cons = voiced_to_unvoiced[ipa_cons]
                pagemsg(
                    "Replacing voiced IPA %s before voiced obstruent with %s at index %s in %s based on pagetitle cons at index %s"
                    % (ipa_cons, new_ipa_cons, ipa_index, ipa, pagetitle_index)
                )
                ipa_chars[ipa_index] = new_ipa_cons
    ...
    for prefix in prefixes:
        ...
        assert m is not None # Should always match esp. since prefixes don't have newlines in them
        ...
        if prefix_final_cons:
            ...
            auto_stress_ipa = re.sub(
                r"\b(" + prefix_prefix + ")(" + accent_c + ")(" + prefix_final_cons + ")",
                r"\1\3\2",
                auto_stress_ipa,
                0,
                re.U,
    ...
    if ipa_with_orig_accents != auto_stress_ipa:
        pagemsg(
            "WARNING: IPA before accent moving %s not same as auto-stressed IPA %s"
    ...
    if pagetitle_from_ipa != pagetitle:
        pagemsg("WARNING: Inferred respelling %s from IPA %s, does not match pagetitle" % (ipa, orig_ipa))
        return None, False
    return ipa, endschwa
...
parser = blib.create_argparser(
    "Convert manual Bulgarian IPA pronunciations to bg-IPA", include_pagefile=True, include_stdin=True
)
