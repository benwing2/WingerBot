...
def process_text_on_page(p):
    ...
    for t in parsed.filter_templates():
        ...
        if (
            tn == "head"
            and getparam(t, "1") == "bg"
            and getparam(t, "2") in ["noun", "nouns", "proper noun", "proper nouns"]
        ):
            pos = getparam(t, "2")
            params = []
            for param in t.params:
                ...
                if (
                    pname not in ["1", "2", "head", "g", "g2", "g3", "3", "4", "5", "6", "7", "8", "9", "10"]
                    or pname == "3"
                    and pval not in ["masculine", "feminine"]
                    or pname in ["5", "7", "9"]
                    and pval != "or"
                ):
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_cats=["Bulgarian proper nouns"]
)
