...
def process_text_on_page(p):
    ...
    newtext = re.sub(
        r"^\{\{bg-verb-form\|[^{}|\n]*?\|head=([^{}|\n]*?)((?:\|g=[a-z]+)?)\}\}, \{\{bg-verb-form\|[^{}|\n]*?\|head=([^{}|\n]*?)\2\}\}$",
        r"{{head|bg|verb form|head=\1|head2=\3\2}}",
        text,
        0,
        re.M,
    )
        ...
        if tn == "bg-verb form of":
            ...
            if parttype == "vnform":
                ...
                if tname(headt) == "head":
                    ...
                    part_forms_to_add_header.append(
                        (headt, t, g == "masculine" and "m" or g == "feminine" and "f" or g == "neuter" and "n" or "p")
                    )
            elif parttype:
                if tname(headt) == "head":
                    heads = blib.fetch_param_chain(headt, "head", "head")
...
parser = blib.create_argparser(
    "Convert Bulgarian verb forms to standard templates", include_pagefile=True, include_stdin=True
)
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_cats=["Bulgarian verb forms"]
)
