        ...
        if tname(t) == "bg-noun-form":
            ...
            if head:
                ...
                if bglib.needs_accents(pagetitle):
                    pagemsg(
                        "WARNING: Can't add head= to {{bg-noun-form}} missing it because pagetitle is multisyllabic: %s"
                        % str(t)
                    )
                else:
            ...
            if existing_form:
                ...
                for f in existing_form:
                    if bglib.remove_accents(f) != pagetitle:
                        pagemsg("WARNING: Existing head %s doesn't match page title: %s" % (f, str(headt)))
                        must_continue = True
                        break
                if must_continue:
                    continue
                ...
                if any(needs_accents) and not all(needs_accents):
                    pagemsg("WARNING: Some but not all existing heads missing accents: %s" % str(headt))
                    continue
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_cats=["Bulgarian noun forms"], edit=True, stdin=True
)
