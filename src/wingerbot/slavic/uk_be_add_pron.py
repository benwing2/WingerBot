...
accentless = {
    ...
    "post": set(["бы", "б", "ж", "же", "ли", "либо", "ль", "ка", "нибудь", "тка"]),
...
allow_unaccented = ["^бальзам.* на душу"]
...
allowed_l3_headings_when_multiple_etyms = ["References", "Further reading"]
...
def contains_non_cyrillic_non_latin(text):
    ...
    return (
        re.sub(r"[\u0300\u0301\u0302\u0308\u0307\u0323 \-,.?!()_/‿ɣɕʑЀ-ԧꚀ-ꚗ'a-zščžáéíóúýàèìòùỳɛě]", "", text.lower())
        != ""
    )
...
def get_headword_pronuns(parsed, pagetitle, pagemsg, expand_text):
        ...
        if com.remove_accents(h1) == com.remove_accents(h2) and len(h1) < len(h2):
            ...
            if len(h1words) == len(h2words):
                for i in range(len(h1words)):
                    if not (
                        h1words[i] == h2words[i]
                        or not com.is_accented(h1words[i])
                        and com.remove_accents(h2words[i]) == h1words[i]
                    ):
                        return False
            return True
        return False
    ...
    new_headword_pronuns = [
        x for x in headword_pronuns if not headword_should_be_removed_due_to_unaccent(x, headword_pronuns)
    ]
    if len(new_headword_pronuns) <= len(headword_pronuns) - 2:
        pagemsg(
            "WARNING: Removed two or more headword pronuns, check that something didn't go wrong: old=%s, new=%s"
...
def pronun_matches(hpron, foundpron, pagemsg):
    ...
    if hpron == foundpron:
        pagemsg(
            "Matching headword pronun %s to found pronun %s after removing grave accents from the latter"
    ...
    if hpron == foundpron:
        pagemsg(
            "Matching headword pronun %s to found pronun %s after lowercasing (and removing grave accents)"
            % (orighpron, origfoundpron)
        )
        return True
...
def match_headword_and_found_pronuns(headword_pronuns, found_pronuns, pagemsg, expand_text):
    ...
    for hpron, foundprons in matches.items():
        ...
        # Do noun stem
        noun_ending_regex = (
            "[аяеоёьыій]́?$" if args.lang == "be" else "[аяеоьиіїй]́?$" if args.lang == "uk" else "[аяеоьий]́?$"
        )
        append_stem_foundstems(
            re.sub(noun_ending_regex, "", hpron),
            frob_foundprons(foundprons, lambda x: re.sub(noun_ending_regex, "", x)),
        )
        # Also compute reduced/unreduced stem
        append_stem_foundstems(get_reduced_stem(hpron), frob_foundprons(foundprons, get_reduced_stem))
        # Also check for adjectival stem
        if args.lang != "bg":  # Adjectives look like nouns in Bulgarian
            adj_ending_regex = "([ыі]́?|[ая]́?я|[аоя]́?е|[ыі]́?я)$" if args.lang == "be" else "([иії]́?й|[аяеєі]́)$"
            adjstem = re.sub(adj_ending_regex, "", hpron)
            if adjstem != hpron:
                foundpronstems = frob_foundprons(foundprons, lambda x: re.sub(adj_ending_regex, "", x))
                append_stem_foundstems(adjstem, foundpronstems)
                pagemsg("Adding adjectival stem mapping %s->%s" % (adjstem, ",".join(str(x) for x in foundpronstems)))
                ...
                for epvowel in [False, True]:
                    ...
                    deredfoundpronstems = frob_foundprons(foundpronstems, lambda x: com.dereduce(x, epvowel))
                    append_stem_foundstems(deredstem, deredfoundpronstems)
                    pagemsg(
                        "Adding adjectival dereduced stem mapping %s->%s"
                        % (deredstem, ",".join(str(x) for x in deredfoundpronstems))
                    )
        # Also check for verbal stem; peel off parts that don't occur in all
        # forms of the verb
        verb_ending_regex = (
            "(ава́?|ну́?|[аеыіяо]́?)ц(ь|ца)?$"
            if args.lang == "be"
            else "(ува́?|ну́?|[аеиіїяо]́?)т[иь](ся)?$" if args.lang == "uk" else "([мая]́?)( с[еи])?$"
        )
        verbstem = re.sub(verb_ending_regex, "", hpron)
        if verbstem != hpron:
            foundpronstems = frob_foundprons(foundprons, lambda x: re.sub(verb_ending_regex, "", x))
            append_stem_foundstems(verbstem, foundpronstems)
            pagemsg("Adding verbal stem mapping %s->%s" % (verbstem, ",".join(str(x) for x in foundpronstems)))
            iotstem = com.iotate(verbstem)
            iotfoundpronstems = frob_foundprons(foundpronstems, lambda x: com.iotate(x))
            append_stem_foundstems(iotstem, iotfoundpronstems)
            pagemsg(
                "Adding verbal iotated stem mapping %s->%s" % (iotstem, ",".join(str(x) for x in iotfoundpronstems))
            )
        ...
        for stem, foundprons in stem_foundprons:
            ...
            for foundpron in foundprons:
                ...
                if stemaccents != foundpronaccents:
                    pagemsg(
                        "WARNING: Mapping %s->%s has different number of acute accents (%s->%s)"
                        % (stem, foundpron, stemaccents, foundpronaccents)
                    )
                    if not stemaccents or not foundpronaccents:
                        pagemsg(
                            "WARNING: In mapping %s->%s, one has no acute accents and the other has acute accents (%s->%s)"
                            % (stem, foundpron, stemaccents, foundpronaccents)
                        )
...
def lookup_pronun_mapping(parsed, pagemsg, errandpagemsg):
    ...
    for lemma in lemmas:
        ...
        if lemma in lemma_headword_to_pronun_mapping_cache:
            ...
            for m in re.finditer(r"^(.*)(\{\{%s(?:\|[^}]*)?\}\})(.*)$" % pron_temp_name, newpage.text, re.M):
                pretext = m.group(1)
...
def process_section(section, indentlevel, headword_pronuns, pagetitle, pagemsg, errandpagemsg, expand_text):
        ...
        def append_pronun_line(pronun, pre="", post=""):
            ...
            if com.needs_accents(pronun, split_dash=True):
                for allow_regex in allow_unaccented:
                    if re.search(allow_regex, pagetitle):
                        pagemsg("Pronunciation lacks accents but pagetitle in allow_unaccented, allowing: %s" % pronun)
                        break
                else:  # no break
            ...
            if contains_non_cyrillic_non_latin(pronun):
                bad_char_msgs.append(
                    "WARNING: Pronunciation %s to be added contains non-Cyrillic non-Latin chars, skipping" % pronun
                )
            elif contains_latin(pronun):
                bad_char_msgs.append("WARNING: Cyrillic pronunciation %s contains Latin characters, skipping" % pronun)
            ...
            if (
                com.is_monosyllabic(pronun)
                and re.sub(AC, "", pronun) == pagetitle
                or re.search("ё", pronun)
                and pronun == pagetitle
            ):
                pronun = "* %s{{%s%s}}%s\n" % (pre, pron_temp_name, headword_annparam, post)
            else:
                pronun = "* %s{{%s|%s%s}}%s\n" % (pre, pron_temp_name, pronun, headword_annparam, post)
        ...
        if not bad_char_msgs and len(headword_pronuns) == 1:
            ...
            if len(hwords) == len(pronwords):
                for i in range(len(hwords)):
                    ...
                    if (
                        len(hword) > len(pronword)
                        and not com.is_accented(pronword)
                        and com.remove_accents(hword) == pronword
                    ):
                        changed = True
                        pronwords[i] = hword
            if changed:
                pron = "".join(pronwords)
                ournotes.append("copy accents from headword to %s= (%s)" % (paramname, pron_temp_name))
    ...
    for t in parsed.filter_templates():
        if tname(t) == pron_temp_name:
            ...
            if com.is_monosyllabic(arg1) and re.sub(AC, "", arg1) == pagetitle:
                notes.append(
                    "remove 1= because monosyllabic and same as pagetitle modulo accents (%s)" % pron_temp_name
                )
                rmparam(t, "1")
            elif re.search("ё", arg1) and arg1 == pagetitle:
                notes.append("remove 1= because same as pagetitle and has ё (%s)" % pron_temp_name)
                rmparam(t, "1")
            else:
    ...
    if foundpronuns:
        ...
        if set(foundpronuns_no_grave) != set(headword_pronuns_no_grave):
            pagemsg(
                "WARNING: Existing pronunciation template (w/o grave accent) has different pronunciation %s from headword-derived pronunciation %s"
                % (joined_foundpronuns, joined_headword_pronuns)
            )
        elif set(foundpronuns) != set(headword_pronuns):
            pagemsg(
                "WARNING: Existing pronunciation template has different pronunciation %s from headword-derived pronunciation %s, but only in grave accents"
                % (joined_foundpronuns, joined_headword_pronuns)
            )
    ...
    if re.search(r"^===+Pronunciation===+$", section, re.M):
        ...
        while True:
            if insert_before >= len(subsections):
                ...
                return None
            if (
                "===Alternative forms===" not in subsections[insert_before]
                and "===Etymology===" not in subsections[insert_before]
            ):
                break
            insert_before += 2
            ...
            if "===Etymology 1===" in sections[j]:
                ...
                if not need_per_section_pronuns:
                    ...
                    if num_secs_with_pronun == 1:
                        ...
                        m = re.search(
                            r"(^===+Pronunciation===+\n.*?)(\Z|^==|^\[\[|^--)",
                            etymsections[first_sec_with_pronun],
                            re.M | re.S,
                        )
                        if not m:
                            p.msg(
...
def process_lemma(index, pagetitle, forms):
    ...
    for t in parsed.filter_templates():
        ...
        if tn == "%s-conj" % args.lang:
            tempcall = re.sub(r"^\{\{%s-conj" % args.lang, "{{%s-generate-verb-forms" % args.lang, str(t))
        elif tn == "%s-ndecl" % args.lang:
            tempcall = re.sub(r"^\{\{%s-ndecl" % args.lang, "{{%s-generate-noun-forms" % args.lang, str(t))
        elif tn == "%s-adecl" % args.lang:
            tempcall = re.sub(r"^\{\{%s-adecl" % args.lang, "{{%s-generate-adj-forms" % args.lang, str(t))
        if tempcall:
...
parser.add_argument(
    "--forms", help="Form codes of non-lemma forms to process in conjunction with --lemmas and --lemma-file."
)
...
if args.lang == "bg":
    form_aliases = {
        ...
        "impf": ["impf_1sg", "impf_2sg", "impf_3sg", "impf_1pl", "impf_2pl", "impf_3pl"],
        "aor": ["aor_1sg", "aor_2sg", "aor_3sg", "aor_1pl", "aor_2pl", "aor_3pl"],
