...
def compare_forms(autoconj, origforms, replforms, pagemsg):
    for slot in set(replforms.keys() + origforms.keys()):
        ...
        if slot not in origforms:
            pagemsg(
                "WARNING: for replacement %s, form %s=%s in replacement forms but missing in original forms"
                % (autoconj, slot, replforms[slot])
            )
            return False
        if slot not in replforms:
            pagemsg(
                "WARNING: for predicted %s, form %s=%s in original forms but missing in replacement forms"
...
def process_section(index, secnum, pagetitle, sectext):
    ...
    for t in parsed.filter_templates():
        ...
        if tn == "be-conj-manual":
            if conjt:
                pagemsg("WARNING: Saw two conjugation templates %s and %s, skipping" % (str(conjt), str(t)))
    ...
    for m in re.finditer("<!-- type (.*?) -->", sectext):
        if autoconj:
            pagemsg("WARNING: Saw two autoconj comments %s and %s, skipping" % (autoconj, m.group(1)))
            return sectext, notes
        autoconj = m.group(1)
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_refs=["Template:be-conj-manual"]
)
