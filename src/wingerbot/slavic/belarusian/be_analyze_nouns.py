...
def process_text_on_page(p):
    ...
    for t in parsed.filter_templates():
        ...
        def stress(endstressed):
            return "endstressed" if endstressed == True else "stemstressed" if endstressed == False else "mixed"
            ...
            for i in range(1, maxparam + 1):
                ...
                for v in vals:
                    if be.is_multi_stressed(v):
                        p.msg("WARNING: Param %s=%s has multiple stresses: %s" % ((str(i), val, str(t))))
                    if be.needs_accents(v):
                        p.msg("WARNING: Param %s=%s has missing stress: %s" % ((str(i), val, str(t))))
        ...
        def infer_animacy(nom_pl, gen_pl, acc_pl):
            ...
            if acc_pl_vals == nom_pl_vals:
                ...
                p.msg("WARNING: Can't infer animacy: nom_pl=%s, gen_pl=%s, acc_pl=%s" % (nom_pl, gen_pl, acc_pl))
                return "unknown"
        ...
        def compare_stems(marked_stem, unmarked_stem):
            return be.destress_vowels_after_stress_movement(
                marked_stem
        ...
        def infer_reducible(nom_sg, gen_sg, gen_pl, gender, seen_patterns):
            if len(seen_patterns) > 1:
                p.msg("WARNING: Multiple patterns %s, not inferring reducible" % ",".join(seen_patterns))
                return None
            if len(seen_patterns) == 0:
                p.msg("WARNING: No patterns, not inferring reducible")
                return None
            ...
            if re.search("[аяеоё]́?$", nom_sg):
                ...
                for gen_pl in gen_pls:
                    ...
                    if compare_stems(be.dereduce(vowel_stem, not epenthetic_stress) or "", nonvowel_stem):
                        retvals.append("*#(-)" if gender == "N" else "*#")
                        continue
                    if compare_stems(vowel_stem + "ав", nonvowel_stem) or compare_stems(
                        vowel_stem + "яв", nonvowel_stem
                    ):
                        if epenthetic_stress:
                            ...
                            retvals.append("" if gender == "N" else "(ў)")
                        continue
                    if compare_stems(be.remove_accents(vowel_stem) + "о́в", nonvowel_stem) or compare_stems(
                        be.remove_accents(vowel_stem) + "ё́в", nonvowel_stem
                    ):
                        if epenthetic_stress:
                            retvals.append("" if gender == "N" else "(ў)")
                        else:
                    ...
                    # else: # no break
                    p.msg(
                        "WARNING: Unable to determine relationship between nom_sg %s and gen_pl %s" % (nom_sg, gen_pl)
                    )
                return ",".join(retvals)
            else:
                orig_nom_sg = nom_sg
                ...
                nom_sg += ending
                if be.destress_vowels_after_stress_movement(
                    nom_sg
                ) == be.undo_mark_stressed_vowels_in_unstressed_syllables(gen_sg):
                    return ""
                p.msg(
                    "WARNING: Unable to determine relationship between nom_sg %s and gen_sg %s" % (orig_nom_sg, gen_sg)
                )
                return None
        ...
        if tn == "be-decl-noun":
            ...
            if (
                gen_sg_end_stressed == "unknown"
                or acc_sg_end_stressed == "unknown"
                or nom_pl_end_stressed == "unknown"
                or loc_pl_end_stressed == "unknown"
            ):
                p.msg("WARNING: Missing stresses, can't determine accent pattern: %s" % str(t))
                continue
            seen_patterns = []
            for pattern, accents in accent_patterns:
                if (
                    matches(ins_sg_end_stressed, accents["inssg"])
            ...
            p.msg(
                ...
                % (
                    ...
                    animacy,
                    ":".join(seen_patterns),
                    stress(gen_sg_end_stressed),
                    stress(dat_sg_end_stressed),
            ...
            if len(heads) > 1:
                p.msg("WARNING: Multiple heads, not inferring declension: %s" % ",".join(heads))
                continue
            if gender == "unknown" or animacy == "unknown":
                p.msg("WARNING: Unknown gender or animacy, not inferring declension")
                continue
            defan = infer_animacy(nom_pl, gen_pl, acc_pl)
            if not (defan == "in" and animacy == "in" or defan == "an" and animacy in ["pr", "anml"]):
                p.msg(
                    "WARNING: Inferred animacy %s != explicit animacy %s, not inferring declension" % (defan, animacy)
                )
                continue
            lemma = heads[0]
            parts = []
            ...
            if gender == "M":
                if re.search("у́?$", gen_sg):
                    ...
                    parts.append("genju")
            p.msg("Inferred declension %s<%s>" % (lemma, ".".join(parts)))
            ...
            loc_sg = fetch("6")
            loc_sg_end_stressed = param_is_end_accented(loc_sg, locative_singular_endings)
            if gen_sg_end_stressed == "unknown" or acc_sg_end_stressed == "unknown":
                p.msg("WARNING: Missing stresses, can't determine accent pattern: %s" % str(t))
                continue
            if not heads:
                p.msg("WARNING: No head found")
            ...
            for pattern, accents in accent_patterns:
                if pattern not in ["a", "d" if re.search("[аяеёо]́?$", lemma) else "b"]:
                    continue
                if matches(ins_sg_end_stressed, accents["inssg"]) and matches(acc_sg_end_stressed, accents["accsg"]):
                    seen_patterns.append(pattern)
            if "a" in seen_patterns and "b" in seen_patterns:
                seen_patterns = ["a", "b"]
            ...
            loc_pl = fetch("6")
            loc_pl_end_stressed = param_is_end_accented(loc_pl)
            if nom_pl_end_stressed == "unknown" or loc_pl_end_stressed == "unknown":
                p.msg("WARNING: Missing stresses, can't determine accent pattern: %s" % str(t))
                continue
            seen_patterns = []
            for pattern, accents in accent_patterns:
                if pattern not in ["a", "b", "e"]:
                    continue
                if matches(nom_pl_end_stressed, accents["nompl"]) and matches(loc_pl_end_stressed, accents["locpl"]):
                    seen_patterns.append(pattern)
            if "a" in seen_patterns and "b" in seen_patterns:
                seen_patterns = ["a", "b"]
...
parser = blib.create_argparser("Analyze Belarusian noun declensions", include_pagefile=True, include_stdin=True)
args = parser.parse_args()
start, end = blib.parse_start_end(args.start, args.end)

blib.do_pagefile_cats_refs(args, start, end, process_text_on_page, new=True, default_cats=["Belarusian nouns"])
