    ...
    [
        ...
        [
            ...
            [
                "ц-stem",
                ["a", "b", "c", "d", "f"],
            ],  # b maybe only ружьецо́, деревцо́; c maybe only се́рдце, де́ревце; d maybe only лицо́; f maybe only крыльцо́
            ["vowel-stem", ["a", "b", "d"]],
            ["i-stem", ["a", "b"]],
            ["3rd-declension", ["c"]],
        ],
...
if args.pos == "noun":
    ...
    for c in cases:
        for n in numbers:
            create_cat("~ with irregular %s %s" % (c, n), ["irregcase", "%s %s" % (c, n)])
    ...
    for stress, gender, sgplstems in adj_decls:
        for sg, pl, stems in sgplstems:
            for stem in stems:
                if stress:
                    create_cat("%s %s accent-%s adjectival ~" % (stem, gender, stress), ["adj", sg, pl])
                else:
                    create_cat("%s %s adjectival ~" % (stem, gender), ["adj", sg, pl])
...
if args.pos == "adj":
    ...
    create_adj_cat(
        ...
        [
            "misc",
            "This category contains Russian adjectives where the ending is stressed. They end in {{lang|ru|-о́й}}, or in some cases {{lang|ru|-и́н}}, {{lang|ru|-о́в}}, or {{lang|ru|-ёв}}.",
        ],
    )
    create_adj_cat(
        "~ with short forms",
    ...
    create_adj_cat(
        ...
        [
            "misc",
            "This category contains Russian adjectives that exist '''only''' as short forms. This means they function only as {{glossary|predicative}} adjectives. Examples are {{m|ru|рад||[[glad]]}}, {{m|ru|до́лжен||[[obligated]], [[must]], [[have to]], [[ought]]}}, and {{m|ru|гора́зд||[[skillful]], [[capable]]}}.",
        ],
    )
