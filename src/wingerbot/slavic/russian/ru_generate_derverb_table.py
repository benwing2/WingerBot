...
def render_groups(groups):
    ...
    def sort_aspect_pair(x, y):
        ...
        retval = compare_aspect_pair(
            rulib.remove_accents(xpf),
...
def combine_prefix(prefix, suffixes, aspect):
    ...
    for suffix in suffixes:
        links.append("{{l|ru|" + paste_verb(prefix, suffix) + ("|g=%s" % aspect if add_aspect else "") + "}}")
    return "* " + ", ".join(links)
...
for lineno, line in blib.iter_items_from_file(args.direcfile, start, end):
    if re.search("^--+$", line):
        ...
        group.append(
            (
                combine_prefix(line, pfsuffixes, "pf"),
        ...
        group.append(
            (
                combine_prefix(pf, pfsuffixes, "pf"),
                ...
                for index, verb in enumerate(re.split(",", direc)):
                    ...
                    if verb:
                        ...
                        if verb.startswith("["):
                            ...
                            assert endbracket
                            links.append(
                                "[{{l|ru|%s%s}}]%s" % (verb, gender, notes and " {{i|%s}}" % ", ".join(notes) or "")
                            )
                        else:
                            links.append(
                                "{{l|ru|%s%s}}%s" % (verb, gender, notes and " {{i|%s}}" % ", ".join(notes) or "")
