...
def process_text_on_page(index, pagetitle, text):
    ...
    def process_new_style_headword(htemp):
        ...
        for i in range(1, highest_numbered_param + 2):
            ...
            if i == highest_numbered_param + 1 or val in ["or", "_", "-"] or re.search("^join:", val):
                end_arg_set = True
    ...
    def process_verb_headword(htemp):
        ...
        words = [x for num, x in enumerate(re.split(r"([^\s\[\]]+|\[\[.*?\]\])", getparam(htemp, "1"))) if num % 2 == 1]
        for word in words:
...
for lemma, nonexistent_msg in sorted(nonexistent_lemmas.items(), key=lambda pair: (-lemma_count[pair[0]], pair[0])):
    msg(
        "* [[%s]] (%s occurrence%s): %s (refs: %s)"
        % (
            lemma,
            lemma_count[lemma],
            "" if lemma_count[lemma] == 1 else "s",
            nonexistent_msg,
