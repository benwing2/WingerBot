            ...
            if prefix is not None:
                if this_prefix != prefix:
                    if remove_stress(this_prefix) == prefix:
                        ...
                        raise InferError(
                            "Saw two different prefixes %s and %s for suffixes %s"
                            % (prefix, this_prefix, ",".join(suffixes_no_stress))
                        )
            prefix = this_prefix
    if prefix is None:
        raise InferError(
            "Can't extract prefix from perfect(s) %s, imperfect(s) %s, possible suffixes %s"
    ...
    def process_verb(verb, bracketed, mods, append_to):
        ...
        if verb.startswith(prefix):
            ...
            if verb.startswith(prefix_no_stress):
                ...
                raise InferError(
                    "Can't extract prefix %s from verb %s (suffixes %s)" % (prefix, verb, ",".join(suffixes_no_stress))
                )
    ...
    for verb, bracketed, mods in impfs:
        ...
        if new_prefix != prefix:
            if first:
                ...
                raise InferError(
                    "Can't extract prefix %s from verb %s for suffixes %s"
    ...
    if line.startswith("suffixes:"):
        ...
        if pf.startswith("-") and impf.startswith("-"):
            current_pf_suffixes = (
                []
                if pf == "-"
                else [(re.sub("^-", "", suffix), brackets, mods) for suffix, brackets, mods in split_verbs(pf)]
            )
            current_impf_suffixes = (
                []
                if impf == "-"
                else [(re.sub("^-", "", suffix), brackets, mods) for suffix, brackets, mods in split_verbs(impf)]
            )
        ...
        def expand_item(item, suffixes, aspect):
            ...
            for ind, (maybe_pref, brackets, mods) in enumerate(maybe_prefs):
                ...
                if maybe_pref.endswith("-"):
                    ...
                    retval.append(
                        rebracket(paste_verb(maybe_pref, suffix), brackets or suffix_brackets, mods + suffix_mods)
                    )
                else:
        ...
        return join_pf_impf(
            expand_item(pf, current_pf_suffixes, "pf"), expand_item(impf, current_impf_suffixes, "impf")
        )
...
def do_line_group(group, is_last_group, fn, outfd, suffix_lines, extra_suffixes, pass_):
    ...
    for lineno, line in items:
        ...
        try:
            if " " not in line:
                ...
                prefix, pf_suffixes, impf_suffixes = extract_prefix_and_suffixes(
                    pfs, impfs, [suffix_no_stress] + extra_suffixes
                )
                ...
                for pf_suffix, brackets, mods in pf_suffixes:
                    if AC in pf_suffix:
                        ...
                        if (
                            unstressed_pf_suffix in unstressed_suffix_to_suffix
                            and unstressed_suffix_to_suffix[unstressed_pf_suffix] != pf_suffix
                        ):
                            linemsg(
                                "Unstressed perfective suffix %s maps to two different stressed suffixes %s and %s"
    ...
    def handle_empty_pf_or_impf(non_empty_suffixes, prefixes, aspect):
        ...
        if len(potential_full_suffixes) == 0:
            for prefix in prefixes:
                ...
                unattached_lines.append(
                    convert_prefix_and_suffixes_to_full(
                        prefix,
            ...
            best_suffixes = sorted(potential_full_suffixes, key=lambda x: (-len(split_verbs(x[0])), x[1]))[0][0]
            suffix_key = (
                (best_suffixes, non_empty_suffixes) if aspect == "impf" else (non_empty_suffixes, best_suffixes)
            )
            for prefix in prefixes:
        ...
        keys_to_delete.append(
            ("" if aspect == "impf" else non_empty_suffixes, non_empty_suffixes if aspect == "impf" else "")
        )
    ...
    for (pf_suffixes, impf_suffixes), prefixes in sorted(
        prefixes_by_suffixes.items(), key=sort_key_for_sorting if args.sort else lambda x: ordering_dict[x[0]]
    ):
        if len(prefixes) == 1 and "." in prefixes[0]:
            msg_and_check(
                convert_prefix_and_suffixes_to_full(prefixes[0], split_verbs(pf_suffixes), split_verbs(impf_suffixes))
            )
        else:
