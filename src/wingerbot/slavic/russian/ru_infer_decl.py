...
ru_decl_noun_unc_cases = ["nom_sg", "gen_sg", "dat_sg", "acc_sg", "ins_sg", "pre_sg", "loc", "voc"]
...
def compare_terms(case, real, pred, pagemsg):
    ...
    for realword, predword in zip(realwords, predwords):
        if realword == predword:
            ...
            pagemsg(
                "For case %s, real word %s in %s missing an accent that's present in predicted word %s in %s; allowed"
...
def synthesize_singular(nompl, prepl, gender, pagemsg):
    ...
    if m:
        ...
        ac = m.group(3)
        if ac:
            sg = stem + "о́й" if gender == "m" else stem + "а́я" if gender == "f" else stem + "о́е"
        elif ty == "soft":
            sg = stem + "ий" if gender == "m" else stem + "яя" if gender == "f" else stem + "ее"
        elif ty == "velar" or ty == "sibilant":
            sg = stem + "ий" if gender == "m" else stem + "ая" if gender == "f" else stem + "ое"
        elif ty == "c":
            sg = stem + "ый" if gender == "m" else stem + "ая" if gender == "f" else stem + "ее"
        else:
            sg = stem + "ый" if gender == "m" else stem + "ая" if gender == "f" else stem + "ое"
        return [sg]
    ...
    return [
        (
            ...
            if gender == "f"
                ...
                add_soft_sign(stem)
                if soft and gender == "m"
                else (
                    stem
                    if gender == "m"
                    else (
                        stem + "е"
                        if soft and gender == "n"
                        else (
                            add_hard_neuter(stem)
                            if gender == "n"
                            else blib.do_assert(False, "Unrecognized gender: %s" % gender)
                        )
                    )
                )
            )
                ...
                for wordforms in words:
                    ...
                    pagemsg(
                        ...
                        % (
                            wordno,
                            (
                                wordforms.get("nom_pl", "(blank)")
                                if numonly == "pl"
                                else wordforms.get("nom_sg", "(blank)")
                            ),
                        )
                    )
...
def get_lemma(linked_headwords, lemma, multiword, pagemsg):
    if lemma in linked_headwords:
        ...
        if "|" in linked_lemma:
            linked_lemma = linked_lemma.replace("|" + make_unstressed_once_ru(lemma) + "]]", "|" + lemma + "]]")
...
def infer_word(forms, noungender, linked_headwords, number, numonly, multiword, pagemsg):
    ...
    for nomsg in nomsgs:
        ...
        def adj_by_prep():
            return (
                numonly == "sg"
                and re.search("[ое][мй]$", make_unstressed_once_ru(presg))
                or numonly != "sg"
                and re.search("[ыи]х$", make_unstressed_once_ru(prepl))
            )

        if re.search("([ыиіо]й|[яаь]я|[оеь]е)$", make_unstressed_once_ru(nomsg)) and adj_by_prep():
            if old_template:
        ...
        if re.search("([ое]в|[ыи]н)([оаъ]?)$", make_unstressed_once_ru(nomsg)) and adj_by_prep():
        ...
        if m:
            ...
            if numonly != "sg":
                ...
                if "," not in nompl:
                    ...
                    if (
                        plstem == stem
                        or plstem == make_unstressed_once_ru(stem)
                        or make_unstressed_once_ru(plstem) == stem
                    ):
                        plstem = None
                    else:
                    ...
                    if (
                        ending in ["а", "я"]
                        and unomplending in ["ы", "и"]
                        or ending not in ["а", "я"]
                        and unomplending in ["а", "я"]
                    ):
