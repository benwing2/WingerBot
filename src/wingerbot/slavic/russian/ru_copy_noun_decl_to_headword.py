...
def process_page_section(index, pagetitle, section):
    ...
    if len(noun_table_templates) < 1:
        if noun_old_templates:
            pagemsg(
                "WARNING: No ru-noun-table templates but found ru-noun-old template(s): %s"
    ...
    for param in noun_table_template.params:
        ...
        if re.search("^[gmf][0-9]*$", name):
            pagemsg("WARNING: Found g=, m= or f= in noun-table, removing: %s" % str(noun_table_template))
        else:
    ...
    if str(headword_template.name) == "ru-proper noun+":
        generate_template = re.sub(r"^\{\{ru-noun-table", "{{ru-generate-noun-args", str(noun_table_template))
        ...
        if declargs["n"] == "s":
            ...
            if ndef_args["n"] == "s":
                ...
                if existing_n and not re.search(r"^s", existing_n):
                    pagemsg("WARNING: Something wrong: Found n=%s, not singular" % existing_n)
                pagemsg("Removing n=sg from headword template")
                rmparam(modified_noun_table_template, "n")
            else:
                pagemsg("WARNING: Unable to remove n= from headword template because n=%s" % ndef_args["n"])

    new_headword_template = re.sub(r"^\{\{ru-noun-table", "{{ru-noun+", str(modified_noun_table_template))
    ...
    if existing_filtered_headword_template != new_headword_template:
        if "[" in existing_filtered_headword_template and "[" not in new_headword_template:
            if blib.remove_links(existing_filtered_headword_template) == new_headword_template:
                ...
                pagemsg(
                    "WARNING: Existing headword template %s would be overwritten with %s but links would be erased, not doing it, check manually"
                    % (existing_filtered_headword_template, new_headword_template)
                )
                return None
        else:
            pagemsg(
                "WARNING: Existing headword template %s will be overwritten with %s"
                % (existing_filtered_headword_template, new_headword_template)
            )
    ...
    if new_headword_template != orig_headword_template:
        pagemsg("Replacing headword %s with %s" % (orig_headword_template, new_headword_template))
