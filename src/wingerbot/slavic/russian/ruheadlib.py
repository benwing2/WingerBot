...
alt_yo_templates = ["ru-noun-alt-ё", "ru-verb-alt-ё", "ru-adj-alt-ё", "ru-proper noun-alt-ё", "ru-pos-alt-ё"]
...
def output_stats(pagemsg):
    ...
    pagemsg(
        "Cache lookups = %s, hits = %s, %0.2f%% hit rate"
        % (
            num_cache_lookups,
            num_cache_hits,
...
def lookup_heads_and_inflections(pagename, pagemsg):
    ...
    if pagename in manually_specified_inflections:
        ...
        if lemma is True:
            ...
            return "manual-override", ({(accented, "", False)}, {(frozenset({(accented, "", False)}), lemma)}, set())
    ...
    if pagename in accented_cache:
        ...
        if not saw_lemma and not inflections_of:
            ...
            if len(yo_pages) > 1:
                pagemsg(
                    "WARNING: lookup_heads_and_inflections: Found multiple alt-ё templates for different lemmas: %s"
                    % ",".join(yo_pages)
                )
            elif len(yo_pages) == 0:
                pagemsg(
                    "WARNING: lookup_heads_and_inflections: Found no lemmas or inflections of lemmas for %s" % pagename
                )
            else:
