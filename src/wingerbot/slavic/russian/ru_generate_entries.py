...
parser.add_argument("--adverb", action="store_true", help="Directive file contains adverbs instead of adjectives.")
parser.add_argument("--noun", action="store_true", help="Directive file contains nouns instead of adjectives.")
...
while True:
    ...
    if len(els) == 2 and els[1].startswith("altyo:"):
        ...
        pos_to_altyo = {"n": "ru-noun-alt-ё", "pn": "ru-proper noun-alt-ё", "adj": "ru-adj-alt-ё", "v": "ru-verb-alt-ё"}
        if pos in pos_to_altyo:
            altyotemp = pos_to_altyo[pos]
            if len(altyoparts) == 2:
                yoline = "{{%s|%s}}" % (altyotemp, altyoparts[1])
            else:
                yoline = "{{%s|%s|%s}}" % (altyotemp, altyoparts[1], altyoparts[2])
        else:
            assert pos in pos_to_full_pos
...
"""
            % (rulib.remove_accents(altyoparts[1]).replace("ё", "е"), pos_to_full_pos[pos], yoline)
        )
        continue
    ...
    if etym == "?":
        ...
        infleclines = ["# {{inflection of|lang=ru|%s||%s}}" % (m.group(3), form) for form in forms[m.group(2)]]
...
%s\n\n""" % (
    ...
    for synantrel in remainder:
        ...
        if not m:
            error(
                "Element %s doesn't start with also:, syn:, ant:, der:, rel:, see:, comp:, pron:, alt: or part:"
                % synantrel
            )
