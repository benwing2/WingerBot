...
# 28. (DONE) Don't include head=/1= if same as pagetitle.
...
# section.
allow_stress_mismatch_list = ["щавель"]
...
def check_for_redundant_translit(ru, tr, pagemsg, warnfun, expand_text):
    ...
    if compare_normalized(autotr, tr):
        ...
        return ru, ""
    pagemsg("Keeping non-redundant translit %s != auto %s for Russian %s" % (tr, autotr, ru))
    return ru, tr
...
def lemma_matches(lemma, ru, tr, warnfun, expand_text):
    ...
    if ru == lemru:
        ...
        if not trmatches:
            warnfun(
                "Value %s matches lemma %s of ru-(proper )noun+, but translit %s doesn't match %s"
                % (ru, lemru, tr, lemtr)
            )
        else:
            return True
...
def create_inflection_entry(
    ...
    pagetitles = set(rulib.remove_accents(infl) for infl, infltr in inflections)
    # If multiple inflections, they should have the same pagetitle minus accents
    assert len(pagetitles) == 1
    pagetitle = list(pagetitles)[0]
    ...
    def pagemsg(text, simple=False, fun=msg):
        fun("Page %s %s: %s" % (index, pagetitle, format_pagemsg_text(text, simple)))
    ...
    for skip_form, skip_lemma in skip_form_pages:
        if skip_form == pagetitle and skip_lemma == rulib.remove_accents(lemma):
    ...
    inflections = [
        check_for_redundant_translit(infl, infltr, pagemsg, warn, expand_text) for infl, infltr in inflections
    ]
    ...
    generic_infltype = (
        "participle" if is_participle else re.sub(" form.*", " form", infltype) if " form" in infltype else infltype
    )
    ...
    for infl, infltr in inflections:
        if infl == "-":
            pagemsg(
                "Not creating %s entry - for %s %s%s"
    ...
    def compare_param(t, param, value, valuetr, param_is_head, issue_warnings=True, allow_stress_mismatch=False):
        ...
        if rulib.is_multi_stressed(paramval):
            warn_if(issue_warnings, "Param %s=%s has multiple accents: %s" % (param, paramval, str(t)))
        if rulib.is_multi_stressed(value):
            warn_if(
                issue_warnings, "Value %s to compare to param %s=%s has multiple accents" % (value, param, paramval)
            )
        ...
        if rulib.remove_accents(value) == rulib.remove_accents(paramval):
            ...
            if valueaccents != paramvalaccents:
                warn_if(
                    issue_warnings,
                    "Value %s (%s accents) matches param %s=%s (%s accents) except for accents, and different numbers of accents: %s"
                    % (value, valueaccents, param, paramval, paramvalaccents, str(t)),
                )
        ...
        if matches:
            ...
            if not valuetr and not trparamval:
                return True
            if (
                allow_stress_mismatch
                and valuetr
                and trparamval
                and rulib.remove_tr_accents(valuetr) == rulib.remove_tr_accents(trparamval)
            ):
                return True
            ...
            warn_if(
                issue_warnings,
                "Value %s matches param %s=%s, but translit %s doesn't match param %s=%s: %s"
    ...
    def template_head_matches(t, inflections, purpose, fail_when_left_over_heads=False, issue_warnings=True):
        ...
        if tname(t) in ["ru-noun+", "ru-proper noun+"]:
            ...
            if lemmaarg is None:
                ...
                for infl, infltr in inflections:
                    for lem in lemmas:
                        if lemma_matches(lem, infl, infltr, lambda x: warn_if(issue_warnings, x), expand_text):
                            some_match = True
                            lemmas.remove(lem)
                            break
                    else:
            ...
            for infl, infltr in inflections:
                for param in headparams:
                    if compare_param(t, param, infl, infltr, param_is_head=True, issue_warnings=issue_warnings):
                        some_match = True
        ...
        if some_match and not all_match:
            warn_if(
                issue_warnings,
                "Some but not all inflections %s match template when %s: %s"
            ...
            if fail_when_left_over_heads:
                warn_if(
                    issue_warnings,
                    "All inflections %s match template, but extra heads in template when %s, treating as a non-match: %s"
                    % (joined_infls_with_tr(), purpose, str(t)),
                )
                return False
            else:
                warn_if(
                    issue_warnings,
                    "All inflections %s match template, but extra heads in template when %s: %s"
                    % (joined_infls_with_tr(), purpose, str(t)),
                )
        return all_match
    ...
    if entrytext:
        ...
        headno = 0
        no_param1 = False
        if len(inflections) == 1 and inflections[0][0] == pagetitle and not inflections[0][1]:
            # Don't add head=/1= params if there's only one inflection that's the
            # same as the pagetitle and there's no translit. But if we add gender
            ...
            for infl, infltr in inflections:
                ...
                if headno == 1:
                    headparams.append(
                        "|%s%s%s" % ("head=" if headtemp_is_head else "", infl, "|tr=%s" % infltr if infltr else "")
                    )
                else:
                    headparams.append("|head%s=%s%s" % (headno, infl, "|tr%s=%s" % (headno, infltr) if infltr else ""))
        ...
        for g in gender:
            genderno += 1
            if genderno == 1:
                genderparams.append("|g=%s" % g if headtemp_is_head else "||%s" % g if no_param1 else "|%s" % g)
            else:
                genderparams.append("|g%s=%s" % (genderno, g))

        # 3. Synthesize headword template.
        new_headword_template = "{{%s%s%s%s}}" % (headtemp, "".join(headparams), "".join(genderparams), headtemp_param)

        # 4. Synthesize definition template.
        new_defn_template = "{{%s%s|%s%s%s}}" % (
            deftemp[0],
            "|ru" if deftemp_needs_lang else "",
            lemma,
        ...
        if is_adjectival_participle:
            ...
            for infl, infltr in inflections:
                part_short_decl = (
                    ("a(2),dated-c(2)" if past_f_end_stressed else "a(2)")
                    if re.search("(е|а́|а|я́|я)нный$", infl)
                    else (
                        "b(2)"
                        if infl.endswith("ённый")
                        else (
                            "c"
                            if infl.endswith("тый") and past_f_end_stressed
                            else "a" if re.search("[мт]ый$", infl) else "-"
                        )
                    )
                )
            ...
            if len(set(part_short_decls)) == 1:
                if len(part_short_decls) > 1:
                    pagemsg("Combining multiple inflections %s into single ru-decl-adj" % joined_infls_with_tr())
                param1 = ",".join(combine_adj_infl_and_tr(infl, infltr) for infl, infltr in inflections)
                new_decl_template_parts.append("{{ru-decl-adj|%s|%s}}\n" % (param1, part_short_decls[0]))
            else:
                if len(part_short_decls) > 1:
                    warn(
                        "Unable to combine multiple inflections %s into single ru-decl-adj because of differing short decls %s"
                        % joined_infls_with_tr(),
                        " ".join(part_short_decls),
                    )
                for part_short_decl, (infl, infltr) in zip(part_short_decl, inflections):
                    new_decl_template_parts.append(
            ...
            if not m:
                ...
                if args.overwrite_page:
                    if pagetitle in pages_already_erased:
                        ...
                        return
                    elif pagetitle in lemmas_to_not_overwrite:
                        ...
                        # Preserve {{also|...}}
                        sections[i] = re.sub(r"^((\s*\{\{also\|.*?\}\}\s*)?).*$", r"\1", sections[i], 0, re.S)
                        pages_already_erased.add(pagetitle)
                ...
                for t in parsed.filter_templates():
                    ...
                    if not deftemp_needs_lang or compare_param(t, lang_param, "ru", None, param_is_head=False):
                        for param in t.params:
                            pnam = pname(param)
                            pvalue = str(param.value)
                            if (
                                pnam not in ["1", "2"]
                                and not (lang_in_1 and pnam == "3")
                ...
                if is_lemma_template:
                    ...
                    for t in parsed.filter_templates():
                        if is_lemma_template(t):
                            ...
                            if is_noun_form:
                                tnam = tname(t)
                                if tnam in ["ru-noun", "ru-proper noun"] and any(
                                    [re.search(r"\bp\b", x) for x in blib.fetch_param_chain(t, "2", "g")]
                                ):
                    ...
                    for pllemma in pllemmas:
                        ...
                        for lemma1, lemma2 in allow_in_same_etym_section:
                            if lemma1 == pllemma and lemma2 == sglemma:
                                pagemsg(
                                    "Found plurale tantum lemma and creating plural noun form, found the pair in allow_in_same_etym_section"
                                )
                                is_known_about = True
                                break
                        if not is_known_about:
                            for lemma1, lemma2 in not_in_same_etym_section:
                                if lemma1 == pllemma and lemma2 == sglemma:
                                    pagemsg(
                                        "Found plurale tantum lemma and creating plural noun form, found the pair in not_in_same_etym_section"
                                    )
                                    is_known_about = True
                                    break
                        if not is_known_about:
                            warn(
                                "Found plurale tantum lemma and creating plural noun form, might need to add lemmas to allow_in_same_etym_section or not_in_same_etym_section"
                            )
                ...
                for process_section_pass in range(4):
                    ...
                    for j in range(2, len(subsections), 2):
                        if re.match("^===+%s===+\n" % pos, subsections[j - 1]):
                            ...
                            # will be left alone.)
                            def check_fix_noun_adj_gender(
                                headword_template, gender, singular_in_existing_defn_templates, fix_incompat
                            ):
                                def gender_compatible(existing, new):
                                    ...
                                    if existing_mf and new_mf and existing_mf != new_mf:
                                        warn("Can't modify mf gender from %s to %s" % (existing_mf, new_mf))
                                    ...
                                    if existing_an and new_an and existing_an != new_an:
                                        warn("Can't modify animacy from %s to %s" % (existing_an, new_an))
                                    ...
                                    if existing_p and new_p and existing_p != new_p:
                                        warn("Can't modify plurality from %s to %s" % (existing_p, new_p))
                                ...
                                existing_genders = blib.fetch_param_chain(headword_template, first_gender_param, "g")
                                        ...
                                        for paramno, existing in enumerate(existing_genders):
                                            ...
                                            if new_gender:
                                                ...
                                                if existing != new_gender:
                                                    newparam = (
                                                        first_gender_param if paramno == 0 else ("g%s" % (paramno + 1))
                                                    )
                                                    pagemsg(
                                                        "Modifying gender param %s from %s to %s"
                                                        % (newparam, existing, new_gender)
                                                    )
                                                    notes.append("modify gender from %s to %s" % (existing, new_gender))
                                            ...
                                            warn(
                                                "Unable to modify %s in existing genders %s to match new gender %s"
                                                % (",".join(harmonization_problems), ",".join(existing_genders), g)
                                            )
                                    if not found_compat:
                                        assert fix_incompat
                                        newparam = blib.append_param_to_chain(
                                            headword_template, g, first_gender_param, "g"
                                        )
                                        pagemsg("Adding new gender param %s=%s" % (newparam, g))
                            ...
                            def check_fix_infl_params(
                                ...
                                if gender:
                                    if is_noun_or_adj:
                                        if not check_fix_noun_adj_gender(
                                            headword_template,
                                            gender,
                                            singular_in_existing_defn_templates,
                                            fix_incompat_gender,
                                ...
                                if is_verb_form:
                                    ...
                                    if blib.remove_param_chain(headword_template, first_gender_param, "g"):
                                        subsections[j] = str(parsed)
                                        sections[i] = "".join(subsections)
                            ...
                            def split_inflection_tag_sets(tags):
                                ...
                                for tag in tags:
                                    if tag == ";":
                                        ...
                                        if ";" in tag:
                                            warn("Found semicolon in tag '%s' in tags %s" % (tag, "|".join(tags)))
                                ...
                                for i, tag in enumerate(ts):
                                    if "//" in tag:
                                        ...
                                        for single_tag in single_tags:
                                            tag_sets.extend(
                                                split_multipart_tag_set(pre_tags + [single_tag] + post_tags)
                                            )
                                        return tag_sets
                                return [ts]
                            ...
                            def check_for_given_inflection_tag(parsed, tag):
                                for t in parsed.filter_templates():
                                    ...
                                    lang_in_1 = deftemp_needs_lang and not t.has("lang")
                                    lang_param = lang_in_1 and "1" or "lang"
                                    if not deftemp_needs_lang or compare_param(
                                        t, lang_param, "ru", None, param_is_head=False, issue_warnings=issue_warnings
                                    ):
                                        for param in t.params:
                                            pnam = pname(param)
                                            pvalue = str(param.value)
                            ...
                            def compare_inflections(t, infls, issue_warnings=True):
                                ...
                                for param in t.params:
                                    ...
                                    if (
                                        name not in ["1", "2"]
                                        and not (lang_in_1 and name == "3")
                                        and re.search("^[0-9]+$", name)
                                        and value
                                    ):
                                        tags.append(value)
                                tag_sets = split_inflection_tag_sets(tags)
                                ...
                                for tag_set_no, split_tag_set_group in enumerate(split_tag_sets):
                                    for indiv_tag_set in split_tag_set_group:
                                        if set(indiv_tag_set) > inflset:
                                            warn_if(
                                                issue_warnings,
                                                "Found actual inflection %s in template %s whose codes are a superset of intended codes %s, accepting"
                                        ...
                                        if indiv_set < inflset:
                                            ...
                                            if (indiv_set | {"pfv"}) == inflset or (indiv_set | {"impfv"}) == inflset:
                                                if len(split_tag_set_group) == 1:
                                                    pagemsg_if(
                                                        issue_warnings,
                                                        "Need to update actual inflection %s in template %s with intended codes %s"
                                                        % ("|".join(indiv_tag_set), str(t), "|".join(infls)),
                                                    )
                                                    ...
                                                    warn_if(
                                                        issue_warnings,
                                                        "Found actual inflection %s in template %s whose codes are a subset of intended codes %s and could update aspect except that multipart tags are present"
                                                        % ("|".join(indiv_tag_set), str(t), "|".join(infls)),
                                                    )
                                return False, 0
                            ...
                            infl_headword_templates_for_already_present_entry = [
                                t
                                for t in parsed.filter_templates()
                                if template_name(t) == headtemp
                                and template_head_matches(
                                    t, inflections, "checking for already-present entry", issue_warnings=issue_warnings
                                )
                            ]
                            ...
                            def check_for_closely_related_lemma(otherlemma, lemma, issue_warnings=True):
                                ...
                                for lemma1, lemma2 in allow_defn_in_same_subsection:
                                    if (
                                        lemma1 == otherlemma
                                        and lemma2 == lemma
                                        or lemma2 == otherlemma
                                        and lemma1 == lemma
                                    ):
                                        pagemsg_if(
                                            issue_warnings,
                                            "Allowing lemma %s to share headword with lemma %s because in allow_defn_in_same_subsection"
                                            % (lemma, otherlemma),
                                        )
                                        return True
                                return False
                            ...
                            for t in parsed.filter_templates():
                                ...
                                if compare_param(
                                    t, lemma_param, lemma, lemmatr, param_is_head=True, issue_warnings=issue_warnings
                                ) and (
                                    not deftemp_needs_lang
                                    or compare_param(
                                        t, lang_param, "ru", None, param_is_head=False, issue_warnings=issue_warnings
                                    )
                                ):
                                    ...
                                    if isinstance(deftemp_param, str):
                                        ...
                                        result, tag_set_no = compare_inflections(
                                            t, deftemp_param, issue_warnings=issue_warnings
                                        )
                                        if result == "update":
                                ...
                                elif check_for_closely_related_lemma(
                                    getparam(t, lemma_param), lemma, issue_warnings=issue_warnings
                                ) and (
                                    not deftemp_needs_lang
                                    or compare_param(
                                        t, lang_param, "ru", None, param_is_head=False, issue_warnings=issue_warnings
                            ...
                            # FIXME, should this check the lemma?
                            singular_in_existing_defn_templates = check_for_given_inflection_tag(parsed, "s")

                            # Make sure there's exactly one headword template.
                            if (
                                len(infl_headword_templates_for_already_present_entry) > 1
                                or len(infl_headword_templates_for_inserting_in_same_section) > 1
                            ):
                                warn(
                                    "Found multiple inflection headword templates for %s; taking no action" % (infltype)
                                )
                            ...
                            def insert_part_decl_if_needed():
                                ...
                                while check_subsection < len(subsections):
                                    ...
                                    if "==Declension==" in subsections[check_subsection]:
                                        ...
                                        if len(decl_templates) != len(new_decl_template_parts):
                                            warn(
                                                "Found %s existing participial decl templates %s but want %s new decl templates %s"
                                                % (
                                                    len(decl_templates),
                                        ...
                                        for k in range(len(decl_templates)):
                                            ...
                                            if str(declt) != str(newdeclt):
                                                if getparam(declt, "1") == getparam(newdeclt, "1"):
                                                    ...
                                                    if (
                                                        shortdecl == "a(2)"
                                                        and newshortdecl == "a(2),dated-c(2)"
                                                        or shortdecl == ""
                                                        and newshortdecl == "-"
                                                    ):
                                                        pagemsg(
                                                            "Updated participle short decl from %s to %s"
                                                            % (shortdecl, newshortdecl)
                                                        )
                                                        notes.append(
                                                            "update participle short decl from %s to %s"
                                                            % (shortdecl, newshortdecl)
                                                        )
                                                        ...
                                                        continue
                                                warn(
                                                    "Existing decl %s and wanted decl %s differ and can't fix"
                                ...
                                pagemsg(
                                    "Inserting declension in existing entry: %s %s, %s %s"
                            ...
                            if (
                                infl_headword_templates_for_already_present_entry
                                and defn_templates_for_already_present_entry
                                and process_section_pass in [0, 1]
                            ):
                                if process_section_pass == 0:
                                    pagemsg("Exists and has Russian section and found %s already in it" % (infltype))
                                # Maybe fix up auxiliary parameters (e.g. gender) in the
                                # headword template.
                                if check_fix_infl_params(
                                    infl_headword_templates_for_already_present_entry[0],
                                    [],
                                    gender,
                                    singular_in_existing_defn_templates,
                            ...
                            # in same section.
                                ...
                                infl_headword_templates_for_inserting_in_same_section
                                and defn_templates_for_inserting_in_same_section
                                and process_section_pass in [2, 3]
                            ):
                                ...
                                # new entry.
                                if check_fix_infl_params(
                                    infl_headword_templates_for_inserting_in_same_section[0],
                                    [],
                                    gender,
                                    singular_in_existing_defn_templates,
                                    ...
                                    if (
                                        deftemp_allows_multiple_tag_sets
                                        and len(defn_templates_for_inserting_in_same_template) > 0
                                        and
                                        ...
                                        pagemsg(
                                            "Insert new tag set into existing {{%s}}" % (tname(defn_template_to_modify))
                                        )
                                        ...
                                        if re.search(
                                            r"^# \{\{%s\|" % construct_alternant_re(deftemp), subsections[j], re.M
                                        ):
                                            if not subsections[j].endswith("\n"):
                                                subsections[j] += "\n"
                                            subsections[j] = check_re_sub(
                                                warn,
                                                "inserting definition into existing section",
                                        ...
                                        sections[i] = "".join(subsections)
                                        pagemsg(
                                            "Insert new defn with {{%s}} at beginning after any existing such defns"
                                            % (deftemp[0])
                                        )
                                        inserted = insert_part_decl_if_needed()
                                        comment = (
                                            "%s new defn with {{%s}} at beginning after any existing such defns: %s %s, %s %s"
                                            % (
                                                (
                                                    "Insert declension in existing entry and insert"
                                                    if inserted
                                                    else "Insert"
                                                ),
                                                deftemp[0],
                ...
                # else of for loop over passes 0-3, i.e. no break out of loop
                    ...
                    def insert_new_text_before_section(insert_at, secbefore_desc, matching):
                        pagemsg(
                            "Found section to insert %s before: [[%s]]" % (generic_infltype, subsections[insert_at + 1])
                        )
                        ...
                        comment = "Insert entry for %s %s of %s before %s" % (infltype, joined_infls, lemma, secmsg)
                        if insert_at > 0:
                    ...
                    def insert_new_text_after_section(insert_at, secafter_desc, matching):
                        pagemsg(
                            "Found section to insert %s after: [[%s]]" % (generic_infltype, subsections[insert_at - 1])
                        )
                        ...
                        comment = "Insert entry for %s %s of %s after %s" % (infltype, joined_infls, lemma, secmsg)
                        subsections[insert_at - 1] = ensure_two_trailing_nl(subsections[insert_at - 1])
                        if indentlevel == 3:
                            subsections[insert_at:insert_at] = [newpos + "\n"]
                        else:
                    ...
                    if is_adjectival_participle:
                        ...
                        for j in range(2, len(subsections), 2):
                            if re.match("^===+(Noun|Adjective)===+", subsections[j - 1]):
                                ...
                                for t in parsed.filter_templates():
                                    if (
                                        tname(t)
                                        in ["ru-adj", "ru-noun", "ru-proper noun", "ru-noun+", "ru-proper noun+"]
                                        and template_head_matches(
                                            t,
                                            inflections,
                                            "checking for existing noun/adjective with headword matching adjectival participle",
                                        )
                        ...
                        if insert_at is not None:
                            comment = insert_new_text_before_section(insert_at, "noun/adjective", "headword")
                            break
                    ...
                    if is_adverbial_participle:
                                ...
                                for t in parsed.filter_templates():
                                    if (
                                        (
                                            ...
                                            or tname(t) == "head"
                                            and getparam(t, "1") == "ru"
                                            and getparam(t, "2") == "preposition"
                                        )
                                        and template_head_matches(
                                            t,
                                            inflections,
                                            "checking for existing adverb/preposition with headword matching adverbial participle",
                                        )
                                        and insert_at is None
                                    ):
                                        insert_at = j - 1
                        ...
                        if insert_at is not None:
                            comment = insert_new_text_before_section(insert_at, "adverb/preposition", "headword")
                            break
                        ...
                        for j in range(2, len(subsections), 2):
                            if re.match("^===+Participle===+", subsections[j - 1]):
                                ...
                                for t in parsed.filter_templates():
                                    if (
                                        tname(t) == "head"
                                        and getparam(t, "1") == "ru"
                                        and getparam(t, "2") == "participle form"
                                        and template_head_matches(
                                            t,
                                            inflections,
                                            "checking for existing participle with headword matching adjective",
                                        )
                                    ):
                                        insert_at = j + 1
                            if is_short_adj_form:
                                if re.match("^===+Adverb===+", subsections[j - 1]):
                                    parsed = blib.parse_text(subsections[j])
                                    for t in parsed.filter_templates():
                                        if tname(t) in ["ru-adv"] and template_head_matches(
                                            t,
                                            inflections,
                                            "checking for existing adverb with headword matching short adjective",
                                        ):
                                            insert_at = j + 1
                                    ...
                                    for t in parsed.filter_templates():
                                        if (
                                            tname(t) == "head"
                                            and getparam(t, "1") == "ru"
                                            and getparam(t, "2") == "predicative"
                                            and template_head_matches(
                                                t,
                                                inflections,
                                                "checking for existing predicative with headword matching short adjective",
                                            )
                                        ):
                                            insert_at = j + 1
                        if insert_at:
                            comment = insert_new_text_after_section(
                                insert_at,
                                "adverb/predicative/participle form" if is_short_adj_form else "participle form",
                                "headword",
                            )
                            break
                    ...
                    def check_for_matching_sg_pl_pair(otherlemma, lemma, first_only=False):
                        otherlemma = rulib.remove_accents(otherlemma)
                        lemma = rulib.remove_accents(lemma)
                        for lemma1, lemma2 in allow_in_same_etym_section:
                            if (
                                ...
                                and lemma2 == lemma
                                or not first_only
                                and lemma2 == otherlemma
                                and lemma1 == lemma
                            ):
                                pagemsg(
                                    "Allowing new subsection for lemma %s in same etym section as lemma %s because in allow_in_same_etym_section list"
                                    % (lemma, otherlemma)
                                )
                                return True
                        return False

                    def matching_defn_templates(parsed, allow_stress_mismatch=False, check_for_sg_pl_pairs=False):
                        retval = []
                        for t in parsed.filter_templates():
                            if tname(t) not in deftemp:
                                continue
                            ...
                            if (
                                compare_param(
                                    ...
                                    lemmatr,
                                    param_is_head=True,
                                    allow_stress_mismatch=allow_stress_mismatch,
                                )
                                or check_for_sg_pl_pairs
                                and check_for_matching_sg_pl_pair(getparam(t, lemma_param), lemma)
                            ) and (
                                not deftemp_needs_lang or compare_param(t, lang_param, "ru", None, param_is_head=False)
                            ):
                                retval.append(t)
                        return retval
                    ...
                    if is_adj_form:
                        ...
                        for j in range(2, len(subsections), 2):
                            if re.match("^===+Noun===+", subsections[j - 1]):
                                ...
                                for t in parsed.filter_templates():
                                    if (
                                        tname(t) in ["ru-noun form"]
                                        and template_head_matches(
                                            t,
                                            inflections,
                                            "checking for existing noun form with headword and defn matching adj form",
                                        )
                                        and defn_templates
                                        and insert_at is None
                                    ):
                        ...
                        if insert_at is not None:
                            comment = insert_new_text_before_section(insert_at, "noun form", "headword and definition")
                            break
                    ...
                    if is_noun_form:
                            ...
                            if re.match("^===+Adjective===+", subsections[j - 1]):
                                ...
                                for t in parsed.filter_templates():
                                    if (
                                        tname(t) == "head"
                                        and getparam(t, "1") == "ru"
                                        and getparam(t, "2") == "adjective form"
                                        and template_head_matches(
                                            t,
                                            inflections,
                                            "checking for existing adj form with headword and defn matching noun form",
                                        )
                                        and defn_templates
                                    ):
                                        insert_at = j + 1
                        ...
                        if insert_at:
                            comment = insert_new_text_after_section(insert_at, "adj form", "headword and definition")
                            break
                    ...
                    if is_noun_form:
                                ...
                                for t in parsed.filter_templates():
                                    if tname(t) in ["ru-noun+", "ru-proper noun+"]:
                                        ...
                                        if otherlemmaarg is None:
                                            warn(
                                                "Error generating noun forms when finding plurale tantum noun lemma for paired lemma in same etym section"
                                            )
                                        else:
                                            otherlemmas = set(re.split(",", otherlemmaarg))
                                            for otherlemma in otherlemmas:
                                                if check_for_matching_sg_pl_pair(otherlemma, lemma, first_only=True):
                                                    insert_at = j + 1
                        ...
                        if insert_at:
                            comment = insert_new_text_after_section(
                                insert_at, "plurale tantum noun", "lemma pair in allow_in_same_etym_section"
                            )
                            break
                    ...
                    for j in range(2, len(subsections), 2):
                        if re.match("^===+%s===+\n" % pos, subsections[j - 1]):
                            parsed = blib.parse_text(subsections[j])
                            defn_templates = matching_defn_templates(
                                parsed,
                    ...
                    if insert_at:
                        comment = insert_new_text_after_section(insert_at, generic_infltype, "definition")
                        break
                    ...
                    if is_noun_form and is_noun_adj_plural:
                        ...
                        for j in range(2, len(subsections), 2):
                            if re.match("^===+Noun===+", subsections[j - 1]):
                                ...
                                # (with the correct language) in the subsection.
                                # FIXME, should this check the lemma?
                                plural_in_existing_defn_templates = check_for_given_inflection_tag(parsed, "p")
                                ...
                                for t in parsed.filter_templates():
                                    if (
                                        tname(t) in ["ru-noun form"]
                                        and template_head_matches(t, inflections, "checking for plural noun form")
                                        and (
                                            plural_in_existing_defn_templates
                                            or any(
                                                [re.search(r"\bp\b", y) for y in blib.fetch_param_chain(t, "2", "g")]
                                            )
                                        )
                                    ):
                        ...
                        if found_plural_noun_form or found_plurale_tantum_lemma:
                            warn(
                                "Creating new etymology for plural noun form and found existing plural noun form or noun lemma"
                            )
                    ...
                    if not sections[i]:  # Erased section
                        ...
                        comment = (
                            "Wrap existing Russian section in Etymology 1, append entry (Etymology 2) for %s %s of %s, pos=%s"
                            % (infltype, joined_infls, lemma, pos)
                        )

                        for j in range(2, len(subsections), 2):
                            if re.match("^===+Etymology===+\n", subsections[j - 1]):
                                pagemsg("Found Etymology section at position %s-%s" % (j - 1, j))
                                # Found etymology section; if there is a preceding section
                                # such as Alternative forms, put the etymology section above
                                # it.
                                if j > 2:
                                    pagemsg(
                                        "Found Etymology section at position %s-%s, below other sections, moving up"
                                        % (j - 1, j)
                                    )
...
def adj_form_inflection_list(old, special, has_nom_mp):
    return [
        ...
        ["acc_p_an", [("an", "acc", "f", "p"), ("an", "acc", "n", "p")] if old and has_nom_mp else ("an", "acc", "p")],
        ["acc_p_in", [("in", "acc", "f", "p"), ("in", "acc", "n", "p")] if old and has_nom_mp else ("in", "acc", "p")],
        ...
        # old=1
        ["acc_mp_an", [("an", "acc", "m", "p"), ("an", "acc", "n", "p")] if special else ("an", "acc", "m", "p")],
        ["acc_mp_in", [("in", "acc", "m", "p"), ("in", "acc", "n", "p")] if special else ("in", "acc", "m", "p")],
...
noun_form_aliases = {
    ...
    "sg": ["nom_sg", "gen_sg", "dat_sg", "acc_sg", "acc_sg_an", "acc_sg_in", "ins_sg", "pre_sg"],
    "pl": ["nom_pl", "gen_pl", "dat_pl", "acc_pl", "acc_pl_an", "acc_pl_in", "ins_pl", "pre_pl"],
}
    ...
    for t in parsed.filter_templates():
        ...
        if new_genders:
            ...
            if genders_seen and new_genders != genders_seen:
                pagemsg(
                    "WARNING: Multiple conflicting gender specs in headwords, found both %s and %s"
                    % (",".join(genders_seen), ",".join(new_genders))
                )
...
def find_inflection_templates(
    text, expected_header, expected_poses, skip_poses, is_inflection_template, find_gender, pagemsg, expand_text
):
    templates = []
        ...
        if sections[i - 1] == "==Russian==\n":
            ...
            for j in range(2, len(subsections), 2):
                ...
                if levelno - last_levelno > 1:
                    pagemsg(
                        "WARNING: Misformatted header level (jump by %s - %s = %s, in section %s)"
                ...
                for t in parsed.filter_templates():
                    if is_inflection_template(t) and not getparam(t, "old").strip():
                        if header != expected_header:
                            pagemsg(
                                "WARNING: Expected inflection template under %s header but instead found under %s header"
                                % (expected_header, header)
                            )
                        pos_header = headers_at_level.get(levelno - 1, None)
                        if pos_header and pos_header not in expected_poses:
                            pagemsg("WARNING: Inflection template under unexpected part of speech %s" % pos_header)
                        if pos_header not in skip_poses:
                            templates.append((t, latest_genders))
                        else:
                            pagemsg(
                                "Skipping inflection template because under part of speech %s: %s"
                                % (pos_header, str(t))
                            )
    return templates
...
def split_forms_with_stress_variants(formvals, forms_desired, dicforms, pagemsg, expand_text):
    ...
    if re.search("[аяеоыи]́?$", dicform1):
        dicform1_reduced_stem = expand_text(
            "{{#invoke:ru-common|dereduce_stem|%s||%s}}"
            % (dicform1_stem_for_reduce, "y" if dicform1_epenthetic_vowel else "")
        )
        dicform2_reduced_stem = expand_text(
            "{{#invoke:ru-common|dereduce_stem|%s||%s}}"
        ...
        for formval in formvals:
            ...
            for formregex, lemma in manual_split_form_list:
                if re.search(formregex, formval):
                    pagemsg("Found matching manually specified form regex %s, lemma %s" % (formregex, lemma))
                    formlemma = lemma
                    break
            if formlemma:
                if formlemma == dicform1:
                    ...
                    pagemsg("WARNING: Lemma %s doesn't match either lemma %s or %s" % (formlemma, dicform1, dicform2))
...
def create_forms(
    ...
    if lemmas_to_process and lemmas_no_jo:
        ...
        pages_to_process = (
            (index, pywikibot.Page(site, page)) for index, page in blib.iter_items(pagetitles_to_process, start, end)
        )
    elif lemmas_to_process:
        pages_to_process = (
            (index, pywikibot.Page(site, page)) for index, page in blib.iter_items(lemmas_to_process, start, end)
        )
    else:
    ...
    for index, page in pages_to_process:
        ...
        inflection_templates = find_inflection_templates(
            page.text,
            expected_header,
            expected_poses,
            skip_poses,
            is_inflection_template,
            get_headword_noun_gender if pos == "noun" and pagetitle not in ignore_headword_gender else None,
            pagemsg,
            expand_text,
        )
        ...
        for infltemp, headword_gender in inflection_templates:
            ...
            for dicform_code in dicform_codes:
                ...
                pagemsg(
                    "WARNING: No dictionary form available among putative codes %s, skipping" % ",".join(dicform_codes)
                )
                continue
            ...
            dicforms = [
                (rulib.remove_monosyllabic_accents(dicru), rulib.remove_tr_monosyllabic_accents(dictr))
                for dicru, dictr in dicforms
            ]
                        ...
                        if formname != dicform_code and formname in split_args and split_args[formname]:
                            ...
                            if formname + "_raw" in split_args:
                                ...
                                if split_args[formname] != raw_form:
                                    pagemsg(
                                        "WARNING: Raw form %s=%s contains footnote symbol (notes=%s)"
                                        % (
                                            formname,
                                            split_args[formname + "_raw"],
                                            infltemp.has("notes")
                                            and "<%s>" % getparam(infltemp, "notes")
                                            or "NO NOTES",
                                        )
                                    )
                            ...
                            formvals_by_pagetitle = OrderedDict()
                            formvals = re.split(",", split_args[formname])
                            if len(formvals) > 1:
                                pagemsg(
                                    "create_forms: Found multiple form values for %s=%s, dictionary form %s%s"
                                    % (
                                        formname,
                                        split_args[formname],
                                        dicformru,
                                        dicformtr and " (%s)" % dicformtr or "",
                                    )
                                )
                            for formval in formvals:
                                ...
                                if skip_inflections and skip_inflections(formname, formvalru, formvaltr):
                                    pagemsg(
                                        "create_forms: Skipping %s=%s%s"
                                        % (formname, formvalru, formvaltr and " (%s)" % formvaltr or "")
                                    )
                                elif formval_no_accents in formvals_by_pagetitle:
                                    formvals_by_pagetitle[formval_no_accents].append((formvalru, formvaltr))
                                else:
                                    formvals_by_pagetitle[formval_no_accents] = [(formvalru, formvaltr)]
                            # Process groups of inflections
                            formvals_by_pagetitle_items = list(formvals_by_pagetitle.items())
                            if len(formvals_by_pagetitle_items) > 1:
                                pagemsg(
                                    "create_forms: For form %s, found multiple page names %s"
                                    % (
                                        formname,
                                        ",".join(
                                            "%s" % formval_no_accents
                                            for formval_no_accents, inflections in formvals_by_pagetitle_items
                                        ),
                                    )
                                )
                            for formval_no_accents, inflections in formvals_by_pagetitle_items:
                                inflections = [
                                    (
                                        rulib.remove_monosyllabic_accents(infl),
                                        rulib.remove_tr_monosyllabic_accents(infltr),
                                    )
                                    for infl, infltr in inflections
                                ]
                                inflections_printed = ",".join(
                                    "%s%s" % (infl, " (%s)" % infltr if infltr else "") for infl, infltr in inflections
                                )

                                if formval_no_accents == rulib.remove_accents(dicformru):
                                    pagemsg(
                                        "create_forms: Skipping form %s=%s because would go on lemma page"
                                        % (formname, inflections_printed)
                                    )
                                else:
                                    if len(inflections) > 1:
                                        pagemsg(
                                            "create_forms: For pagetitle %s, found multiple inflections %s"
                                    ...
                                    if type(inflsets) is not list:
                                        inflsets = [inflsets]
                                    form_gender = headword_gender or (
                                        get_gender(infltemp, formname, split_args) if get_gender else []
                                    )
                                    ...
                                    if formname == "past_pasv_part" and form_gender == ["impf"]:
                                        ...
                                        for infl, infltr in inflections:
                                            if pppp_set and infl in pppp_set:
                                                pagemsg(
                                                    "create_forms: Skipping imperfective past passive participle %s%s because in perfective past passive participle list"
                                    ...
                                    for inflset in inflsets:
                                        ...
                                        if (
                                            pos == "noun"
                                            and multiple_noun_animacies
                                            and "an" not in inflset
                                            and "in" not in inflset
                                            and (
                                                formname == "acc_pl"
                                                or formname == "acc_sg"
                                                and [re.search(r"\bm\b", g) for g in inflset_gender]
                                                and (
                                                    split_args["acc_sg"] == split_args.get("nom_sg", None)
                                                    or split_args["acc_sg"] == split_args.get("gen_sg", None)
                                                )
                                            )
                                        ):
                                            ...
                                            if found_animate and found_inanimate:
                                                pagemsg(
                                                    "WARNING: Something wrong, lemma is bianimate (gender=%s) and animacy codes not inflset %s"
                                                    % (",".join(inflset_gender), "|".join(inflset))
                                                )
                                            elif found_animate:
                                                newinflset = ("an",) + inflset
                                                pagemsg(
                                                    "Multiple noun animacies, adding animate form code, modifying inflset from %s to %s"
                                                ...
                                                pagemsg(
                                                    "Multiple noun animacies, adding inanimate form code, modifying inflset from %s to %s"
                                                    % ("|".join(inflset), "|".join(newinflset))
                                                )
                                                inflset = newinflset
                                        ...
                                        if pos == "noun" and formname == "loc":
                                            ...
                                            def frob_locative(ru, tr):
                                                ...
                                                if ru != newru:
                                                    pagemsg("Modifying locative from %s to %s" % (ru, newru))
                                                    ru = newru
                                                if tr:
                                                    tr = re.sub("^(v|na|v/na|na/v) ", "", tr)
                                                return ru, tr

                                            inflections = [frob_locative(ru, tr) for ru, tr in inflections]
                                            ...
                                            for ru, tr in inflections:
                                                if " " in ru and " " not in dicformru:
                                                    pagemsg(
                                                        "WARNING: Space in locative %s but not in lemma %s, skipping"
                                                        % (ru, dicformru)
                                                    )
                                                    skip_locative = True
                                                    break
                                        ...
                                        if pos == "verb" and "part" in inflset:
                                            ...
                                            if "pres" in inflset:
                                                ...
                                                if "past" not in inflset:
                                                    pagemsg(
                                                        "WARNING: Something wrong, neither 'pres' nor 'past' in participle inflset: %s"
                                                        % (",".join(inflset))
                                                    )
                                                headtemp_tense = "past"
                                            if "act" in inflset:
                                                ...
                                                if "pass" not in inflset:
                                                    pagemsg(
                                                        "WARNING: Something wrong, none of 'act', 'pass' or 'adv' in participle inflset: %s"
                                                        % (",".join(inflset))
                                                    )
                                            ...
                                            if "past_f" in split_args:
                                                ...
                                                if saw_non_end_stressed_past_f and saw_end_stressed_past_f:
                                                    pagemsg(
                                                        "WARNING: Saw both ending-stressed and non-ending-stressed past_f when determining short participle type: %s"
                                                        % split_args["past_f"]
                                                    )
...
def generate_adj_forms(t, expand_text):
    if tname(t) == "ru-decl-adj":
        ...
        return expand_text(re.sub(r"^\{\{ru-decl-adj-irreg\s*\|", r"{{ru-generate-adj-forms|-|manual|", str(t)))
...
def create_adj_forms(
    ...
    create_forms(
        ...
        "Declension",
        ["Adjective", "Participle", "Pronoun", "Proper noun"],
...
def generate_numeral_noun_forms(t, expand_text):
    if tname(t) == "ru-noun-table":
        ...
        temp_to_expand = (
            "{{ru-noun-table|a|%s|manual|a=%s|nom_sg=%s|nom_pl=%s|gen_sg=%s|gen_pl=%s|dat_sg=%s|dat_pl=%s|acc_sg=%s|acc_pl=%s|ins_sg=%s|ins_pl=%s|pre_sg=%s|pre_pl=%s|loc=%s|voc=%s|notes=%s}}"
            % (
                getparam(t, "1"),
                getparam(t, "a") or "bi",
                getparam(t, "1"),
        ...
        temp_to_expand = (
            "{{ru-noun-table|a|%s|manual|a=%s|n=sg|nom_sg=%s|gen_sg=%s|dat_sg=%s|acc_sg=%s|ins_sg=%s|pre_sg=%s|loc=%s|voc=%s}}"
            % (
                getparam(t, "1"),
                getparam(t, "a") or "bi",
                getparam(t, "1"),
    ...
    return expand_text(re.sub(r"^\{\{ru-noun-table", "{{ru-generate-noun-args", temp_to_expand))
...
parser.add_argument(
    "--perfective-past-passive-participles",
    "--pppp",
...
parser.add_argument(
    "--numeral", action="store_true", help="""If specified, create numeral forms instead of noun/adj forms."""
)
parser.add_argument(
...
if args.perfective_past_passive_participles:
    pppp_set = set(blib.fetch_items_from_file(args.perfective_past_passive_participles))
else:
    pppp_set = None
if args.adj_form:
    function_to_call = (
