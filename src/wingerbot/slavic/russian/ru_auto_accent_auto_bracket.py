...
monosyllabic_accented_prepositions = [prep + AC for prep in monosyllabic_prepositions]
...
ignore_capitalized_variant_words = ["я", "в", "с", "к", "о", "а", "у", "и", "вы", "по"]
...
def lookup_term_for_accents(term, termtr, verbose, pagemsg, expect_cap):
    ...
    for pagename in pagenames_to_consider:
        ...
        for adj_lemma in adj_lemmas:
            ...
            if adj_cache_result is not None and (adj_cache_result not in ["redirect", "no-russian"]):
        ...
        if AC in maybe_decap_term:  # don't check for ё in case of multiword exprs.
            heads = set((ru, tr, is_lemma) for ru, tr, is_lemma in heads if terms_match(ru, maybe_decap_term))
            new_inflections_of = set()
            for h, l in inflections_of:
                h = frozenset((ru, tr, is_lemma) for ru, tr, is_lemma in h if terms_match(ru, maybe_decap_term))
                if h:
        ...
        def stringize_heads(heads):
            return ",".join(
                "%s%s%s" % (ru, "//%s" % tr if tr else "", "[lemma]" if is_lemma else "") for ru, tr, is_lemma in heads
            )

        def stringize_inflections_of(inflections_of):
            return "; ".join("%s:%s" % (stringize_heads(heads), lemma) for heads, lemma in inflections_of)

        cached_msg = " (cached)" if cached is True else " (manual override)" if cached == "manual-override" else ""
        ...
        if len(heads) == 0:
            ...
            if cache_result is not None and (cache_result not in ["redirect", "no-russian"]):
        ...
        if len(heads) > 1:
            ...
            if len(heads_ignoring_lemma) == 1:
                pagemsg(
                    "lookup_term_for_accents: Found multiple heads for %s%s but all match except some are lemmas and some non-lemmas: %s"
                ...
                pagemsg(
                    "WARNING: lookup_term_for_accents: Found multiple heads for %s%s: %s"
                    % (pagename, cached_msg, stringize_heads(heads))
                )
                return term, termtr, None
        else:
        ...
        if pagename != real_pagename:
            ...
            if newtr:
                ...
                newtr = rulib.recompose(
                    real_pagename_first_letter_translit + newtr[len(real_pagename_first_letter_translit) :]
                )

        if semi_verbose:
            pagemsg(
                "lookup_term_for_accents: Found head %s%s%s" % (newterm, "//%s" % newtr if newtr else "", cached_msg)
            )
        ...
        if re.search("[!?]$", newterm) and not re.search("[!?]$", term):
            ...
            if rulib.remove_accents(newterm_wo_punc) == rulib.remove_accents(term):
                pagemsg(
                    "lookup_term_for_accents: Removing punctuation from %s when matching against %s" % (newterm, term)
                )
        ...
        if rulib.remove_accents(newterm) != rulib.remove_accents(term):
            ...
            # otherwise do so.
            if "ё" in newterm and (rulib.remove_accents(newterm.replace("ё", "е")) == rulib.remove_accents(term)):
                # Allow mismatch in ё vs. е because we handle ru-*-alt-ё templates
                # in lookup_heads_and_inflections.
                pass
            elif cached != "manual-override":
                pagemsg(
                    "WARNING: lookup_term_for_accents: Accented term %s differs from %s in more than just accents%s"
        ...
        if keep_existing:
            ...
            if newtr and termtr and newtr != termtr:
                pagemsg(
                    "WARNING: Existing translit %s//%s and new translit %s//%s don't agree, not changing translit%s"
        ...
        if len(inflections_of) > 0 and saw_lemma:
            ...
            pagemsg(
                "WARNING: lookup_term_for_accents: Found lemma and inflections of one or more lemmas for %s%s: head(s) %s, lemma(s) of which this term is an inflection %s"
                % (pagename, cached_msg, stringize_heads(heads), stringize_inflections_of(inflections_of))
            )
            return newterm, newtr, None
        elif len(inflections_of) > 1:
            # (b) both lemma and non-lemma.
            pagemsg(
                "WARNING: lookup_term_for_accents: Found inflections of multiple lemmas for %s%s: head(s) %s, lemmas of which this term is an inflection %s"
                % (pagename, cached_msg, stringize_heads(heads), stringize_inflections_of(inflections_of))
            )
        else:
...
def find_accented_split_words(
    term, termtr, words, trwords, verbose, pagetitle, pagemsg, template, add_brackets, expect_cap
):
    newterm = term
    newtr = termtr
    ...
    for i in range(1, len(words), 2):
        ...
        if word.count("[") != word.count("]"):
            pagemsg(
                "WARNING: find_accented_split_words: Unbalanced brackets in word #%s %s: %s"
                % (i // 2, word, "".join(words))
            )
            unbalanced = True
            break
        ...
        for i in range(len(words)):
            ...
            if word and i % 2 == 1:
                if i >= 2 and blib.remove_links(words[i - 2]) in monosyllabic_accented_prepositions:
                    ...
                    pagemsg(
                        "find_accented_split_words: Not accenting term %s%s preceded by accented preposition %s"
                        % (word, "//" + trword if trword else "", words[i - 2])
                    )
                    ru = word
                    tr = trword
                    # FIXME! Bracket term as necessary.
                elif i >= 2 and words[i - 1] in ["(", ")", "[", "]"] and re.search(ends_in_cyrillic_re, words[i - 2]):
                    ...
                    pagemsg(
                        "find_accented_split_words: Not accenting partial-word term %s%s in %s%s%s"
                        % (word, "//" + trword if trword else "", words[i - 2], words[i - 1], word)
                    )
                    ru = word
                    tr = trword
                elif (
                    i <= len(words) - 3
                    and words[i + 1] in ["(", ")", "[", "]"]
                    and re.search(begins_in_cyrillic_re, words[i + 2])
                ):
                    # See preceding case.
                    pagemsg(
                        "find_accented_split_words: Not accenting partial-word term %s%s in %s%s%s"
                        % (word, "//" + trword if trword else "", word, words[i + 1], words[i + 2])
                    )
                    ru = word
                    tr = trword
                else:
                    # Otherwise, actually look it up.
                    ru, tr, this_substs = find_accented(
                        word,
                        trword,
                        verbose,
                        pagetitle,
                        ...
                        or (
                            re.search('(> *|“|"|[/.…!:—}]  ?)$', words[i - 1])
                            and not (i >= 2 and len(words[i - 2]) == 1 and words[i - 2].isupper())
                        ),
                    )
                if "," in tr:
                ...
                if tr:
                    if not trwords:
                        if this_substs:
                            ...
                            if m:
                                tr = m.group(1)
                            subst_ru = ru_reverse_translit.reverse_translit(tr, cyrillic=bare_ru)
                            if bare_ru != subst_ru:
                ...
                if trword and word != trword:
                    pagemsg(
                        "WARNING: find_accented_split_words: Separator <%s> at index %s has manual translit <%s> that's different from it: %s"
...
def find_accented_1(term, termtr, verbose, pagetitle, pagemsg, template, add_brackets, expect_cap):
    ...
    if m:
        ...
        # and bracket.
        newterm, newtr, substs = find_accented(
            re.sub("#Russian$", "", m.group(1)), termtr, verbose, pagetitle, pagemsg, template, "outer", expect_cap
        )
        if re.search(r"^\[\[([^\[\]]*)\]\]$", newterm):
            # If term is already bracketed, just return it.
            if "|" in newterm:
                pagemsg(
                    "WARNING: find_accented_1: Already bracketed term %s referencing non-lemma, replacing with lemma reference %s"
                    % (term, newterm)
                )
            return newterm, newtr, substs
        ...
        if m:
            ...
            if newtr:
                ...
                if not mm:
                    pagemsg(
                        "WARNING: find_accented_1: Boldfaced term %s but not corresponding translit %s"
                        % (newterm, newtr)
                    )
                else:
                    newtr = mm.group(1)
    ...
    if m:
        newterm, newtr, substs = find_accented(
            m.group(2), termtr, verbose, pagetitle, pagemsg, template, False, expect_cap
        )
    ...
    if m:
        ...
        if termtr:
            ...
            if not mm:
                pagemsg(
                    "WARNING: find_accented_1: Boldfaced term %s but not corresponding translit %s" % (term, termtr)
                )
            else:
                termtr = mm.group(1)
        newterm, newtr, substs = find_accented(
            m.group(1), termtr, verbose, pagetitle, pagemsg, template, False, expect_cap
        )
    ...
    newterm, newtr, lemma = lookup_term_for_accents(term, termtr, verbose, pagemsg, expect_cap)
    substs = []
    ...
    if newterm == term and newtr == termtr:
        ...
        inner_add_brackets = False if lemma or add_brackets == "outer" else add_brackets
        split_regex = r"('''.*?'''|\[\[.*?\]\]|[^()\[\]{} \u00A0\n—,.…?!:;/\"«»„“”<>]+)"
        words = re.split(split_regex, term)
        trwords = re.split(split_regex, termtr) if termtr else []
        if trwords and len(words) != len(trwords):
            pagemsg(
                "WARNING: find_accented_1: %s Cyrillic words but different number %s translit words: %s//%s"
                % (len(words), len(trwords), term, termtr)
            )
            ...
            if term.startswith("-") or term.endswith("-"):
                ...
                if trwords and len(words) != len(trwords):
                    ...
                    newterm, newtr, substs = find_accented_split_words(
                        term,
                        termtr,
                        words,
                        trwords,
            ...
            newterm, newtr, substs = find_accented_split_words(
                term, termtr, words, trwords, verbose, pagetitle, pagemsg, template, inner_add_brackets, expect_cap
            )
...
def find_accented(term, termtr, verbose, pagetitle, pagemsg, template, add_brackets, expect_cap):
    if semi_verbose:
        pagemsg("find_accented: Call with term %s%s" % (term, "//%s" % termtr if termtr else ""))
    term, termtr, substs = find_accented_1(
        term, termtr, verbose, pagetitle, pagemsg, template, add_brackets, expect_cap
    )
    if semi_verbose:
        pagemsg(
            "find_accented: Return %s%s%s"
            % (term, "//%s" % termtr if termtr else "", ", subst=" + ",".join(substs) if substs else "")
        )
...
def process_template(
    ...
    if find_accents:
        newval, newtr, substs = find_accented(
            val, valtr, verbose, pagetitle, pagemsg, template, str(template.name) in link_expandable_templates, True
        )
        if newval != val or newtr != valtr:
            if ruheadlib.normalize_text(newval) != ruheadlib.normalize_text(val):
                pagemsg(
                    "WARNING: process_template: Accented page %s changed from %s in more than just accents/links/boldface"
                    % (newval, val)
                )
            ...
            if newtr:
                if not trparam:
                    pagemsg(
                        "WARNING: process_template: Unable to change translit to %s because no translit param available (Cyrillic param %s): %s"
                    ...
                    if valtr:
                        if newtr == valtr:
                            pagemsg(
                                "process_template: Replacing tr=%s with subst=%s with the same effect"
                                % (valtr, subst_param)
                            )
                        else:
                            pagemsg(
                                "WARNING: process_template: Replacing non-equivalent translit param %s (not = new %s) with subst=%s: origt=%s"
                        ...
                        if cursubst_param:
                            ...
                            # combine them.
                            curtr = rulib.xlit_text(apply_substs(val, cursubst_param, pagemsg), pagemsg, semi_verbose)
                            if curtr == newtr:
                                pagemsg(
                                    "NOTE: process_template: Not adding subst=%s because already existing subst=%s has same effect"
                                ...
                                normalized_substs = [
                                    cursubst if "//" in cursubst else cursubst.replace("/", "//")
                                    for cursubst in cursubsts
                                ]
                                for subst in substs:
                                ...
                                pagemsg(
                                    "WARNING: process_template: New tr %s not same as existing tr (from subst=) %s; combined existing subst=%s with new subst=%s to form subst=%s: origt=%s"
                            ...
                            new_subst_param = ",".join(normalized_substs)
                            pagemsg("NOTE: process_template: Added subst=%s to origt=%s" % (new_subst_param, origt))
                            addparam(template, "subst", new_subst_param)
                else:
                    if valtr and valtr != newtr:
                        pagemsg(
                            "WARNING: process_template: Changed translit param %s from %s to %s: origt=%s"
                            % (trparam, valtr, newtr, origt)
                        )
                    if not valtr:
                        pagemsg(
                            "NOTE: process_template: Added translit param %s=%s to template: origt=%s"
                            % (trparam, newtr, origt)
                        )
                    addparam(template, trparam, newtr)
            elif valtr:
                pagemsg(
                    "WARNING: process_template: Template has translit %s but lookup result has none, leaving translit alone: origt=%s"
                    % (valtr, origt)
                )
    ...
    return (
        [
            "auto-accent %s%s%s"
            % (newval, "//%s" % newtr if newtr else "", ", subst=" + ",".join(substs) if substs else "")
        ]
        if changed
        else False
    )
...
def auto_accent_auto_bracket_russian(find_accents, accent_hidden, cattype, direcfile, save, verbose, start, end):
    if direcfile:
        processing_lines = []
        for index, line in blib.iter_items_from_file(direcfile, start, end):
            m = re.match(
                r"^(Page [^ ]+ )(.*?)(: .*?:) Processing: (\{\{.*?\}\})( <- \{\{.*?\}\} \(\{\{.*?\}\}\))$", line
            )
            if not m:
            ...
            def check_template_for_missing_accent(pagetitle, index, pagetext, template, templang, ruparam, trparam):
                def output_line(directive):
                    msg(
                        ...
                        % (pagenum, pagetitle, tempname, directive, str(template), rest)
                    )
            ...
            blib.process_links(
                ...
                None,
                None,
                check_template_for_missing_accent,
                join_actions=join_changelog_notes,
                split_templates=None,
                pages_to_do=[(pagetitle, repltext)],
                quiet=True,
            )
            if index % 100 == 0:
...
parser.add_argument("--find-accents", action="store_true", help="Look up the accents in existing pages")
