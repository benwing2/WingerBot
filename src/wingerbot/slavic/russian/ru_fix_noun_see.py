...
def process_text_on_page(p):
    ...
    return str(parsed), "Replace ru-decl-noun-see with ru-noun-table, taken from headword template (%s)" % str(
        headword_template.name
    )


parser = blib.create_argparser(
    "Convert ru-decl-noun-see into ru-noun-table decl template, taken from headword ru-(proper )noun+ template",
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_refs=["Template:ru-decl-noun-see"]
)
