        ...
        if (
            ...
            for arg_set in arg_sets:
                ...
                if not m:
                    ...
                    if not m:
                        p.errandmsg("Unrecognized conjugation type: %s" % arg_set[0])
                        continue
                ...
                if new_style:
                    ...
                    if conj_type in ["1a", "2a", "2b"]:
                        ...
                        p.errandmsg("Unknown conjugation type " + conj_type)
                    if inf:
                        if refl:
            ...
            if orig_arg_sets != arg_sets or rm_pres_stem:
                ...
                if not p.title.startswith("User:Benwing2/"):
                    ...
                    orig_result = p.expand_text(orig_tempcall)
                    if not orig_result:
                        p.errandmsg("WARNING: Error expanding original template %s" % orig_tempcall)
                        continue
                    ...
                    new_result = p.expand_text(new_tempcall)
                    if not new_result:
                        p.errandmsg("WARNING: Error expanding new template %s" % new_tempcall)
                        continue
