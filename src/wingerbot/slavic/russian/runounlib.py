    ...
    for i in range(1, highest_numbered_param + 2):
        ...
        if end_arg_set:
            ...
            if i - offset == 1 and not arg1_is_stress(val):
                ...
                arg_set.append("")
            if i - offset > 4:
                pagemsg("WARNING: Too many arguments for argument set: arg %s = %s" % (i, (val or "(blank)")))
            arg_set.append(val)
...
def check_old_noun_headword_forms(headword_template, args, subpagetitle, pagemsg, laxer_comparison=False):
    ...
    def compare_forms(case, form1, form2, form1_lemma=False):
        ...
        if form1:
            if (
                set(form1) == set(form2)
                or set(try_to_stress(x) for x in form1) == set(form2)
                or len(form1) == 1
                and len(form2) == 1
                and compare_single_form(form1[0], form2[0])
            ):
                pass
            else:
                pagemsg(
                    "WARNING: case %s, existing forms %s not same as proposed %s"
    ...
    if compare_genders(genders, proposed_genders):
        ...
        if (cur_in or not cur_an) and proposed_an or (cur_an or not cur_in) and proposed_in:
            pagemsg(
                "WARNING: Animacy mismatch, skipping: cur=%s proposed=%s"
...
def fix_old_headword_params(headword_template, new_params, genders, pagemsg):
    ...
    for param in headword_template.params:
        ...
        if name not in ["1", "2", "3", "4"] and re.search(r"^[0-9]+$", name):
            pagemsg(
                "WARNING: Extraneous numbered param %s=%s in headword template, skipping"
    ...
    for param in headword_template.params:
        name = str(param.name)
        if name not in ["1", "2", "3", "4", "g", "gen", "pl", "tr"] and not re.search(
            r"^(head|g|gen|pl|tr)[0-9]+$", name
        ):
...
def convert_zdecl_to_ru_noun_table(decl_z_template, subpagetitle, pagemsg, headword_template=None):
    ...
    if not m:
        pagemsg("WARNING: Unable to recognize z-decl gender/anim spec, skipping: %s" % zgender_anim)
        return None
    ...
    need_gender = (
        re.search("[иы]́?$", zlemma)
        or zgender == "n"
        and re.search("[яа]́?$", zlemma)
        or zgender == "m"
        and re.search("[яа]́?$", zlemma)
        and "(1)" in zspecial
        or zlemma.endswith("ь")
    )
    if not need_gender:
        normal_gender = re.search("[оеё]́?$", zlemma) and "n" or re.search("[ая]́?$", zlemma) and "f" or "m"
    ...
    if rulib.try_to_stress(subpagetitle) == stressed_lemma:
        pagemsg(
            "Removing lemma %s because identical to subpagetitle %s (modulo monosyllabic stress differences): %s"
    ...
    def anim_mismatch(zdecl_an, allowed_headword_ans):
        if headword_anim_spec and headword_anim_spec not in allowed_headword_ans:
            pagemsg(
                ...
                % (zdecl_an, headword_anim_spec, ",".join(allowed_headword_ans), zdecl, str(headword_template))
            )
    ...
    if zanim == "an":
        ...
        if headword_anim_spec in ["ai", "ia"]:
            pagemsg("Converting z-decl -inan to a=%s: %s" % (headword_anim_spec, zdecl))
    ...
    if znum:
        if znum == "pl":
            pagemsg("WARNING: Found n=pl in z-decl, should convert manually to plural lemma: %s" % zdecl)
    ...
    for param in preserve_params:
        ...
        pagemsg(
            "Preserving z-decl override %s=%s%s%s: %s"
            % (
                newparam,
                newval,
    ...
    if loc:
        if loc == "в":
            ...
            newloc = "в/на +"
        pagemsg("Preserving z-decl locative loc=%s (canonicalized from loc=%s): %s" % (newloc, loc, zdecl))
        decl_template.add("loc", newloc)
    par = getp("par")
    if par:
        newpar = "+"
        pagemsg("Preserving z-decl partitive par=%s (canonicalized from par=%s): %s" % (newpar, par, zdecl))
