    ...
    for t in parsed.filter_templates():
        ...
        if tn in ru_head_templates:
            ...
            if headtype in ru_heads_to_warn_about:
                p.msg("WARNING: Found %s" % headname)
            found_this_head = True
        if found_this_head:
...
parser = blib.create_argparser(
    "Find Russian terms without a proper headword line", include_pagefile=True, include_stdin=True
)
args = parser.parse_args()
start, end = blib.parse_start_end(args.start, args.end)

blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, new=True, default_cats=["Russian %x" for x in poses]
)
output_heads_seen()
