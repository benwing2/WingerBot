...
"""
        ...
        if etym.startswith("acr:"):
            ...
            if etym == "r":
                ...
                if m:
                    ...
                    langtext = ""
                etymtext = "%s{{affix|ru|%s%s}}%s" % (prefix, "|".join(do_split(r"\+", etym)), langtext, suffix)
        etymtext = "===Etymology===\n%s\n\n" % etymtext
    ...
    for corverb in corverbs:
        ...
        corverbtext += "|%s%s=%s" % (
            ...
            "" if corverbno == 1 else str(corverbno),
            re.sub(r"^\*", "", corverb),
        )
        corverbno += 1
    ...
    passivetext = (
        "# {{passive of|lang=ru|%s%s}}\n" % (verbbase, trverbbase and "|tr=%s" % trverbbase or "")
        if etym == "r"
        else ""
    )
...
{{ru-conj|%s|%s|%s}}""" % (
    ...
    for synantrel in els[5:]:
        ...
        if not m:
            error(
                "Element %s doesn't start with syn:, ant:, der:, rel:, see:, pron:, alt:, def:, note:, wiki:, enwiki:, usage:, cat: or tcat:"
                % synantrel
            )
            ...
            if vals == "-":
                ...
                for derrelgroup in do_split(",", vals):
                    ...
                    for derrel in do_split(":", derrelgroup):
                        ...
                        gender_arg = "|g=impf|g2=pf" if headword_aspect == "both" else "|g=" + headword_aspect
                        if "#ref" in derrel:
                            ...
                            corverb_impf_override = any(corverb for corverb in corverbs if corverb.startswith("*"))
                            if isrefl:
                                ...
                                refverb = (
                                    re.search("и́?$", verb) and verb + "сь" or rulib.try_to_stress(verb) + "ся"
                                ) + gender_arg
                                correfverbs = []
                                for corverb in corverbs:
                                    ...
                                    correfverbs.append(
                                        ...
                                        % (
                                            (
                                                re.search("и́?$", corverb)
                                                and corverb + "сь"
                                                or rulib.try_to_stress(corverb) + "ся"
                                            ),
                                            "impf" if headword_aspect == "pf" or impf_override else "pf",
                                        )
                                    )
