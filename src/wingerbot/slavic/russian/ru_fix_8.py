    ...
    for t in parsed.filter_templates():
        ...
        if tname(t) in ["ru-conj"] and re.search(r"^8[ab]", param2):
            ...
            if past_m:
                ...
                stem = getparam(t, "3")
                if stem == past_m:
                    p.msg("Stem %s and past_m same" % stem)
                    ...
                    p.msg("Stem %s and past_m %s are different, putting past_m in param 5" % (stem, past_m))
                    t.add("5", past_m)
...
parser = blib.create_argparser("Fix up class-8 arguments")
args = parser.parse_args()
start, end = blib.parse_start_end(args.start, args.end)

blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_cats=["Russian class 8 verbs"]
)
