...
hard_sign_matching = double_quote_like + ['"']
...
russian_to_latin_lookalikes = dict(
    list(russian_to_latin_lookalikes_lc.items()) + list(russian_to_latin_lookalikes_cap.items())
)
# When converting Latin to Russian, only do lowercase so we don't do phrases
# like X, C++, витамин C, сульфат железа(II), etc.
latin_to_russian_lookalikes = dict([(y, x) for x, y in russian_to_latin_lookalikes_lc.items()])
...
russian_lookalikes_re = "[" + "".join([x for x in russian_to_latin_lookalikes.keys() if len(x) == 1]) + "]"
latin_lookalikes_re = "[" + "".join([x for x in russian_to_latin_lookalikes.values() if len(x) == 1]) + "]"
...
tt_to_russian_matching_uppercase = {
    ...
    # or similar
    "Щ": ["Šč", "Shch", "Sch", "Sč", "Š(č)", "Ŝč", "Ŝć", ("Ŝ",), "Š'", "ʂ", "Sh'", "Š", "Sh"],  # No cap equiv: "ʂ"?
...
tt_to_russian_matching_3char = {
    " — ": [" — ", "—", " - ", "-"],
    "ы́й": ["y" + AC + "j", "yj", "y" + AC + "ĭ", "yĭ", "y" + AC + "i", "yi", "y" + AC + "y", "yy", "y" + AC, "y"],
    "и́й": ["i" + AC + "j", "ij", "i" + AC + "ĭ", "iĭ", "i" + AC + "y", "iy", "y" + AC + "i", "yi", "i" + AC, "i"],
}
...
def pre_canonicalize_latin(text, russian=None, msgfun=msg):
    ...
    # "compose" digraphs
    text = rsub(
        text, "[czskCZSK]h", {"ch": "č", "zh": "ž", "sh": "š", "kh": "x", "Ch": "Č", "Zh": "Ž", "Sh": "Š", "Kh": "X"}
    )
    ...
    if russian:
        if (
            text.startswith("'''")
            and text.endswith("'''")
            and not russian.startswith("'''")
            and not russian.endswith("'''")
        ):
            text = text[3:-3]
        elif (
            text.startswith("''")
...
def post_canonicalize_latin(text, msgfun=msg):
    ...
    non_cons = r"(^|[aeiouyěɛAEIOUYĚƐʹʺ\W%s%s]%s)" % (capital_e_subst, small_e_subst, ACGROPT)
    # repeat to handle sequences of EEEEE... or eeeee....
    for i in range(2):
        text = re.sub("(%s)%s" % (non_cons, capital_e_subst), r"\1Je", text, 0, re.U)
        text = re.sub("(%s)%s" % (non_cons, small_e_subst), r"\1je", text, 0, re.U)
...
def pre_pre_canonicalize_russian(text, msgfun=msg):
    ...
    if newtext != text:
        if re.search("[A-Za-z]", newtext):
            msgfun("WARNING: Russian %s has Latin chars in it after trying to correct them, not correcting" % text)
        # Don't do things like расставить все точки над i
        elif re.search(r"\b[A-Za-z]\b", text, re.U):
            msgfun("WARNING: Russian %s has one-char Latin word in it, not correcting" % text)
        else:
            text = newtext
...
def tr_matching(russian, latin, err=False, msgfun=msg):
    ...
    for accent, english in [(AC, "acute"), (GR, "grave")]:
        for word in russian_words:
            if len(rsub(word, "[^" + accent + "]", "")) > 1:
                msgfun("WARNING: Russian %s has multiple %s accents" % (russian, english))
        for word in latin_words:
            if len(rsub(word, "[^" + accent + "]", "")) > 1:
                msgfun("WARNING: Latin %s has multiple %s accents" % (latin, english))

    # Change grave to acute if no acute accent also in word and only one
    # grave accent in word.
    new_latin_words = []
    for word in latin_words:
        if re.search(GR, word) and not re.search(AC, word) and len(rsub(word, "[^" + GR + "]", "")) == 1:
            msgfun("Changing grave to acute in word %s (Latin %s, Russian %s)" % (word, latin, russian))
    ...
    def get_matches():
        ...
        if matches == None:
            if True:
                error("Encountered non-Russian (?) character " + ac + " at index " + str(rind[0]))
            else:
                matches = [ac]
    ...
    def cant_match():
        if rind[0] < rlen and lind[0] < llen:
            error(
                "Unable to match Russian character %s at index %s, Latin character %s at index %s"
                % (ru[rind[0]], rind[0], la[lind[0]], lind[0])
            )
        elif rind[0] < rlen:
            error("Unable to match trailing Russian character %s at index %s" % (ru[rind[0]], rind[0]))
        else:
...
def test(latin, russian, should_outcome, expectedrussian=None):
    ...
    if canonrussian != expectedrussian:
        print("Canon Russian FAILED, expected %s got %s" % (expectedrussian, canonrussian))
