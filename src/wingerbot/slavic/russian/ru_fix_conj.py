...
def process_text_on_page(p):
    ...
    for t in parsed.filter_templates():
        ...
        if tn.startswith("ru-conj-") and tn != "ru-conj-verb-see":
            ...
            notes.append(
                "ru-conj-* -> ru-conj, moving params up by one%s" % (variant and " (and move variant spec)" or "")
            )
            seenval = False
...
parser = blib.create_argparser(
    "Convert ru-conj-* to ru-conj and move variant", include_pagefile=True, include_stdin=True
)
