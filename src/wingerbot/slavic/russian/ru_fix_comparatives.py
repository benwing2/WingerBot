...
def process_text_on_page(index, pagetitle, text):
    ...
    if hascomp:
        if len(headword_templates) > 1 or len(decl_templates) > 1:
            ...
            pagemsg("WARNING: Strange, decl template but no headword template: %s" % str(decl_templates[0]))
        elif len(headword_templates) == 1 and not decl_templates:
            pagemsg("WARNING: Strange, headword template but no decl template: %s" % str(headword_templates[0]))
            ...
            if decl == "-" or decl == "?" or not decl:
                ...
                if not re.search("^[abc]'*$", decl):
                    ...
                    return
                if decl == "a" and not pagetitle.endswith("ой") or decl == "b" and pagetitle.endswith("ой"):
                    compspec = "+"
                else:
                    compspec = "+" + decl
            comparatives = expand_text("{{#invoke:ru-headword|generate_comparative|%s|%s}}" % (head, compspec))
            if not comparatives:
