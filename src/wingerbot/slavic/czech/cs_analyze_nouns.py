    ...
    for t in parsed.filter_templates():
        ...
        def default_nom_pl_animate_masc(lemma, reducible):
            return (
                ...
                re.search("[ňďťščžřj" + cs.labial + "l]an$", lemma)
                and ["é", "i"]  # most now can also take -i
                or
                # proper names: Baťové, Novákové, Petrové, Tomášové, Vláďové; but exclude demonyms
                re.search("^" + cs.uppercase_c, lemma)
                and not re.search("ec$", lemma)
                and ["ové"]
                or
                # demonyms: [[Albánec]], [[Gruzínec]], [[Izraelec]], [[Korejec]], [[Libyjec]], [[Litevec]], [[Němec]], [[Portugalec]]
                re.search("^" + cs.uppercase_c + ".*ec$", lemma)
                and ["i"]
                or
        ...
        def determine_default_reducible(lemma, inferred_decl):
            ...
            if (
                re.search("[iyuíeě]$", lemma)
                or gender == "m"
                and re.search("[ao]$", lemma)
                or ".tstem" in inferred_decl
            ):
                return False
        ...
        def construct_reducible_and_altspec(reducible, default_reducible, altspec):
            if reducible is None and altspec is None:
                return ""
            if reducible == default_reducible or (
                default_reducible == "mixed" and reducible in [[True, False], [False, True]]
            ):
                return altspec
        ...
        def infer_decl(lemma, sgonly=False):
            ...
            if lemma.endswith("o"):
                ...
                if fetch("ins_sg").endswith("í"):
                    ...
                    if not decl:
                        ...
                        is_hard = (
                            (not sgonly and "y" in ins_pl_endings) or "a" in gen_sg_endings or "u" in gen_sg_endings
                        )
                        softhard = "mixed" if is_soft and is_hard else "soft" if is_soft else "hard"
                        default_softhard = (
                            "soft" if re.search("[cčjřšžťďň]$", lemma) or lemma.endswith("tel") else "hard"
                        )
                        if fetch("acc_sg") == fetch("gen_sg"):
                            animate = True
                            decl = "m.an"
            ...
            if compute_overrides == "m":
                ...
                toponym_loc_e = toponym and (
                    toponym_gen_a or re.search("štejn$", lemma) or re.search("[gh]rad$", lemma)
                )
                ...
                expected_gen_sg_ending = (
                    toponym_gen_a and ["a"] or toponym_gen_a_u and ["a", "u"] or not animate and ["u"] or ["a"]
                )
                ...
                expected_loc_sg_ending = (
                    toponym_loc_e and ["ě"] or toponym_loc_e_u and ["ě", "u"] or expected_dat_sg_ending
                )
                ...
                if not sgonly:
                    expected_loc_pl_ending = (
                        ["ích", "ách"]
                        if re.search("[cč]ek$", lemma) and reducible and not animate
                ...
                expected_loc_sg_ending = (
                    ...
                    re.search("dlo$", lemma)
                    and ["ě", "u"]
                    or re.search("lo$", lemma)
                    and ["ě"]
                    or (re.search("[sc]tvo$", lemma) or re.search("ivo$", lemma))
                    and ["u"]
                    or
