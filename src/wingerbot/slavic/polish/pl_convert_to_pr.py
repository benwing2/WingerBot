        ...
        if tn in ["pl-pronunciation", "pl-p"]:
                    ...
                    if caption:
                        ...
                        if m:
                            ...
                            if audio_num_caption in ["Audio", "Audio %s" % i]:
                                pagemsg(
                                    "Ignoring 'Audio #' portion of caption '%s' in %s=: %s" % (caption, param, origt)
                                )
                                caption = text_caption
                            else:
                                pagemsg(
                                    "WARNING: Mismatch in number of 'Audio #' portion of caption '%s' in %s=: %s"
                                    % (caption, param, origt)
                                )
                                stopping_warning.append(
                                    "mismatch in number of 'Audio #' portion of caption '%s' in %s=" % (caption, param)
                                )
                    if caption:
                        m = re.search("^''(.*)''$", caption)
                        ...
                        if caption == p.title:
                            ...
                            pagemsg(
                                "WARNING: Assuming italicized caption %s is text in %s=: %s" % (caption, param, origt)
                            )
            ...
            if not args.dont_compare_pronuns:
                ...
                if ipas:
                    ...
                    pl_p_args = "".join(
                        "|plp%s=%s" % ("" if i == 0 else str(i + 1), respelling)
                ...
                if fixstress and fixstress != "0":
                    if len(pl_p_prons) > 1:
                        pagemsg(
                            "WARNING: Saw multiple respellings along with |fs=%s, can't handle: %s" % (fixstress, origt)
                        )
                        ...
                        if " " in pl_p_pron_1:
                            pagemsg(
                                "WARNING: Saw space in {{pl-p}} output %s along with fs=1, can't handle: %s"
                                % (pl_p_pron_1, origt)
                            )
                ...
                def do_pl_pr_obj(prefix_respelling, pl_pr_obj, do_diff):
                        ...
                        for pl_p_pron, pl_pr_pron in zip(pl_p_prons, pl_pr_prons):
                            ...
                            if len(pl_p_pron_words) == len(pl_pr_pron_words):
                                ...
                                for i in range(len(pl_p_pron_words)):
                                    monostress_words.append(
                                        add_monosyllabic_stress_to_words(pl_p_pron_words[i], pl_pr_pron_words[i])
                                    )
                        ...
                        while True:
                            ...
                            while True:
                                new_pl_pr_pron = re.sub(
                                    ...
                                    lambda m: m.group(1) + long_voicing[m.group(2)] + m.group(3),
                                    pl_pr_pron,
                                )
                                if new_pl_pr_pron == pl_pr_pron:
                                    break
                            ...
                            while True:
                                new_pl_pr_pron = re.sub(
                                    ...
                                    lambda m: m.group(1) + short_voicing[m.group(2)] + m.group(3),
                                    pl_pr_pron,
                                )
                                if new_pl_pr_pron == pl_pr_pron:
                                    break
                    ...
                    for cmpfun, cmp_prefix in cmp_funs:
                        for cmp1, cmp2, cmp_suffix in [
                            ...
                            (
                                syladjusted_joined_pl_p_prons,
                                joined_pl_pr_prons,
                                "_MODULO_MONOSYLLABIC_STRESS_AND_SYL_ADJUST",
                            ),
                            (
                                syladjusted_joined_pl_p_prons,
                                voicing_adjusted_joined_pl_pr_prons,
                                "_MODULO_MONOSYLLABIC_STRESS_SYL_ADJUST_AND_VOICING_ADJUST",
                            ),
                            (
                                syladjusted_joined_pl_p_prons,
                                clitic_adjusted_joined_pl_pr_prons,
                                "_MODULO_MONOSYLLABIC_STRESS_SYL_ADJUST_VOICING_ADJUST_AND_CLITIC_ADJUST",
                            ),
                    ...
                    pagemsg(
                        "{{pl-p%s}} = %s; {{pl-pr%s}} = %s: %s"
                        % (
                            pl_p_args,
                            joined_pl_p_prons,
                            "|" + prefix_respelling if prefix_respelling else pl_pr_args,
                            joined_pl_pr_prons,
                            pron_diff,
                        )
                    )
            ...
            for prefix_respelling, pl_pr_obj in prefix_pl_pr_objs:
                pron_diff, joined_pl_p_prons, joined_pl_pr_prons, auto_hyphenations, auto_rhymes = do_pl_pr_obj(
                    prefix_respelling, pl_pr_obj, do_diff=False
                )
                if pron_diff:
                ...
                pron_diff, joined_pl_p_prons, joined_pl_pr_prons, auto_hyphenations, auto_rhymes = do_pl_pr_obj(
                    None, prefix_pl_pr_objs[0][1], do_diff=True
                )
            ...
            if hyphenations:
                if carry_over_respelling:
                    pagemsg(
                        "WARNING: Keeping hyphenation(s) %s because respelling(s) being carried over, must check manually: %s"
                        % (",".join(hyphenations), origt)
                    )
                    stopping_warning.append(
                        "hyphenation(s) kept because respelling(s) carried over, must check manually"
                    )
                elif set(auto_hyphenations) == set(hyphenations):
                    pagemsg(
                        "Ignoring hyphenation(s) %s same as auto-hyphenation(s): %s" % (",".join(hyphenations), origt)
                    )
                    hyphenations = []
                else:
                    pagemsg(
                        "WARNING: Keeping hyphenation(s) %s not same as auto-hyphenation(s) %s, must check manually: %s"
                        % (",".join(hyphenations), ",".join(auto_hyphenations) or "-", origt)
                    )
                    stopping_warning.append(
                        "hyphenation(s) not same as auto-hyphenation(s) %s, must check manually"
                        % (",".join(auto_hyphenations) or "-")
                    )
            if rhymes:
                if carry_over_respelling:
                    pagemsg(
                        "WARNING: Keeping rhyme(s) %s because respelling(s) being carried over, must check manually: %s"
                        % (",".join(rhymes), origt)
                    )
                    ...
                    pagemsg(
                        "WARNING: Keeping rhyme(s) %s not same as auto-rhyme(s) %s, must check manually: %s"
                        % (",".join(rhymes), ",".join(auto_rhymes), origt)
                    )
                    stopping_warning.append(
                        "rhyme(s) not same as auto-rhyme(s) %s, must check manually" % ",".join(auto_rhymes)
                    )
            ...
            if no_change:
                modt = list(
                    blib.parse_text(
                        ...
                        % (
                            "|" + new_respellings
                            if new_respellings and (has_prefix_respelling or carry_over_respelling)
                            else ""
                        )
                    ).filter_templates()
                )[0]
            ...
            if no_change:
                ...
                msg(
                    ...
                    % (
                        ...
                        joined_pl_pr_prons,
                        pron_diff,
                        "; ".join(stopping_warning) if stopping_warning else "",
                        str(modt),
                    )
                )
            else:
...
parser.add_argument(
    ...
    action="store_true",
    help="Disable comparing generated {{pl-p}} and {{pl-pr}} pronuns; for testing only.",
)
