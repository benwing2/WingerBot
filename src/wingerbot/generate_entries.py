...
parser.add_argument(
    "--pos",
    help="Specify part of speech (v, n, pn, adj, adjform, adv, pcl, pred, prep, conj, int) instead of including it as first field.",
)
...
def get_els_and_pos(line, error):
    ...
    els = [
        el.replace(r"\s", " ") if i == 2 and (pos in ["n", "pn", "adj"]) else el.replace("_", " ").replace(r"\u", "_")
...
%s\n\n""" % (
        ...
        if etym.startswith("acr:"):
            ...
            etymtext = "%s{{%s|%s|%s|%s%s}}." % (
                prefix,
                ("der" if use_der else "bor+"),
                lang,
                m.group(1),
                m.group(2),
    ...
    if pos == "v":
        ...
        if lang == "bg":
            ...
            if (
                conjparts[0] == "1a"
                and not re.search("[аяі]́?ти$", non_refl_verb)
                or conjparts[0] in ["2a", "2b"]
                and not re.search("[ую]ва́?ти$", non_refl_verb)
                or conjparts[0] == "3a"
                and not re.search("нути$", non_refl_verb)
                or conjparts[0] in ["3b", "3c"]
                and not re.search("ну́ти$", non_refl_verb)
                or conjparts[0] == "4a"
                and not re.search("[иї]ти$", non_refl_verb)
                or conjparts[0] in ["4b", "4c"]
                and not re.search("[иї]́ти$", non_refl_verb)
                or conjparts[0] in ["5a", "6a"]
                and not re.search("[аія]ти$", non_refl_verb)
                or conjparts[0] in ["5b", "5c", "6b", "6c"]
                and not re.search("[аія]́ти$", non_refl_verb)
            ):
            ...
            if is_reflexive and has_ppp_spec:
                ...
                error(
                    "Non-reflexive transitive verb %s must be specified for having/not having a PPP: %s"
                    % (term[0], conj)
                )
            aspect = None
    ...
    for t in term:
        ...
        if lang != "pt":
            ...
            if pos == "n" and not is_invar_gender and re.search(r"\|(adv|absn)", hdecltext):
                error(
                    "Term %s is supposed to be a noun but has adjective properties in the declension: %s"
                    % (t, hdecltext)
                )
    ...
    for synantrel in remainder:
        ...
        if sartype == "also":
            ...
            synantguts = "====%s====\n%s\n" % ("Synonyms" if sartype == "syn" else "Antonyms", "".join(lines))
            if sartype == "syn":
                syntext = synantguts
            else:
                anttext = synantguts
...
# {{inflection of|%s|%s||%s|m|s|past|pass|part}}\n\n""" % (
            ...
            if (
                (sartype == "der" and dertext != None)
                or (sartype == "rel" and reltext != None)
                or (sartype == "see" and seetext != None)
            ):
            ...
            if vals == "-":
                ...
                for derrelgroup in do_split(",", vals):
                    ...
                    lines.append("* %s\n" % ", ".join(links))
                derrelguts = "====%s====\n%s\n" % (
                    "Derived terms" if sartype == "der" else "Related terms" if sartype == "rel" else "See also",
                    "".join(lines),
                )
                if sartype == "der":
                    dertext = derrelguts
...
while True:
    ...
    while True:
        ...
        nextline_lemmas, nextline_first_lemma_no_accents, starts_with_exclamation_point, nextline_prons = get_lemmas(
            nextline
        )
        if starts_with_exclamation_point and first_lemma_no_accents != nextline_first_lemma_no_accents:
            fatal(
                line,
                "If lemma %s starts with exclamation point, it must be the same as previous lemma %s without accents"
    ...
    for etymnum, (etym_section, pronuns) in enumerate(etym_sections):
        for lemmanum, lemmaline in enumerate(etym_section):
            ...
            headertext, bodytext, footertext = process_line(
                lemmaline,
                first_lemma_no_accents,
