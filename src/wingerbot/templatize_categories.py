...
def process_text_on_page(p, langcode, langname, topicstemp):
    ...
    newtext = re.sub(
        r"\[\[\s*[Cc][Aa][Tt]\s*:\s*(%s)\s*:\s*(.*?)\s*\]\]" % escaped_langcode, r"[[Category:\1:\2]]", text
    )
    if newtext != text:
    ...
    if newtext != text:
        ...
        text = newtext
    newtext = re.sub(
        r"\[\[\s*[Cc][Aa][Tt][Ee][Gg][Oo][Rr][Yy]\s*:\s*(%s)\s*:\s*(.*?)\s*\]\]" % escaped_langcode,
        r"[[Category:\1:\2]]",
        text,
    )
    if newtext != text:
        ...
        text = newtext
    newtext = re.sub(
        r"\[\[\s*[Cc][Aa][Tt][Ee][Gg][Oo][Rr][Yy]\s*:\s*(%s[ _].*?)\s*\]\]" % escaped_langname, r"[[Category:\1]]", text
    )
    if newtext != text:
    ...
    newtext = re.sub(
        ...
        lambda m: process_sort_key(m, "%s|%s" % (topicstemp, langcode)),
        text,
    )
    if newtext != text:
        notes.append(
            "templatize topical categories with sort key for langcode=%s using {{%s}}" % (langcode, topicstemp)
        )
        text = newtext
    newtext = re.sub(
        r"\[\[Category:%s:\s*([^|\[\]{}]*?)\s*\]\]" % escaped_langcode, r"{{%s|%s|\1}}" % (topicstemp, langcode), text
    )
    if newtext != text:
        notes.append("templatize topical categories for langcode=%s using {{%s}}" % (langcode, topicstemp))
        text = newtext
    newtext = re.sub(
        r"\{\{\s*(?:c|C|top|topic|topics|catlangcode)\s*\|\s*%s\s*\|\s*(.*?)\s*\}\}" % escaped_langcode,
        r"{{%s|%s|\1}}" % (topicstemp, langcode),
        text,
    )
    if newtext != text:
    ...
    while True:
        newtext = re.sub(
            r"\{\{%s\|%s\|([^=\[\]{}]*)\}\}\n\{\{%s\|%s\|([^=\[\]{}]*)\}\}"
            % (re.escape(topicstemp), escaped_langcode, re.escape(topicstemp), escaped_langcode),
            r"{{%s|%s|\1|\2}}" % (topicstemp, langcode),
            text,
        )
        if newtext != text:
    ...
    newtext = re.sub(
        ...
        lambda m: process_sort_key(m, "cln|%s" % langcode),
        text,
    )
    if newtext != text:
    ...
    if newtext != text:
        ...
        text = newtext
    newtext = re.sub(
        r"\{\{\s*(?:catlangname|cln)\s*\|\s*%s\s*\|\s*(.*?)\s*\}\}" % escaped_langcode,
        r"{{cln|%s|\1}}" % langcode,
        text,
    )
    if newtext != text:
    ...
    while True:
        newtext = re.sub(
            r"\{\{cln\|%s\|([^=\[\]{}]*)\}\}\n\{\{cln\|%s\|([^=\[\]{}]*)\}\}" % (escaped_langcode, escaped_langcode),
            r"{{cln|%s|\1|\2}}" % langcode,
            text,
        )
        if newtext != text:
