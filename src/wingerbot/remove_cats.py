...
def process_text_on_page(p):
    ...
    for t in parsed.filter_templates():
        tn = tname(t)
        if (
            tn in topics_templates
            or tn in catlangname_templates
            or tn in categorize_templates
            or tn in no_lang_templates
        ):
    ...
    if removed_cats:
        notes.append(
            "remove categories: %s%s" % (",".join(removed_cats), " (%s)" % args.comment if args.comment else "")
        )
