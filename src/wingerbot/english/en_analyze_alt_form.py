...
for lineno, line in blib.iter_items_from_file(args.direcfile, start, end):
    ...
    m = re.search(
        r"^Page ([0-9]+) (.*?): Found match for regex:.*?\{\{(?:alternative form of|alt form|altform|alt form of)\|en\|(.*?)[|}]",
        line,
    )
    if m:
    ...
    if not m:
        m = re.search(
            r"^Page ([0-9]+) (.*?): Found match for regex:.*?\{\{(?:alternative spelling of|alt sp|alt spell|altspell|altsp|alt sp of|alt spelling of)\|en(?:\|from=[^{}=|]*)?\|(.*?)[|}]",
            line,
        )
        if m:
            from_template = "altsp"
    ...
    transformations = [
        ...
        (
            lambda x: re.sub(r"\b([Aa])rse", r"\1ss", x),
            "arse -> ass",
            Pondian("arse-form", "ass-form", pronun_different=True),
        ),
        ...
        (
            lambda x: re.sub("([Aa])luminium", r"\1luminum", x),
            "aluminium -> aluminum",
            Pondian("aluminium-form", "aluminum-form", pronun_different=True),
        ),
        (
            lambda x: re.sub("([Nn])eurone\b", r"\1euron", x),
            "neurone -> neuron",
            Pondian("neurone-form", "neuron-form", pronun_different=True),
        ),
        ...
        (
            lambda x: re.sub("([Ss])ceptic", r"\1keptic", x),
            "sceptic -> skeptic",
            Pondian("sceptic-form", "skeptic-form"),
        ),
        (
            lambda x: re.sub("([Aa])rtefact", r"\1rtifact", x),
            "artefact -> artifact",
            Pondian("artefact-form", "artifact-form"),
        ),
        ...
        (
            lambda x: re.sub("([Pp])ractise", r"\1ractice", x),
            "practise -> practice",
            Pondian("practise-form", "practice-form"),
        ),
        (
            lambda x: re.sub("([Mm])anoeuvre", r"\1aneuver", x),
            "manoeuvre -> maneuver",
            Pondian("manoeuvre-form", "maneuver-form"),
        ),
        (
            lambda x: re.sub("([Dd]ef|[Oo]ff|[Ll]ic|[Pp]ret)ence", r"\1ense", x),
            "defence/offence/licence/pretence -> defense/offense/license/pretense",
            Pondian("ence-form", "ense-form"),
        ),
        (
            lambda x: re.sub(
                r"([Cc]ol|[FfSs]av|[Ll]ab|[Vv]ap|[Oo]d|[Ss]ucc|[Hh]arb|[Aa]r[bm]|[TtHhRr]um|[Rr]ig|[Bb]ehavi|[Ee]nam|[Ee]ndeav|[Nn]eighb|[Cc]lam|[Dd]emean|[Hh]on|[Rr]anc|[Ss]avi|[Pp]arl|[Gg]lam)our",
                r"\1or",
                x,
            ),
            "common-word -our -> -or",
            Pondian("our-form", "or-form"),
        ),
        (
        ...
        (
            ise_to_ize,
            "-ise/-iser/-ises/-ised/-is(e)ing/-is(e)ation(al)/is(e)able/is(e)ability -> same with -iz-",
        ...
        (
            lambda x: re.sub(r"ll(|[eo]r'?s?'?|ed|en|ist|ing|ful|ous|ate(?:[drs]|rs)?)\b", r"l\1", x),
            "ll -> l",
            Pondian("ll-form", "l-form"),
        ),
    ]
    ...
    if to_page.replace("'", "") == from_page.replace("'", ""):
        if check_for_pronounced_schwa(to_page) or check_for_pronounced_schwa(from_page):
            pagemsg(
                "WARNING: Saw from-page '%s' same as to-page with apostrophes removed and needs manual checking for pronounced schwa"
                % from_page
            )
        else:
    ...
    if remove_accents(to_page) == remove_accents(from_page):
        if check_for_accented_ed(to_page) or check_for_accented_ed(from_page):
            pagemsg(
                "WARNING: Saw from-page '%s' same as to-page with accents removed and needs manual checking for accented -ed"
                % from_page
            )
        else:
    ...
    if mould_to_mold(to_page) == mould_to_mold(from_page):
        ...
        continue
    if defence_offence_licence_to_defense_offense_license(
        to_page
    ) == defence_offence_licence_to_defense_offense_license(from_page):
        pagemsg(
            "Saw from-page '%s' same as to-page with defence/offence/licence -> defense/offense/license" % from_page
        )
        continue
    ...
    if ise_to_ize(to_page) == ise_to_ize(from_page):
        pagemsg(
            "Saw from-page '%s' same as to-page with -ise/-iser/-ises/-ised/-is(e)ing/-is(e)ation(al)/is(e)able/is(e)ability -> same with -iz-"
            % from_page
        )
        continue
    if ise_to_ize(to_page, omit_extra_e=True) == ise_to_ize(from_page, omit_extra_e=True):
        pagemsg(
            "Saw from-page '%s' same as to-page with -iseing/-iseation(al)/iseable/iseability -> same with -iz- and omit extra -e-"
            % from_page
        )
        continue
    if ise_to_ize(to_page, with_y=True) == ise_to_ize(from_page, with_y=True):
        pagemsg(
            "Saw from-page '%s' same as to-page with -yse/-yser/-yses/-ysed/-ys(e)ing/-ys(e)ation(al)/ys(e)able/ys(e)ability -> same with -yz-"
            % from_page
        )
        continue
    if ise_to_ize(to_page, omit_extra_e=True, with_y=True) == ise_to_ize(from_page, omit_extra_e=True, with_y=True):
        pagemsg(
            "Saw from-page '%s' same as to-page with -yseing/-yseation(al)/yseable/yseability -> same with -yz- and omit extra -e-"
            % from_page
        )
        continue
    ...
    if ll_to_l(to_page) == ll_to_l(from_page):
        pagemsg(
            "Saw from-page '%s' same as to-page with -ll(ed)/-ller(s)/-lling/-llate(d)/-llater(s) -> same with -l-"
            % from_page
        )
        continue
