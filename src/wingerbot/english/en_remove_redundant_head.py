...
def canonicalize_existing_linked_head(head, pagemsg, link_the=True):
    ...
    for i, word in enumerate(words):
        if word == "[[one]][['s]]":
            ...
            "with",
            "under",
        ] or (word == "a" and i > 0 and words[i - 1] == " " and i < len(words) - 1 and words[i + 1] == " "):
...
def process_text_on_page(p):
    ...
    def add_single_word_links(space_word, data):
        ...
        for j, word in enumerate(words):
            if j < len(words) - 1 and data.include_hyphen_prefixes and word in data.include_hyphen_prefixes:
                ...
                if (
                    (not data.no_split_apostrophe_words or word not in data.no_split_apostrophe_words)
                    and data.split_apostrophe
                    and "'" in word
                ):
    ...
    def add_links_to_multiword_term(term, data):
        if "[" in term or "]" in term:
            return term
        if " " not in term:
            data = MultiwordLinksData(
                True, data.split_apostrophe, data.no_split_apostrophe_words, data.include_hyphen_prefixes
            )
        words = term.split(" ")
        linked_words = []
        for word in words:
    ...
    def get_autohead(pagename, t):
        ...
        if getp("nolinkhead") or not re.search(r"[ '-\[\]]", pagename):
            ...
            return add_links_to_multiword_term(
                pagename,
                MultiwordLinksData(
                    en_split_hyphen_when_space,
                    en_split_apostrophe,
                    en_no_split_apostrophe_words,
                    en_include_hyphen_prefixes,
                ),
        ...
        if params_to_check:
            ...
            for param in params_to_check:
                ...
                if " " in paramval and paramval == p.title:
                    ...
                    t.add("nolinkhead", "1")
                    notes.append(
                        "convert %s=%s in {{%s}} that's identical to pagename to nolinkhead=1" % (param, paramval, tn)
                    )
                    continue
                ...
                if "[[" in paramval:
                    ...
                    if canonval == paramval:
                        ...
                        rmparam(t, param)
                        notes.append(
                            "remove redundant %s=%s (canonicalized to %s) in {{%s}}" % (param, paramval, canonval, tn)
                        )
                        continue
                if paramval:
...
blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_cats=["English lemmas"],
)
