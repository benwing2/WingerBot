...
possible_onsets = set(
    [
        ...
        "v",
        "vj",
        "vɹ",  # arguable whether we should include vɹ
        "w",
        "z",
        "ð",
        "ðj",
...
def process_cmu_pronun(index, word, pronun):
    ...
    # ɚ before vowel -> əɹ
    ipa_word = rsub_repeatedly("ɚ(" + accent_c + "?" + vowel_c + ")", r"əɹ\1", ipa_word)
    # ɚ after vowel -> əɹ
    ipa_word = rsub_repeatedly("(" + vowel_c + ")ɚ", r"\1əɹ", ipa_word)
    # ɝ before vowel -> ɜɹ
    ipa_word = rsub_repeatedly("ɝ(" + accent_c + "?" + vowel_c + ")", r"ɜɹ\1", ipa_word)
    # əl after consonant not before vowel -> l̩
    ipa_word = rsub_repeatedly("(" + consonant_c + ")əl($|" + consonant_c + ")", r"\1l̩\2", ipa_word)
    # ən after alveolar and not before vowel -> n̩
    ipa_word = rsub_repeatedly("([tdszln])ən($|" + consonant_c + ")", r"\1n̩\2", ipa_word)
    # Move stress before any initial consonant cluster
    ipa_word = re.sub("^(" + consonant_c + "+)(" + accent_c + ")", r"\2\1", ipa_word)
    ...
    ipa_word = re.sub("(" + vowel_c + ")(" + consonant_c + "+)(" + accent_c + ")", move_before_onset, ipa_word)
