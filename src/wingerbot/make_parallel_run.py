...
parser.add_argument(
    "--overlap",
    help="Number of terms that each run will overlap with the next run. Set to 0 for no overlap.",
    type=int,
    default=5,
)
parser.add_argument(
    "--no-sleep",
    help="Don't sleep at beginning of runs (normally done so offsets will remain true; not necessary if offsets won't change as files are saved).",
    action="store_true",
)
...
for run in range(args.num_parts):
    num_terms_per_run = (
        first_part_num_terms if run == 0 else second_part_num_terms if run == 1 else other_part_num_terms
    )
    sleep_prefix = ""
    ...
    command_with_indices = (
        command_with_indices.replace("%START", str(first_term_index))
        .replace("%END", str(last_term_index))
        .replace("%SAVE", save)
        .replace("%SLEEP", sleep_prefix)
    )
