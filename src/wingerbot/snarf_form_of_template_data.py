...
def process_text_on_page(p):
    ...
    for t in parsed.filter_templates():
        ...
        if tn == "#invoke:form of/templates":
            ...
            cats = [
                re.sub(
                    r"\{\{#invoke:form of/templates\|normalize_pos\|\{\{#if:\{\{\{p\|\}\}\}\|\{\{\{p\|\}\}\}\|\{\{#if:\{\{\{POS\|\}\}\}\|\{\{\{POS\|\}\}\}\|(.*?)\}\}\}\}\}\}",
                    r"{{{p|{{{POS|\1}}}}}}",
                    cat,
                )
                for cat in cats
...
parser = blib.create_argparser("Generate input to make_form_of_table.py")
args = parser.parse_args()
start, end = blib.parse_start_end(args.start, args.end)

blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, default_cats=["Form-of templates"]
)
