    ...
    for t in parsed.filter_templates():
        ...
        if tn in quote_templates:
            ...
            if re.search(r"^\{\{ja-usex\|.*\}\}$", textval, re.S):
                ...
                pagemsg("WARNING: Found {{ja-usex| embedded in quote text but not whole param: %s" % origt)

    return newtext, notes


parser = blib.create_argparser("Move ja-usex calls outside of quote-*", include_pagefile=True, include_stdin=True)
args = parser.parse_args()
start, end = blib.parse_start_end(args.start, args.end)

blib.do_pagefile_cats_refs(
    args, start, end, process_text_on_page, edit=True, stdin=True, default_refs=["Template:ja-usex"]
)
