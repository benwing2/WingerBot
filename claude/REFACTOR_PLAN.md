...
| Top-level `*.py` [p-z] | ~46 | ✅ DONE | 46 files auto-refactored; manual fixes: push_find_regex_changes (6-arg→(p,contents,prev_comment,origcontents); pagemsg/errandpagemsg→p.msg/p.errandmsg; do_process_text_on_page 3-arg→1-arg p; do_process_page wrapped with ProcessPageParams); put_attributive_first (remove unused errpagemsg def, errandmsg import; text→p.text); remove_redundant_bidi_marks (text→p.text in 2 calls); rename_lang_of_entry (first text→p.text); rename_lang_specific_form_of_templates (index→p.index, errandmsg→p.errandmsg); rename_quote_templates (remove invalid def p.msg block); reorder_related_terms_descendants (text→p.text); rewrite_multiple_doublets (text→p.text in re.sub and comparison); rewrite_projectlinks (add text=p.text before loop); snarf_numbers (extra-param sig→(p,langcodes); do_process wrapper); sort_and_reformat_translations (text→p.text); split_regional_label_module (index→p.index, text→p.text); split_rfvn (text→p.text); swap_pronun_and_etym (text→p.text); templatize_categories (extra-param sig→(p,langcode,langname,topicstemp); do_process wrapper); templatize_given_name_cognate (text→p.text; loop index left alone); templatize_name_translit (text→p.text); templatize_place (index→p.index x2, text→p.text) |
