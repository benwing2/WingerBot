...
def canonicalize_form(term, tr):
  ...
  elif term.endswith(ar.TAM) and not term.endswith(ar.AH) and not term.endswith(ar.ALIF + ar.TAM):
    term = term[:-1] + ar.AH
...
def parse_term_with_tr(termspec):
  ...
  for k in range(1, len(segs), 2):
    ...
    if seg.startswith("<tr:"):
      if tr is not None:
        raise BadTranslitException("WARNING: Saw <tr:...> twice at position %s=%s in %s" % (k, segs[k], termspec))
      tr = seg[4:-1]
    else:
...
def process_text_on_page(index, pagetitle, text):
  ...
  noun_inflections = [
    ...
    ("pl", "plural", lambda g: default_feminine_plural if g == "f" else default_masculine_plural),
  ...
  adj_templates = {
    "ar-adj", "ar-act-participle", "ar-pass-participle",
  }
  adj_inflections = [
  ...
  for t in parsed.filter_templates():
    try:
      ...
      for inflspec in inflections:
        field, desc, default_fn = inflspec
        if genders:
          default_fn = default_fn(genders)
        val = getp(field)
        if val:
          # split on comma but not inside of <...>
          ...
          if len(defaults) == 1:
            ...
            for seg in segs:
              ...
              if segterm is None:
                pagemsg("WARNING: Couldn't parse term and translit in %s=%s: %s" % (field, seg, str(t)))
                must_continue = True
                break
              ...
              if def_segterm == canon_segterm and def_segtr == canon_segtr:
                pagemsg("Replaced %s=%s with +: %s" % (field, canonseg, str(t)))
                newsegs.append("+" + segother)
                notes.append("replace %s=%s with +%s in {{%s}}" % (field, seg, segother, tn))
              elif seg == headspec:
            ...
            if set(defaults) <= set(canon_segs):
              saw_plus = False
              for (canon_segterm, canon_segtr), (segterm, segtr, segother) in zip(canon_segs, orig_segs):
      ...
      if tn == "ar-adj":
        ...
        if f == "+" or pl == "+" or fpl == "+":
          if pl and fpl:
            ...
            if pl:
              pagemsg("NOTE: Would replace {{ar-adj}} with auto-defaulting {{ar-adj+}} but feminine plural is missing: %s"
                % str(t))
            else:
              pagemsg("NOTE: Would replace {{ar-adj}} with auto-defaulting {{ar-adj+}} but masculine and feminine plural are missing: %s"
                % str(t))
...
parser = blib.create_argparser("Convert Arabic headwords to use + for default",
