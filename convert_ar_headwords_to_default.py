...
def canonicalize_form(term, tr):
  ...
  elif term.endswith(ar.TAM) and not term.endswith(ar.AH) and not term.endswith(ar.ALIF + ar.TAM) and not term.endswith(ar.AMAD + ar.TAM):
    term = term[:-1] + ar.AH
...
def process_text_on_page(index, pagetitle, text):
  ...
  noun_inflections = [
    ...
    ("pl", "plural", lambda g: default_feminine_plural if g == "f" else [
      ("+", default_masculine_plural), ("+f", default_feminine_plural)]),
            ...
            if len(defaults) == 1:
              ...
              for seg in segs:
                ...
                if def_segterm == canon_segterm and def_segtr == canon_segtr:
                  ...
                  notes.append("replace %s=%s with %s%s in {{%s}}" % (field, seg, plus_symbol, segother, tn))
                elif seg == headspec:
