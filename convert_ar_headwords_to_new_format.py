...
def convert_inflection(t, pref, pagemsg, fetch_nominal_inflections=False, no_join=False, heads=None):
  ...
  if fetch_nominal_inflections:
    ...
    if num_consvals > 0 and num_consvals != num_vals:
      pagemsg("WARNING: Saw %s value%s for %s= but %s value%s for %scons=, can't handle: %s" % (
        num_vals, "s" if num_vals > 1 else "", pref, num_consvals, "s" if num_consvals > 1 else "", pref,
        str(t)))
      return None
    for i, consval in enumerate(consvals):
      ...
      if consval is not None and "<" in consval:
        ...
        return None
      if vals[i] is None:
        pagemsg("WARNING: No value for %s%s= but saw %scons%s=, skipping: %s" % (
          pref, paramind, pref, paramind, str(t)))
        return None
      defcons = default_construct_state(vals[i])
      if defcons == consval:
        pagemsg("Construct value %scons%s=%s is the default for value %s%s=%s, removing: %s" % (
    ...
    for suf, desc in [("obl", "oblique"), ("inf", "informal"), ("def", "definite")]:
      ...
      if inflvals:
        pagemsg("WARNING: Saw %s value%s for %s%s=, can't handle, skipping: %s" % (
  ...
  for i, val in enumerate(vals, start=1):
    ...
    if val is None:
      if tr or g or g2:
        pagemsg("WARNING: No value for %s= but saw %str=, %sg= or %sg2=, skipping: %s" % (
      ...
      if not tr and not g and not g2 and pref == "cons":
        ...
        if heads and len(heads) == 1 and "<" not in heads[0]:
          ...
          if defcons == val:
            pagemsg("Head construct value cons%s=%s is the default for value head %s, replacing with '+': %s" % (
...
def convert_head_tr_gender(t, pagemsg):
  ...
  if heads and len(trs) > len(heads):
    pagemsg("WARNING: Saw %s translit%s but only %s head%s, skipping: %s" % (
...
def process_text_on_page(index, pagetitle, text):
  ...
  noun_templates = {
    "ar-noun", "ar-proper noun", "ar-prop", "ar-coll-noun", "ar-sing-noun", "ar-noun-nisba", "ar-noun-sound",
    # ar-numeral uses noun inflections and ar-pron(oun) uses a subset of noun inflections
    "ar-numeral", "ar-pron", "ar-pronoun",
  }
  noun_inflections = [
  ...
  adj_templates = {
    "ar-adj", "ar-adjective", "ar-adj-sound", "ar-adj-in", "ar-adj-an", "ar-nisba", "ar-act-participle", "ar-pass-participle",
  }
  adj_inflections = [
    ("cons", "construct state", True),
    ("def", "definite state", True),
  ...
  head_gender_only_templates = {
      "ar-adj-dual", "ar-adj-fem", "ar-adj-masc", "ar-adj-pl", "ar-adv", "ar-adverb", "ar-con", "ar-interj",
      "ar-noun-dual", "ar-noun-form", "ar-noun-pl", "ar-particle", "ar-prep", "ar-preposition",
  }
  ...
  for t in parsed.filter_templates():
    ...
    for param in t.params:
      pn = pname(param)
      if (pn not in ["1", "2"] and not re.search("^(head|g)[0-9]+$", pn) and not re.search("^tr[0-9]*$", pn) and
          (not possible_inflection_pref_re or not re.search(possible_inflection_pref_re, pn))):
    ...
    for inflspec in inflections:
      if len(inflspec) == 3:
        ...
        inflpref, desc = inflspec
        nosubinfl = False
      converted_infl = convert_inflection(t, inflpref, pagemsg, fetch_nominal_inflections=not nosubinfl, heads=heads)
...
parser = blib.create_argparser("Convert Arabic headwords to use comma-separated items and inline modifiers",
